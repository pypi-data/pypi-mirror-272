...
def computing_sky(gal, band, size_image_phy, imag,
    ...
    if sky_method == 'masking_sources':
        ...
        if version <= '1.5.0':
            ...
            print('median_sky_g = {:.4f}'.format(median_sky))
            print('std_sky_g = {:.4f}'.format(std_sky))
        else:
            sky_method ='none'
...
def plot_stat(morph, gal, band, imag, imagm, median_sky_g, area, t, zp, psfg,
...
def ex_phot_mod (gal, tab_gal, conn, size_image_phy=50, band='R',
    ...
    if mask_stars == 'yes':
        stars_mask, mask_stars, positions = masking_stars(gal, band, size_image_phy, imag,
        std_sky, fwhm=fwhm, threshold=threshold, roundlo=roundlo,
        roundhi=roundhi, sharplo=sharplo, aper_stars=aper_stars, aper_center=
        aper_center, show_plot=show_plot)
    else:
        stars_mask = np.full(imag.shape, False)
