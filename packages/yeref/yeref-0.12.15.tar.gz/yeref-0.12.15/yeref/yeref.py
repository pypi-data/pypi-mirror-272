...
<body>
    ...
    <script>
        ...
        document.getElementById('nft-check').addEventListener('click', async () => {{
            ...
            if (nftAddress !== "") {{ 
                uinfo = await sendRequest("POST", `${{argument}}nft`, {{ 'tid': tid, 'address': tonConnectUI.account.address, 'collection': collection, 'nftAddress': nftAddress, 'initData': tg.initData }})
                await setParameters()
                return
            }}
        }})
