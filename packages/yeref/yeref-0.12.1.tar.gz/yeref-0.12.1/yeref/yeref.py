...
<body>
        ...
        <div class="subscribe">
            ...
            <div class="subscribe-inner">
                ...
                <select id="period">
                    ...
                    <option value="month" selected>Месячная</option>
    ...
    <script>
        ...
        tg.MainButton.onClick(async () => {{
            ...
            await sendRequest("POST", "/cpaysave", body)
            await sleep(333)
            tg.close()
        }})
...
<body>
    <div class="container">
        ...
        <div class="subscribe">
            <div class="subscribe-ft">
                <div class="subscribe-ft-text">{11} {4} {5} {22}</div>
            ...
            <div class="subscribe-nft">
                <div class="subscribe-nft-text">{14}</div>
                <div id="nft-get">{15}</div>
                <div id="nft-check">{16}</div>
                <input type="text" id="nft-address" minlength="48" maxlength="48" placeholder="{17}">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.getElementById('ft-link').textContent = "{13}"
            document.getElementById('nft-link').textContent = "{18}"
        ...
        tg.MainButton.onClick(async () => {{ 
            if (display_ft === 'flex' && !upayinfo['is_upaydt']) {{ tg.showAlert("{23}"); return }}
            else if (display_nft === 'flex' && !upayinfo['is_upaynft']) {{ tg.showAlert("{24}"); return }}
        ...
        document.getElementById('ft-send').addEventListener('click', async () => {{
            try {{
                ...
                if (!(tonConnectUI.connected)) {{ tg.showAlert("{21}"); return }}
                const tonweb = new window.TonWeb()
                let cell = new tonweb.boc.Cell()
                let payload = ""
                let message = []
