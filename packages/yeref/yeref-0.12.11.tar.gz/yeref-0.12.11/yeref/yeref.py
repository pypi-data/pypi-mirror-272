...
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{            
            ...
            if (window.location.href.split("/").pop().split("?")[0] === 'cpay') {{
            ...
            else if (window.location.href.split("/").pop().split("?")[0] === 'ccomment') {{
                document.getElementById('period').querySelector('option[value="once"]').textContent = "{10}"
                document.getElementById('period').querySelector('option[value="week"]').removeAttribute('value');
                document.getElementById('period').querySelector('option[value="month"]').removeAttribute('value');
                document.getElementById('period').querySelector('option[value="year"]').removeAttribute('value');
                document.getElementById('period').getElementsByTagName('option')[0].selected = true

            }}
            document.querySelector('.address-wallet-text').textContent = "{14}"
            document.getElementById('address-wallet-input-id').setAttribute('placeholder', "{15}")
            ...
            document.getElementById("address-wallet-input-id").value = wallet;
            document.getElementById("address-collection-input-id").value = collection;
            
            let val = document.getElementById('subscribe-datalist-id').value            
            if (val !== 'TON') document.querySelector('.address-wallet').style.display = 'flex'
            else document.querySelector('.address-wallet').style.display = 'none'
            
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-wallet-input-id')).getPropertyValue('width')
            await sleep(333)
            document.querySelector('[data-tc-button="true"]').style.minWidth = window.getComputedStyle(document.getElementById('address-wallet-input-id')).getPropertyValue('width')
        }})
        ...
        document.getElementById('subscribe-datalist-id').addEventListener('input', async () => {{
            let val = document.getElementById('subscribe-datalist-id').value            
            if (val !== 'TON') document.querySelector('.address-wallet').style.display = 'flex'
            else document.querySelector('.address-wallet').style.display = 'none'
            document.querySelector('.address-wallet-text').textContent = `${{addrCollectText}} ${{val}}:*`
        }})
        ...
        tg.MainButton.onClick(async () => {{
            ...
            wallet = document.getElementById("address-wallet-input-id").value
            collection = document.getElementById("address-collection-input-id").value

            if (!(tonConnectUI.connected)) {{ tg.showAlert("{20}"); return }}
            if (currency !== "TON" && wallet === "") {{ tg.showAlert("{21}"); return }}
            if (wallet.length && (wallet.length !== 48 || (!wallet.startsWith("UQ") && !wallet.startsWith("EQ")))) {{ tg.showAlert("{23}"); return }}
            if (collection.length && (collection.length !== 48 || (!collection.startsWith("UQ") && !wallet.startsWith("EQ")))) {{ tg.showAlert("{23}"); return }}
            ...
            console.log('sdfsdf', window.location.href.split("/").pop().split("?")[0])
            await sendRequest("POST", `/${{window.location.href.split("/").pop().split("?")[0]}}save`, body)
            await sleep(333)
            tg.close()
        }})
...
<body>
    <div class="container">
        ...
        <div class="subscribe">
            <div class="subscribe-ft">
                ...
                <a class="optional" id="ft-link">нет активных подписок</a>
                <!-- <a class="optional" id="ft-link">Подписка активирована до 12/01/24</a> -->
            </div>
            <div class="subscribe-nft">
                ...
                <a class="optional" id="nft-link">нет необходимого NFT</a>
                <!-- <a class="optional" id="nft-link">NFT уже получен</a> -->
            </div>
        </div>
        <div class="sticker">
            <tgs-player id="lottie" autoplay loop mode="normal"
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            upayinfo = await sendRequest("POST", "/upayinfo", {{ 'tid': tid, 'initData': tg.initData }})
            await setParameters()
        }})
        
        async function setParameters() {{
            if (Object.keys(upayinfo).length > 0) {{
                console.log('upayinfo', upayinfo)
                document.getElementById('ft-link').textContent = upayinfo['upaydt']
                document.getElementById('nft-link').textContent = upayinfo['upaynft']
                document.getElementById('nft-address').value = upayinfo['nft']
        
                if (upayinfo['is_upaydt']) document.getElementById('ft-send').textContent = '✔️ {12} {4} {5}'
                else document.getElementById('ft-send').textContent = '{12} {4} {5}'
                if (upayinfo['is_upaynft']) document.getElementById('nft-get').textContent = '✔️ {15}'
        ...
        tg.MainButton.onClick(async () => {{ 
            if (display_ft === 'flex' && !upayinfo['is_upaydt']) {{ tg.showAlert("{21}"); return }}
            else if (display_nft === 'flex' && !upayinfo['is_upaynft']) {{ tg.showAlert("{22}"); return }}
        ...
        document.getElementById('ft-send').addEventListener('click', async () => {{
            try {{
                ...
                if (wallet !== "") {{
                    ...
                    cell2.bits.writeString(`${{tid}}_${{tg.initDataUnsafe['user']['id']}}_${{period}}`);
                          
                    cell.bits.writeUint(0xf8a7ea5, 32)
                    cell.bits.writeUint(Date.now(), 64)
                    cell.bits.writeCoins(tonweb.utils.toNano(amount))
                    ...
                    message = [{{
                        address: wallet,
                        amount: tonweb.utils.toNano('0.12').toString(),
                        payload: payload,
                    }}]
                }}
                else {{
                    cell.bits.writeUint(0, 32);
                    cell.bits.writeString(`${{tid}}_${{tg.initDataUnsafe['user']['id']}}_${{period}}`);
                ...
                upayinfo = await sendRequest("POST", "/upayft", body)
                await setParameters()
            }} catch (error) {{
        ...
        document.getElementById('nft-check').addEventListener('click', async () => {{
            if (!(tonConnectUI.connected)) {{ tg.showAlert("{19}"); return }}
            let nftAddress = document.getElementById('nft-address').value.trim()
            console.log('nftAddress', nftAddress)
            
            if (collection !== "" && (nftAddress.length !== 48 || (!nftAddress.startsWith("UQ") && !nftAddress.startsWith("EQ")))) {{
                tg.showAlert("{23}")
                await setParameters()
                return 
            }}
            ...
            if (nftAddress !== "") {{ 
                upayinfo = await sendRequest("POST", "/get_nft_data", {{ 'tid': tid, 'address': tonConnectUI.account.address, 'collection': collection, 'nftAddress': nftAddress, 'initData': tg.initData }})
                await setParameters()
                return
            }}
        }})
        
        let startUrl = `/web?tgWebAppStartParam=${{tid}}_upay&${{tg.initData}}`
        console.log('startUrl = ', startUrl)
        fetchData(startUrl)
    </script>
</body>
