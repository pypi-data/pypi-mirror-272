...
async def get_buttons_main(lz, bot_un, BASE_D):
    ...
    try:
        result = [
            ...
            types.InlineKeyboardButton(text=f"{(await read_likes(BASE_D))}♥️", callback_data=f"like"),
            ...
            types.InlineKeyboardButton(text="ᵇᵒᵗ⁺", switch_inline_query_current_chat=f"~")
        ]
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
