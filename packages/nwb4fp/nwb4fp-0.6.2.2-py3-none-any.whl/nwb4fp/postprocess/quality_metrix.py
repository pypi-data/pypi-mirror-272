...
def qualitymetrix(path, temp_folder):
    ...
    sort_merge.set_property(key='group', values = sorting.get_property("channel_group"))
    analyzer2 = si.create_sorting_analyzer(sorting=sort_merge, recording=rec_w, format='binary_folder', folder=fr"{temp_folder}_output",overwrite=True)
