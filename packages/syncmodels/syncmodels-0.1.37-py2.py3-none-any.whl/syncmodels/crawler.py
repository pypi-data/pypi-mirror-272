...
class iBot(iAgent):
    ...
    def _build_params(self, **context):
        ...
        wave0 = context.get(MONOTONIC_KEY, {})
        if wave0:
            # find the best attribute to start sync
            # TODO: delegate in specific function / plugin
            CANDIDATES = [
                r'timestamp',
                r'date',
            ]
            ...
            if key:
                ...
                params[MONOTONIC_SINCE_VALUE] = wave0[key]
                # params[MONOTONIC_SINCE] = {key: wave0[key]}
...
class SQLiteSession(iSession):
    ...
    async def _get_schema(self, uri):
        ...
        fields = [
            ...
            for row in cursor.execute(f"PRAGMA table_xinfo({table});")
        ]
        fields = {_['name']: _ for _ in fields}, fields
        self.CACHE.get(uri["path"], {})[uri["query_"]["kind"]] = fields
        return fields
    ...
    async def get(self, url, headers=None, params=None, **kw):
        ...
        uri = parse_uri(url)
        fields, ord_fields = self._schema(uri) or await self._get_schema(uri)
