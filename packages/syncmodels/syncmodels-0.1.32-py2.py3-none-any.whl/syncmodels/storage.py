...
from .definitions import MONOTONIC_KEY, ALT_KEY
...
def filter_private(data):
    return { key: value for key, value in data.items() if not re.match(REG_PRIVATE_KEY, key)}
...
QUERY_SELECT_TUBE_SYNC = f"SELECT * FROM {TUBE_SYNC} WHERE source=$source AND target=$target"""
...
class SurrealistStorage(Storage):
    ...
    DB_LAYOUT = {
        ...
        DEFINE FIELD id ON TABLE {TUBE_META} TYPE string;
        """,
        #TUBE_WAVE: f"""
    ...
    def __init__(self, url="./db", user="root", password="root", ns="test", db="test", mode=iCRUD.MODE_SNAPSHOT):
        super().__init__(url=url, mode=mode)  # TODO: FIXME: DO NOT CALL BASE CLASS, will corrupt url
        self.url = url
        self.cache = {}
        self.user = user
        self.password = password
    ...
    def _update_database_layout(self):
        """
    ...
    def last_waves(self, sources: List, uid: UID) -> Dict[str, WAVE]:
        "TBD"
        query = f"""
    ...
    def get(self, fqid, cache=True, mode=None):
    ...
    def since(self, fqid, wave, max_results=100):
        query = f"""
        SELECT *
        FROM {fqid}
    ...
    def put(self, fqid, data: Dict, mode=None) -> bool:
        if not self.connection:
            self._connect()
        mode = mode or self.mode
        try:
            if mode in (self.MODE_SNAPSHOT, ):
                current = self.get(fqid)
                ...
                if current:
                    ...
                    current = filter_private(current)
                ...
                thing = data.pop("id")
                if thing == fqid:
                    assert ':' in fqid, "updating an item must have valid fqui, i.e. 'table:id' "
                ...
                result = self.connection.update(TUBE_WAVE, {'wave': monotonic}, record_id=tube_name)
                foo = 1
            else:
                raise RuntimeError(f"UNKOWN mode: {mode} for put()")
    ...
    def update_sync_wave(self, sources, target, wave):
        ...
        for source in sources:
            ...
            if record_id:
                # already in memory
                result = self.connection.query(QUERY_UPDATE_TUBE_SYNC, {'record_id': record_id, MONOTONIC_KEY: wave})
                assert result.result and result.result[0][MONOTONIC_KEY] == wave, f"Update {TUBE_SYNC} for {record_id} doesn't work!!"
