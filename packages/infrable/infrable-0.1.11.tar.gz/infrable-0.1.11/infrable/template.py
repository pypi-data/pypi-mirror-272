...
def _jinja_render(content: str, ctx: dict[str, Any], **kwargs: Any) -> str:
    return JinjaTemplate(content, undefined=StrictUndefined).render(ctx, **kwargs)
...
class Metadata:
    ...
    def parse(cls, path: Path, ctx: dict[str, Any]):
        ...
        rendered = _jinja_render(raw, ctx=ctx, _template={"src": str(path)})
