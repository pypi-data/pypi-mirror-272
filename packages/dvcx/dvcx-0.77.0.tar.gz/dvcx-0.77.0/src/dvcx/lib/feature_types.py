...
from dvcx.lib.feature import TYPE_TO_DVCX, Feature, ShallowFeature
from dvcx.query.schema import Column
from dvcx.sql.types import NullType
...
def bind_value_params(feature: type[Feature], *args, **kwargs) -> type[Feature]:
    cls = create_model(
        feature.__name__ + "_bind",
        __base__=feature,  # type: ignore
    )

    cls.get_value = lambda self: feature.get_value(self, *args, **kwargs)  # type: ignore
    return cls


def overwrite_value_function(
    fr_class: FeatureLike, func, *args, **kwargs
) -> type[Feature]:
    feature = FeatureTypes._to_features(fr_class)[0]
    ...
    cls.get_value = lambda self: func(self, *args, **kwargs)  # type: ignore
    return cls
...
def pydantic_to_feature(data_cls: type[BaseModel]) -> type[Feature]:
    ...
    for name, field_info in data_cls.model_fields.items():
        ...
        if anno not in TYPE_TO_DVCX:
            ...
            try:
                Feature._convert_type(anno)
            except RuntimeError:
                if not issubclass(anno, Feature):  # type: ignore[arg-type]
...
class FeatureTypes:
    ...
    def _to_features(cls, *fr_classes: FeatureLike) -> Sequence[type["Feature"]]:
        return [
            cls.column_class(fr) if isinstance(fr, (str, Column)) else fr
