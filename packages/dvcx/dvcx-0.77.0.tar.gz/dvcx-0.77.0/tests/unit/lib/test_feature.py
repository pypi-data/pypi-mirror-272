...
from dvcx.lib.feature_types import FeatureTypes, bind_value_params, pydantic_to_feature
...
def test_bind():
    my_prefix = "it_s_suffix99_"
    my_suffix = "_something42"
    my_value = "test_9_value"

    class MyColumn(FeatureTypes.column_class("val", str)):
        def get_value(self, prefix: str, suffix: str) -> str:
            return prefix + super().get_value() + suffix

    fr = MyColumn(val=my_value)

    assert fr.get_value(my_prefix, my_suffix) == my_prefix + my_value + my_suffix
    with pytest.raises(TypeError):
        assert fr.get_value() == my_value

    cls1 = bind_value_params(MyColumn, prefix=my_prefix, suffix=my_suffix)
    o1 = cls1(val=my_value)
    assert o1.get_value() == my_prefix + my_value + my_suffix

    cls2 = bind_value_params(MyColumn, suffix=my_suffix)
    o1 = cls2(val=my_value)
    with pytest.raises(TypeError):
        o1.get_value()

    cls3 = bind_value_params(MyColumn, my_prefix, my_suffix)
    o1 = cls3(val=my_value)
    assert o1.get_value() == my_prefix + my_value + my_suffix
