    ...
    try:
        if dateWindow:
            ...
            if not isinstance(dateWindow, list) or not isinstance(start_dt, str) or not isinstance(end_dt, str):
                print(
                    "不合法的dateWindow, 正确示例:  ['2021-10-11,'2021-12-31']表示日期在2021-10-11到2021-12-31日之间(2021-12-31 当天包含在内) 空字符串''代表今天.")
                return None
            if start_dt > end_dt:
...
@assert_auth
def notice_update_dominant():
    return qedataClient.instance()('notice_update_dominant', **locals())
...
def get_price(security, start_date, end_date, freq='minute', fields=None, overnight=False, silent=False,
    ...
    if freqtype == 1:
        if isStock:
            ...
            dfcols = ['open', 'close', 'high', 'low', 'volume', 'money', 'position', 'upperlimit', 'lowerlimit',
                      'presett',
                      'preclose', 'settle']
    else:
...
@assert_auth
def get_interest_rate(date=None):
    if date == None:
        date = "none"
...
def _collect_func():
    ...
    for func in globals().keys():
        if func.startswith("get") or func.startswith("sm_get") or func.startswith("is_") or func.startswith(
                "update_") or func.startswith("notice_"):
            funcs.append(func)
    return funcs
