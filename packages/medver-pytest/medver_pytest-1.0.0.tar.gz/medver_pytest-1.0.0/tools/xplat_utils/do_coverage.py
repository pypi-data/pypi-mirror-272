...
class DoCoverage:
    ...
    # @return None
    def run(self, action):
        svc.log.start(f'{svc.gbl.tag}: starting {action}...')
        ...
        with open(path, 'r', encoding='utf-8') as fp:
            ...
            for line in lines:
                ...
                if not in_details:
                    ...
                    if m:
                        svc.log.line(f'{svc.gbl.tag}:    {names[detail_num]: <10}: {m.group(1): >3}')
                        detail_num += 1
    ...
    def _cpp_gen_coverage(self):
        ...
        cmd = ('gcovr --html-details '  # show individual source files
               ...
               '--sort-percentage '  # sort source files based on percentage uncovered lines
               '--print-summary '  # print summary to stdout
               f'-o {report_page} '  # location of report main page
               )

        # all directories named in xplat.do_ut
        for d in svc.cfg.do_ut.cov_dirs:
            cmd += f'--filter {d} '  # location of source files being covered
