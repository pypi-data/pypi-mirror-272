...
class CheckPythonSdkVersion(Check):
    ...
    LATEST_SDK_VERSION = semver.VersionInfo.parse("2.0.1")
    ...
    def check_python_sdk(
        ...
        try:
            ...
            yield WarningMessage(
                f"Issue parsing version found for the Splunk SDK for Python ({ver}). Error: {err}.",
        ...
        if parsed_ver < self.MINIMUM_SDK_VERSION:
            ...
            yield FailMessage(
                f"Detected an outdated version of the Splunk SDK for Python ({ver}).",
                file_name=path_in_app,
                line_number=line_number,
