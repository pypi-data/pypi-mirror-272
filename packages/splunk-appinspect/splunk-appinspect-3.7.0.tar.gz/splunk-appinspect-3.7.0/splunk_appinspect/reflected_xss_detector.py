...
from splunk_appinspect.regex_matcher import JSReflectedXSSMatcher, RegexBundle, RegexMatcher
...
class ReflectedXSSDetector:
    ...
    def detect(self):
        ...
        for rule in self.rule_list:
            matched.extend(rule.check(self.app))
        return matched
        ...
        for _, result_list in id_to_match_result_dict.items():
            ...
            if len(result_list) < 200:
                for result in result_list:
                    ...
                    matched.append((reporter_output, result[0], result[1]))
        return matched
                    ...
                    for token in tokens:
                        ...
                        if token in src_value:
                            reporter_output = (
                                f"{self.rule_description}. The following line will be inspected during code review."
                                f" Match: {src_value}"
                                f" File: {Path(directory, f)}"
                            )
                            matched.append((reporter_output, Path(directory, f)))
                            break
        return matched
    ...
    def check(self, app):
        ...
        for directory, f, _ in app.iterate_files(types=[".js"]):
            ...
            for result in result_list:
                ...
                matched.append((reporter_output, Path(directory, f), result[0]))
        return matched
...
class UserHTMLReflectedXSSDetectRule(ReflectedXSSDetectRule):
    ...
    def check(self, app):
        ...
        for directory, f, _ in app.iterate_files(types=[".html"]):
            current_file_full_path = app.get_filename(directory, f)
            current_file_path = Path(directory, f)
            soup = BeautifulSoup(
                open(current_file_full_path, "rb").read(),
                "html.parser",
                store_line_numbers=False,
            )
            matched.extend(self.check_file(app, current_file_path, soup))
        return matched
...
class SvgTagXSSCheck(UserHTMLReflectedXSSDetectRule):
    def check_file(self, app, current_file_path, soup):
        ...
        for result in soup.find_all("svg"):
            attr_dict = result.attrs
            if "onload" in attr_dict and self._xml_attribute_exists_and_pattern_match_check(
...
class TableAndTdXSSCheck(UserHTMLReflectedXSSDetectRule):
    def check_file(self, app, current_file_path, soup):
        ...
        for result in soup.find_all(["table", "td"]):
            attr_dict = result.attrs
            if "background" in attr_dict and self._xml_attribute_exists_and_pattern_match_check(
...
class LinkXSSCheck(UserHTMLReflectedXSSDetectRule):
    def check_file(self, app, current_file_path, soup):
        ...
        for result in soup.find_all("link"):
            attr_dict = result.attrs
            if "href" in attr_dict and self._xml_attribute_exists_and_pattern_match_check(
...
class DivStyleSheetXSSCheck(UserHTMLReflectedXSSDetectRule):
    def check_file(self, app, current_file_path, soup):
        ...
        for result in soup.find_all("div"):
            attr_dict = result.attrs
            if "style" in attr_dict and self._xml_attribute_exists_and_pattern_match_check(
                "style",
                attr_dict,
                None,
