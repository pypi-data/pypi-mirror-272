...
class RemoteObjectsManager:
    __slots__ = (
        ...
        'objects_db',
        'objects_id_gen',
        'serializable_data_types',
        'serializable_any',
        'serializable_int',
        'serializable_float',
        'serializable_complex',
        'serializable_str',
        'serializable_bytes',
        'serializable_bytearray',
        'serializable_bool',
        'serializable_none',
        'serializable_list',
        'serializable_tuple',
        'serializable_set',
        'serializable_frozenset',
        'serializable_dict',
        'serializable_slice',
        'known_classes',
        'known_classes_by_id',
        'known_classes_info',
        'known_classes_info_by_id',
        'on_new_class_handler',
        'on_new_obj_info_handler',
        'objects_ids',
        'objects_ids_by_id',
    )
    ...
    def serialize_impl(self, obj: Any, ignore_empty_classes: bool = False) -> Tuple[bool, int]:
            ...
            if 0 == type_id:
                ...
                for slot_name, slot_value in filled_slot_names_with_values_gen(obj):
                    ...
                    if self.serializable_str:
                        ...
                        adjusted_slot_name = self.ats(4, slot_name)
                    
                    item_exists, item_value = self.serialize_impl(slot_value)
                    if item_exists:
                        if self.serializable_tuple:
                            new_obj_slots.append((adjusted_slot_name, item_value))
                        elif self.serializable_list:
                            new_obj_slots.append([adjusted_slot_name, item_value])
                        else:
                            new_obj_slots.append(self.ats(10, (adjusted_slot_name, item_value)))
                ...
                if hasattr(obj, '__dict__'):
                    for key, value in obj.__dict__.items():
                        ...
                        if self.serializable_str:
                            ...
                            adjusted_key = self.ats(4, key)

                        value_exists, value_value = self.serialize_impl(value)
                        if value_exists:
                            new_obj_dict[adjusted_key] = value_value
                        else:
                            continue
                    ...
                    if new_obj_dict:
                        if self.serializable_dict:
                            ...
                            new_obj_dict = self.ats(13, new_obj_dict)
                ...
                if isinstance(obj, MutableMapping):
                    for key, value in obj.items():
                        key_exists, key_value = self.serialize_impl(value)
                        if not key_exists:
                            continue
                    ...
                    if obj_mapping:
                        if self.serializable_dict:
                            ...
                            obj_mapping = self.ats(13, obj_mapping)
                elif isinstance(obj, MutableSequence):
                    for item in obj:
                        item_exists, item_value = self.serialize_impl(item)
                        if item_exists:
                            obj_sequence.append(item_value)
                        else:
                            continue
                ...
                if obj_type in self.known_classes:
                    ...
                    adjusted_owning_names_path: List[str] = [self.ats(4, name) for name in owning_names_path]
                    ...
                    if result_exists:
                        class_info = (
                            self.ats(1, class_id),
                            self.ats(4, class_name),
                            self.ats(4, module_importable_str),
                            self.ats(9, adjusted_owning_names_path),
                        )

                        if self.serializable_tuple:
                            adjusted_class_info = class_info
                        else:
                            adjusted_class_info = self.ats(10, class_info)
                ...
                object_info = (
                    ...
                    self.ats(8, None),
                    self.ats(1, class_id),
                    new_obj_slots if new_obj_slots else self.ats(9, list()),
                    new_obj_dict if new_obj_dict else self.ats(13, dict()),
                    obj_mapping if obj_mapping else self.ats(13, dict()),
                    obj_sequence if obj_sequence else self.ats(9, list()),
                    obj_set if obj_set else self.ats(11, set()),
                )
            elif known_data:
                object_info = (
                    self.ats(1, object_id),
        ...
        if self.serializable_tuple:
            ...
            adjusted_object_info = self.ats(10, object_info)
    ...
    def serialize(self, obj: Any, ignore_empty_classes: bool = False) -> Tuple[bool, int]:
        with DisableGC():
            return self.serialize_impl(obj, ignore_empty_classes)
    
    s = serialize
    
    def deserialize_class_impl(self, class_info: Tuple) -> Tuple[bool, int, Type]:
        if self.serializable_tuple:
            pass
        else:
            class_info = self.afs(10, class_info)

        class_id: int = self.afs(1, class_info[0])
        if class_id in self.known_classes_by_id:
            ...
            class_name: str = self.afs(4, class_info[1])
            module_importable_str: str = self.afs(4, class_info[2])
            owning_names_path: List[str] = self.afs(9, class_info[3])
            adjusted_owning_names_path: List[str] = [self.afs(4, name) for name in owning_names_path]
    ...
    def deserialize_class(self, class_info: Tuple) -> Tuple[bool, int, Type]:
        with DisableGC():
    ...
    def deserialize_container_impl(self, type_id: DataType, obj: Any) -> Any:
        ...
        if 9 == type_id:
            new_obj = list()
            for item_id in obj:
                new_obj.append(self.objects_db[self.afs(1, item_id)])
        elif 10 == type_id:
            new_obj = list()
            for item_id in obj:
                new_obj.append(self.objects_db[self.afs(1, item_id)])
            ...
            new_obj = set()
            for item_id in obj:
                new_obj.add(self.objects_db[self.afs(1, item_id)])
        elif 12 == type_id:
            new_obj = set()
            for item_id in obj:
                new_obj.add(self.objects_db[self.afs(1, item_id)])
            ...
            for key_id, value_id in obj.items():
                new_obj[self.objects_db[int(self.afs(1, key_id))]] = self.objects_db[self.afs(1, value_id)]
    ...
    def deserialize_obj_impl(self, obj_info: Tuple) -> Tuple[bool, int, Any]:
        if self.serializable_tuple:
            pass
        else:
            obj_info = self.afs(10, obj_info)
        
        object_id: int = self.afs(1, obj_info[0])
        if object_id in self.objects_db:
            return ResultExistence[Tuple[int, Any]](True, (object_id, self.objects_db[object_id]))
        
        type_id: int = self.afs(1, obj_info[1])
        ...
        if serializable:
            ...
            if 0 == type_id:
                class_id: int = self.afs(1, obj_info[3])
                ...
                if obj_info[7]:
                    ...
                    for item_id in contained_sequence:
                        item_id = self.afs(1, item_id)
                        child_item = self.objects_db[item_id]
                        obj.append(child_item)
                
                if obj_info[8]:
                    contained_set = self.afs(11, obj_info[8])
    ...
    def deserialize_obj(self, obj_info: Tuple) -> Tuple[bool, int, Any]:
        with DisableGC():
