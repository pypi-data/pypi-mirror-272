...
codegen_files_ext = {'.c', '.cpp', '.c++', '.cxx', '.cc'}
headers_ext = {'.h', '.hpp', '.h++', '.hh', '.hxx'}
libs_ext = {'.lib', '.dylib', '.dll', '.so', '.exp', '.obj'}
...
def find_package_data(package_src_relative_path: str, good_packages: List[str], depth: Optional[int] = 1, root_rel: Optional[RelativePath] = None) -> Tuple[Dict[str, List[str]], List[str]]:
    ...
    for package_name in good_packages:
        ...
        pyx_files = filtered_file_list_traversal(package_full_path, FilteringType.including, {'.pyx'}, remove_empty_items=True)
...
def remove_header_files(ext_modules: Sequence[CythonExtension]) -> Sequence[CythonExtension]:
    for ext_module in ext_modules:
        if isinstance(ext_module, CythonExtension):
            new_sources = list()
...
def process_macros(ext_modules: Sequence[CythonExtension]) -> Sequence[CythonExtension]:
    for ext_module in ext_modules:
        if isinstance(ext_module, CythonExtension):
...
class BuildConfig:
    def __init__(self) -> None:
        ...
        self.additional_pyx_flags: Dict[str, Tuple[bool, Union[str, int]]] = dict()
