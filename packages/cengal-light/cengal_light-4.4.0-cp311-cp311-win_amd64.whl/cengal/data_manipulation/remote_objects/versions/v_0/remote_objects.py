...
class RemoteObjectsManager:
    ...
    def adjust_to_serializable(self, type_id: DataType, obj: Any) -> Any:
        if DataType.int_ == type_id:
            if self.serializable_int:
                ...
                return bytearray(obj.to_bytes(8, 'little'))
            ...
            if self.serializable_str:
                return obj
            elif self.serializable_bytes:
                return obj.encode('utf-8')
            elif self.serializable_bytearray:
                return bytearray(obj.encode('utf-8'))
            elif self.serializable_int:
                if self.serializable_tuple:
                ...
                if self.serializable_tuple:
                    return tuple(float(ord(c)) for c in obj)
                elif self.serializable_list:
                    return [float(ord(c)) for c in obj]
                else:
                    pass
            else:
                pass
            ...
            if self.serializable_bool:
                ...
                return bytearray(obj.to_bytes(1, 'little'))
            elif self.serializable_str:
                return str(obj)
            elif self.serializable_float:
            ...
            if self.serializable_none:
                return obj
            elif self.serializable_bool:
                return False
            elif self.serializable_int:
                return 0
            elif self.serializable_bytes:
            ...
            if self.serializable_dict:
                return obj
            elif self.serializable_tuple:
                return tuple(obj.items())
            elif self.serializable_list:
                return list(list(pair) for pair in obj.items())
            else:
                pass
    ...
    def adjust_from_serializable(self, type_id: DataType, obj: Any) -> Any:
        if DataType.int_ == type_id:
            if self.serializable_int:
                ...
                return int.from_bytes(bytes(obj), 'little')
            elif self.serializable_str:
                return int(obj)
            else:
                pass
            ...
            if self.serializable_float:
                ...
                return unpack('=d', obj)[0]
            elif self.serializable_bytearray:
                return unpack('=d', bytes(obj))[0]
            elif self.serializable_str:
                return float(obj)
            else:
                pass
            ...
            if self.serializable_str:
                ...
                if self.serializable_tuple:
                    return ''.join(chr(int(round(c))) for c in obj)
                elif self.serializable_list:
                    return ''.join(chr(int(round(c))) for c in obj)
                else:
                    pass
            ...
            if self.serializable_bool:
                ...
                return bool(int.from_bytes(obj, 'little'))
            elif self.serializable_bytearray:
                return bool(int.from_bytes(bytes(obj), 'little'))
            elif self.serializable_str:
                return bool(obj)
            elif self.serializable_float:
                return bool(int(round(obj)))
            else:
                pass
        elif DataType.none_ == type_id:
            return None
    ...
    def serialize_impl(self, obj: Any, ignore_empty_classes: bool = False) -> ResultExistence[int]:
        ...
        if serializable:
            if known_container:
                object_info = (
                    self.ats(DataType.int_, object_id),
                    self.ats(DataType.int_, type_id.value),
                    self.sc(type_id, obj),
                )
            else:
                object_info = (
            ...
            if DataType.class_ == type_id:
                ...
                for slot_name, slot_value in filled_slot_names_with_values_gen(obj):
                    # if not hasattr(slot_value, '__set__'):
                    ...
                    if self.serializable_str:
                        ...
                        adjusted_slot_name = self.ats(DataType.str_, slot_name)
                ...
                if hasattr(obj, '__dict__'):
                    for key, value in obj.__dict__.items():
                        ...
                        if self.serializable_str:
                            ...
                            adjusted_key = self.ats(DataType.str_, key)
                    ...
                    if new_obj_dict:
                        if self.serializable_dict:
                            ...
                            new_obj_dict = self.ats(DataType.dict_, new_obj_dict)
                ...
                if isinstance(obj, MutableMapping):
                    ...
                    if obj_mapping:
                        if self.serializable_dict:
                            ...
                            obj_mapping = self.ats(DataType.dict_, obj_mapping)
                    ...
                    if obj_set:
                        if self.serializable_set:
                            ...
                            obj_set = self.ats(DataType.set_, obj_set)
                else:
                    pass
                ...
                if obj_type in self.known_classes:
                    ...
                    adjusted_owning_names_path: List[str] = [self.ats(DataType.str_, name) for name in owning_names_path]
                    ...
                    if result_exists:
                        class_info = (
                            self.ats(DataType.int_, class_id),
                            self.ats(DataType.str_, class_name),
                            self.ats(DataType.str_, module_importable_str),
                            self.ats(DataType.list_, adjusted_owning_names_path),
                        )

                        if self.serializable_tuple:
                            adjusted_class_info = class_info
                        else:
                            adjusted_class_info = self.ats(DataType.tuple_, class_info)
                ...
                object_info = (
                    self.ats(DataType.int_, object_id),
                    self.ats(DataType.int_, type_id.value),
                    self.ats(DataType.none_, None),
                    self.ats(DataType.int_, class_id),
                    new_obj_slots if new_obj_slots else self.ats(DataType.list_, list()),
                    new_obj_dict if new_obj_dict else self.ats(DataType.dict_, dict()),
                    obj_mapping if obj_mapping else self.ats(DataType.dict_, dict()),
                    obj_sequence if obj_sequence else self.ats(DataType.list_, list()),
                    obj_set if obj_set else self.ats(DataType.set_, set()),
                )
            elif known_data:
                object_info = (
                    self.ats(DataType.int_, object_id),
        ...
        if self.serializable_tuple:
            adjusted_object_info = object_info
        else:
            adjusted_object_info = self.ats(DataType.tuple_, object_info)
    ...
    def deserialize_class_impl(self, class_info: Tuple) -> ResultExistence[Tuple[int, Type]]:
        if self.serializable_tuple:
            ...
            class_info = self.afs(DataType.tuple_, class_info)

        class_id: int = self.afs(DataType.int_, class_info[ClassInfoFields.class_id.value])
        if class_id in self.known_classes_by_id:
            ...
            class_name: str = self.afs(DataType.str_, class_info[ClassInfoFields.class_name.value])
            module_importable_str: str = self.afs(DataType.str_, class_info[ClassInfoFields.module_importable_str.value])
            owning_names_path: List[str] = self.afs(DataType.list_, class_info[ClassInfoFields.owning_names_path.value])
            adjusted_owning_names_path: List[str] = [self.afs(DataType.str_, name) for name in owning_names_path]
            obj_class: Type = entity_by_name_module_importable_str_and_owning_names_path(class_name, module_importable_str, adjusted_owning_names_path)
    ...
    def deserialize_container_impl(self, type_id: DataType, obj: Any) -> Any:
        ...
        if DataType.list_ == type_id:
            new_obj = list()
            for item_id in obj:
                new_obj.append(self.objects_db[self.afs(DataType.int_, item_id)])
        elif DataType.tuple_ == type_id:
            new_obj = list()
            for item_id in obj:
                new_obj.append(self.objects_db[self.afs(DataType.int_, item_id)])
            ...
            new_obj = set()
            for item_id in obj:
                new_obj.add(self.objects_db[self.afs(DataType.int_, item_id)])
        elif DataType.frozenset_ == type_id:
            new_obj = set()
            for item_id in obj:
                new_obj.add(self.objects_db[self.afs(DataType.int_, item_id)])
            ...
            for key_id, value_id in obj.items():
                new_obj[self.objects_db[int(self.afs(DataType.int_, key_id))]] = self.objects_db[self.afs(DataType.int_, value_id)]
    ...
    def deserialize_obj_impl(self, obj_info: Tuple) -> ResultExistence[Tuple[int, Any]]:
        if self.serializable_tuple:
            ...
            obj_info = self.afs(DataType.tuple_, obj_info)
        
        object_id: int = self.afs(DataType.int_, obj_info[ObjectInfoFields.object_id.value])
        if object_id in self.objects_db:
            return ResultExistence[Tuple[int, Any]](True, (object_id, self.objects_db[object_id]))
        
        type_id: DataType = DataType(self.afs(DataType.int_, obj_info[ObjectInfoFields.type_id.value]))
        ...
        if serializable:
            ...
            if DataType.class_ == type_id:
                class_id: int = self.afs(DataType.int_, obj_info[ObjectInfoFields.class_id.value])
                ...
                if obj_info[ObjectInfoFields.clonable_slots.value]:
                    ...
                    for slot_name, slot_id in clonable_slots:
                        slot_name = self.afs(DataType.str_, slot_name)
                        slot_id = self.afs(DataType.int_, slot_id)
                        child_obj = self.objects_db[slot_id]
                        setattr(obj, slot_name, child_obj)
                
                if obj_info[ObjectInfoFields.clonable_dict_items.value]:
                    clonable_dict_items = self.afs(DataType.dict_, obj_info[ObjectInfoFields.clonable_dict_items.value])
                    for key, value_id in clonable_dict_items.items():
                        key = self.afs(DataType.str_, key)
                        value_id = self.afs(DataType.int_, value_id)
                        child_obj = self.objects_db[value_id]
                        setattr(obj, key, child_obj)
                
                if obj_info[ObjectInfoFields.contained_mapping.value]:
                    contained_mapping = self.afs(DataType.dict_, obj_info[ObjectInfoFields.contained_mapping.value])
                    for key_id, value_id in contained_mapping.items():
                        key_id = self.afs(DataType.int_, key_id)
                        value_id = self.afs(DataType.int_, value_id)
                ...
                if obj_info[ObjectInfoFields.contained_sequence.value]:
                    ...
                    for item_id in contained_sequence:
                        item_id = self.afs(DataType.int_, item_id)
                        child_item = self.objects_db[item_id]
                        obj.append(child_item)
                
                if obj_info[ObjectInfoFields.contained_set.value]:
                    contained_set = self.afs(DataType.set_, obj_info[ObjectInfoFields.contained_set.value])
