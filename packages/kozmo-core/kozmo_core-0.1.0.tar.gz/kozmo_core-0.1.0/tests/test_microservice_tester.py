...
from kozmo_core.microservice_tester import (
    KozmoTesterException,
    reconciliate_cont_type,
    run_method,
    run_send_feedback,
)
from kozmo_core.proto import prediction_pb2
from kozmo_core.utils import array_to_grpc_datadef, kozmo_message_to_json
...
def test_reconciliate_exception():
    arr = np.array([1, 2])
    with pytest.raises(KozmoTesterException):
