...
from kozmo_core.metrics import KozmoMetrics
from kozmo_core.proto import prediction_pb2
from kozmo_core.user_model import KozmoComponent
from kozmo_core.utils import kozmo_message_to_json
from kozmo_core.wrapper import KozmoModelGRPC, get_grpc_server, get_rest_microservice
...
class UserObjectLowLevel:
    ...
    def aggregate_grpc(
        self, request: Union[prediction_pb2.KozmoMessage, List, Dict]
        ...
        response = prediction_pb2.KozmoMessage(data=datadef)
        if is_proto:
            return response
        else:
            return kozmo_message_to_json(response)
...
def test_aggreate_ok_kozmo_messages():
    user_object = UserObject()
    kozmo_metrics = KozmoMetrics()
    app = get_rest_microservice(user_object, kozmo_metrics)
    client = app.test_client()
    rv = client.get('/aggregate?json={"kozmoMessages":[{"data":{"ndarray":[1]}}]}')
    logging.info(rv)
...
def test_aggreate_combines_tags():
    ...
    rv = client.get('/aggregate?json={{"kozmoMessages":{}}}'.format(msgs))
    logging.info(rv)
    j = json.loads(rv.data)
    logging.info(j)
    assert rv.status_code == 200
...
def test_aggreate_bad_messages():
    ...
    app = get_rest_microservice(user_object, kozmo_metrics)
    client = app.test_client()
    rv = client.get('/aggregate?json={"kozmoMessages":[{"data2":{"ndarray":[1]}}]}')
    assert rv.status_code == 400
    j = json.loads(rv.data)
    logging.info(j)
    assert j["status"]["reason"] == "MICROSERVICE_BAD_DATA"
...
def test_aggreate_ok_2messages():
    ...
    client = app.test_client()
    rv = client.get(
        '/aggregate?json={"kozmoMessages":[{"data":{"ndarray":[1]}},{"data":{"ndarray":[2]}}]}'
    )
    logging.info(rv)
    j = json.loads(rv.data)
    logging.info(j)
...
def test_aggreate_ok_bindata():
    ...
    rv = client.get(
        '/aggregate?json={"kozmoMessages":[{"binData":"'
        + bdata_base64
        + '"},{"binData":"'
        + bdata_base64
        + '"}]}'
...
def test_aggreate_ok_strdata():
    ...
    client = app.test_client()
    rv = client.get(
        '/aggregate?json={"kozmoMessages":[{"strData":"123"},{"strData":"456"}]}'
    )
    logging.info(rv)
    j = json.loads(rv.data)
    logging.info(j)
...
def test_aggregate_proto_combines_tags():
    ...
    msg1 = prediction_pb2.KozmoMessage(data=datadef1, meta=meta1)
    msg2 = prediction_pb2.KozmoMessage(data=datadef2, meta=meta2)
    request = prediction_pb2.KozmoMessageList(kozmoMessages=[msg1, msg2])
    resp = app.Aggregate(request, None)
    jStr = json_format.MessageToJson(resp)
    j = json.loads(jStr)
...
def test_aggregate_proto_bin_data():
    ...
    app = KozmoModelGRPC(user_object, kozmo_metrics)
    binData = b"\0\1"
    msg1 = prediction_pb2.KozmoMessage(binData=binData)
    request = prediction_pb2.KozmoMessageList(kozmoMessages=[msg1])
    resp = app.Aggregate(request, None)
...
def test_get_grpc_server():
    ...
    kozmo_metrics = KozmoMetrics()
    server = get_grpc_server(user_object, kozmo_metrics)


def test_unimplemented_aggregate_raw_on_kozmo_component():
    class CustomKozmoComponent(KozmoComponent):
        def aggregate(self, Xs, features_names):
            return sum(Xs) * 2

    user_object = CustomKozmoComponent()
    kozmo_metrics = KozmoMetrics()
    app = get_rest_microservice(user_object, kozmo_metrics)
    client = app.test_client()
    rv = client.get(
