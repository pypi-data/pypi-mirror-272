...
from kozmo_core.utils import json_to_kozmo_message, kozmo_message_to_json
...
class UserObject(KozmoComponent):
...
class UserObjectLowLevel(KozmoComponent):
...
class UserObjectLowLevelWithStatusInResponse(KozmoComponent):
    def __init__(self, metrics_ok=True, ret_nparray=False):
        self.metrics_ok = metrics_ok
        self.ret_nparray = ret_nparray
        self.nparray = np.array([1, 2, 3])
...
class UserObjectLowLevelWithStatusInResponseWithPredictRaw(KozmoComponent):
    def __init__(self, check_name):
        self.check_name = check_name

    def predict_raw(self, msg):
        msg = json_to_kozmo_message(msg)
        if self.check_name == "img":
            file_data = msg.binData
            img = Image.open(io.BytesIO(file_data))
            img.verify()
            return {
                "meta": kozmo_message_to_json(msg.meta),
...
class UserObjectLowLevelWithPredictRaw(KozmoComponent):
    def __init__(self, check_name):
        self.check_name = check_name
...
class UserObjectLowLevelGrpc(KozmoComponent):
    def __init__(self, metrics_ok=True, ret_nparray=False):
        self.metrics_ok = metrics_ok
        self.ret_nparray = ret_nparray
        self.nparray = np.array([1, 2, 3])
...
def fixture_get_model_name(mocker):
    return mocker.patch(
        "kozmo_core.utils.get_model_name", autospec=True, return_value="my-test-model"
    )
...
def fixture_get_image_name(mocker):
    return mocker.patch(
        "kozmo_core.utils.get_image_name",
        autospec=True,
...
def test_model_error_status_code():
    class ErrorUserObject:
        def predict(self, X, features_names, **kwargs):
            raise KozmoMicroserviceException("foo", status_code=403)
...
def test_model_kozmo_json_ok():
    user_object = UserObject()
    kozmo_metrics = KozmoMetrics()
    app = get_rest_microservice(user_object, kozmo_metrics)
    client = app.test_client()
    rv = client.get("/kozmo.json")
    assert rv.status_code == 200
...
def test_proto_bin_data():
    ...
    request = prediction_pb2.KozmoMessage(binData=bdata_base64)
    resp = app.Predict(request, None)
...
def test_proto_bin_data_nparray():
    ...
    app = KozmoModelGRPC(user_object, kozmo_metrics)
    binData = b"\0\1"
    request = prediction_pb2.KozmoMessage(binData=binData)
    resp = app.Predict(request, None)
    jStr = json_format.MessageToJson(resp)
    j = json.loads(jStr)
    logging.info(j)
...
def test_unimplemented_predict_raw_on_kozmo_component():
...
def test_unimplemented_feedback_raw_on_kozmo_component():
    class CustomKozmoComponent(KozmoComponent):
