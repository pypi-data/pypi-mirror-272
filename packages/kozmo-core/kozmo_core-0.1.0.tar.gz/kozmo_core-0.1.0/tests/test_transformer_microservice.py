...
class UserObjectLowLevelRawInherited(KozmoComponent):
    def __init__(self, metrics_ok=True, ret_nparray=False):
        self.metrics_ok = metrics_ok
        self.ret_nparray = ret_nparray
        self.nparray = np.array([1, 2, 3])
...
def test_transformer_input_bin_data():
    ...
    j = json.loads(rv.data)
    sm = prediction_pb2.KozmoMessage()
...
def test_unimplemented_transform_input_raw_on_kozmo_component():
...
def test_unimplemented_transform_output_raw_on_kozmo_component():
    class CustomKozmoComponent(KozmoComponent):
        def transform_output(self, X, features_names, **kwargs):
            return X * 2
