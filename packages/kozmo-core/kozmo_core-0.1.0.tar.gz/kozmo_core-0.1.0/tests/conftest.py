...
from kozmo_core.wrapper import get_rest_microservice
...
def client_gets_metrics(monkeypatch, request):
    value = request.param
    monkeypatch.setattr(
        kozmo_core.user_model, "INCLUDE_METRICS_IN_CLIENT_RESPONSE", value
    )
    monkeypatch.setattr(
        kozmo_core.kozmo_methods, "INCLUDE_METRICS_IN_CLIENT_RESPONSE", value
    )
    return value
...
def app():
    ...
    return get_rest_microservice(user_object, kozmo_metrics)
