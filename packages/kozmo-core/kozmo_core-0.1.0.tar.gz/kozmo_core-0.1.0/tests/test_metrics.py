...
from kozmo_core.user_model import client_custom_metrics
from kozmo_core.wrapper import (
    KozmoModelGRPC,
...
def test_split_image_tag():
    image = "kozmoai/sklearn-iris:0.1"
    expected_name = "kozmoai/sklearn-iris"
...
def test_component_ok():
    c = Component(True)
    print(client_custom_metrics(c, KozmoMetrics(), TEST_METRIC_METHOD_TAG))
    assert client_custom_metrics(c, KozmoMetrics(), TEST_METRIC_METHOD_TAG) == [
        EXPECTED_COUNTER_METRIC
    ]


def test_component_bad():
    with pytest.raises(KozmoMicroserviceException):
        c = Component(False)
        client_custom_metrics(c, KozmoMetrics(), TEST_METRIC_METHOD_TAG)


def test_metrics_clear():
    metrics = KozmoMetrics()
...
class UserObjectLowLevelGrpc:
    ...
    def predict_raw(self, msg):
        ...
        request = prediction_pb2.KozmoMessage(data=datadef, meta=meta)
        return request
...
def verify_kozmo_metrics(data, mycounter_value, histogram_entries, method):
    expected_base_tags = {"method": method}
    base_tags_key = KozmoMetrics._generate_tags_key(expected_base_tags)
    expected_custom_tags = {"mytag": "mytagvalue", "method": method}
    custom_tags_key = KozmoMetrics._generate_tags_key(expected_custom_tags)
    ...
    distinct_tags_one_key = KozmoMetrics._generate_tags_key(expected_distinct_tags_one)
    ...
    distinct_tags_two_key = KozmoMetrics._generate_tags_key(expected_distinct_tags_two)
...
def test_kozmo_metrics_predict(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()

    app = get_rest_microservice(user_object, kozmo_metrics)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 1, [0.0202], PREDICT_METRIC_METHOD_TAG)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 2, [0.0202, 0.0202], PREDICT_METRIC_METHOD_TAG)


@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_send_feedback(cls):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    tags_key = KozmoMetrics._generate_tags_key(expected_tags)
    assert data["COUNTER", "kozmo_api_model_feedback_reward", tags_key] == {
        "value": 42.0,
        "tags": expected_tags,
    }
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_aggregate(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    rv = client.get(
        '/aggregate?json={"kozmoMessages": [{"data": {"names": ["input"], "ndarray": ["data"]}}]}'
    )
    assert rv.status_code == 200
    assert ("metrics" in json.loads(rv.data)["meta"]) == client_gets_metrics

    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 1, [0.0202], AGGREGATE_METRIC_METHOD_TAG)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 2, [0.0202, 0.0202], AGGREGATE_METRIC_METHOD_TAG)


@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_transform_input(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 1, [0.0202], INPUT_TRANSFORM_METRIC_METHOD_TAG)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 2, [0.0202, 0.0202], INPUT_TRANSFORM_METRIC_METHOD_TAG)


@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_transform_output(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 1, [0.0202], OUTPUT_TRANSFORM_METRIC_METHOD_TAG)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 2, [0.0202, 0.0202], OUTPUT_TRANSFORM_METRIC_METHOD_TAG)


@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_route(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 1, [0.0202], ROUTER_METRIC_METHOD_TAG)
    ...
    data = kozmo_metrics.data[os.getpid()]
    verify_kozmo_metrics(data, 2, [0.0202, 0.0202], ROUTER_METRIC_METHOD_TAG)


@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_predict(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()

    app = KozmoModelGRPC(user_object, kozmo_metrics)
    datadef = prediction_pb2.DefaultData(
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_aggregate(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
    ...
    msg1 = prediction_pb2.KozmoMessage(data=datadef1)
    msg2 = prediction_pb2.KozmoMessage(data=datadef2)

    request = prediction_pb2.KozmoMessageList(kozmoMessages=[msg1, msg2])
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_transform_input(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_transform_output(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_route(cls, client_gets_metrics):
    user_object = cls()
    kozmo_metrics = KozmoMetrics()
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevel])
def test_kozmo_metrics_endpoint(cls, client_gets_metrics):
    ...
    metrics_app = get_metrics_microservice(kozmo_metrics)
...
@pytest.mark.parametrize("cls", [UserObject, UserObjectLowLevelGrpc])
def test_proto_kozmo_metrics_endpoint(cls, client_gets_metrics):
    def _match_label(line):
        _data, value = line.split()
        name, labels = _data.split()[0].split("{")
        labels = labels[:-1]
