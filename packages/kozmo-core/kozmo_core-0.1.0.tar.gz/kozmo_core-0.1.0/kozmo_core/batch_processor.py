...
from kozmo_core.kozmo_client import (
    KozmoCallCredentials,
    KozmoChannelCredentials,
    KozmoClient,
)

# lower case as grpc really does not like upper case here and rest does not care
KOZMO_PUID_HEADER = "kozmo-puid"

CHOICES_GATEWAY_TYPE = ["ambassador", "istio", "kozmo"]
...
def start_multithreaded_batch_worker(
    ...
    if use_ssl or len(call_credentials_token) > 0:
        ...
        credentials = KozmoCallCredentials(token=token)
        channel_credentials = KozmoChannelCredentials(verify=ssl_verify)
...
def _start_request_worker(
    ...
    sc
        An initialised Kozmo Client configured to send the requests to
    method:
...
def _send_batch_predict_multi_request(
    ...
    """
    Send an request using the Kozmo Client with batch context including the
    ...
    traced back individually in the Kozmo Request Logger context. Each request
    ...
    sc
        The instance of KozmoClient to use to send the requests to the kozmo model
    retries
    ...
    kozmo_puid = input_data[0][1]
    instance_ids = [f"{kozmo_puid}-item-{n}" for n, _ in enumerate(input_data)]
    ...
    predict_kwargs["headers"] = {KOZMO_PUID_HEADER: kozmo_puid}
    ...
    try:
        for i in range(retries):
            try:
                kozmo_payload = sc.predict(**predict_kwargs)
                assert kozmo_payload.success
                response = kozmo_payload.response
                break
...
def _send_batch_predict(
    ...
    predict_kwargs["meta"] = tags
    predict_kwargs["headers"] = {KOZMO_PUID_HEADER: batch_instance_id}
    try:
        ...
        for i in range(retries):
            try:
                kozmo_payload = sc.predict(**predict_kwargs)
                assert kozmo_payload.success
                str_output = json.dumps(kozmo_payload.response)
                break
            except (requests.exceptions.RequestException, AssertionError) as e:
                logger.error(
...
def _send_batch_feedback(
    ...
    """
    Send an request using the Kozmo Client with feedback
    ...
    try:
        ...
        for i in range(retries):
            try:
                kozmo_payload = sc.feedback(**feedback_kwargs)
                assert kozmo_payload.success

                # Update Tags so we can track feedback instances in output file
                tags = kozmo_payload.response.get("meta", {}).get("tags", {})
                tags.update(meta["tags"])
                if "meta" not in kozmo_payload.response:
                    kozmo_payload.response["meta"] = {}
                kozmo_payload.response["meta"]["tags"] = tags
                str_output = json.dumps(kozmo_payload.response)
                break
            except (requests.exceptions.RequestException, AssertionError) as e:
...
@click.option(
    "--deployment-name",
    "-d",
    envvar="KOZMO_BATCH_DEPLOYMENT_NAME",
    required=True,
    help="The name of the KozmoDeployment to send the requests to",
)
@click.option(
    "--gateway-type",
    "-g",
    envvar="KOZMO_BATCH_GATEWAY_TYPE",
    type=click.Choice(CHOICES_GATEWAY_TYPE),
    default="istio",
    help="The gateway type for the kozmo model, which can be through the ingress provider (istio/ambassador) or directly through the service (kozmo)",
)
@click.option(
    "--namespace",
    "-n",
    envvar="KOZMO_BATCH_NAMESPACE",
    default="default",
    help="The Kubernetes namespace where the KozmoDeployment is deployed in",
)
@click.option(
    "--host",
    "-h",
    envvar="KOZMO_BATCH_HOST",
    default="istio-ingressgateway.istio-system.svc.cluster.local:80",
    help="The hostname for the kozmo model to send the request to, which can be the ingress of the Kozmo model or the service itself",
)
@click.option(
    "--transport",
    "-t",
    envvar="KOZMO_BATCH_TRANSPORT",
    type=click.Choice(CHOICES_TRANSPORT),
    default="rest",
    help="The transport type of the KozmoDeployment model which can be REST or GRPC",
)
@click.option(
    "--data-type",
    "-a",
    envvar="KOZMO_BATCH_DATA_TYPE",
    type=click.Choice(CHOICES_DATA_TYPE),
    default="data",
    help="Whether to use json, strData or Kozmo Data type for the payload to send to the KozmoDeployment which aligns with the KozmoClient format",
)
@click.option(
    "--payload-type",
    "-p",
    envvar="KOZMO_BATCH_PAYLOAD_TYPE",
    type=click.Choice(CHOICES_PAYLOAD_TYPE),
    default="ndarray",
    help="The payload type expected by the KozmoDeployment and hence the expected format for the data in the input file which can be an array",
)
@click.option(
    "--workers",
    "-w",
    envvar="KOZMO_BATCH_WORKERS",
    type=int,
    default=1,
...
@click.option(
    "--input-data-path",
    "-i",
    envvar="KOZMO_BATCH_INPUT_DATA_PATH",
    type=click.Path(),
...
@click.option(
    "--output-data-path",
    "-o",
    envvar="KOZMO_BATCH_OUTPUT_DATA_PATH",
    type=click.Path(),
    default="/assets/input-data.txt",
...
@click.option(
    "--method",
    "-m",
    envvar="KOZMO_BATCH_METHOD",
    type=click.Choice(CHOICES_METHOD),
    default="predict",
    help="The method of the KozmoDeployment to send the request to which currently only supports the predict method",
)
@click.option(
    "--log-level",
    "-l",
    envvar="KOZMO_BATCH_LOG_LEVEL",
...
@click.option(
    "--batch-interval",
    "-t",
    envvar="KOZMO_BATCH_MIN_INTERVAL",
    default=0,
    type=float,
...
@click.option(
    ...
    envvar="KOZMO_BATCH_CALL_CREDENTIALS_TOKEN",
    default="",
    type=str,
    help="Auth token used by Kozmo Client, if supplied and using REST the transport protocol will be https.",
)
@click.option(
    "--ssl-verify",
    envvar="KOZMO_BATCH_SSL_VERIFY",
    default=True,
    type=bool,
...
def run_cli(
    ...
    """
    Command line interface for Kozmo Batch Processor, which can be used to send requests
    through configurable parallel workers to Kozmo Core models. It is recommended that the
    respective Kozmo Core model is also optimized with number of replicas to distribute
    and scale out the batch processing work. The processor is able to process data from local
    filestore input file in various formats supported by the KozmoClient module. It is also
    ...
    steps such as downloading the data from object store or starting a kozmo core model with replicas.
    See the Kozmo Core examples folder for implementations of this batch module with Kozmo Core.
    """
