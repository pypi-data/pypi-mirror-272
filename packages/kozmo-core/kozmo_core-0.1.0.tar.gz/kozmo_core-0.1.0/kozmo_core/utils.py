...
from kozmo_core.env_utils import (
    ENV_MODEL_IMAGE,
    ENV_MODEL_NAME,
...
def json_to_kozmo_message(
    message_json: Union[List, Dict]
) -> prediction_pb2.KozmoMessage:
    """
    Parses JSON input to a KozmoMessage proto
    ...
    if message_json is None:
        message_json = {}
    message_proto = prediction_pb2.KozmoMessage()
    try:
        json_format.ParseDict(message_json, message_proto)
        return message_proto
    except json_format.ParseError as pbExc:
        raise KozmoMicroserviceException("Invalid JSON: " + str(pbExc))


def json_to_kozmo_model_metadata(
    metadata_json: Dict,
) -> prediction_pb2.KozmoModelMetadata:
    """
    Parses JSON input to KozmoModelMetadata proto
    ...
    if metadata_json is None:
        metadata_json = {}
    metadata_proto = prediction_pb2.KozmoModelMetadata()
    try:
        json_format.ParseDict(metadata_json, metadata_proto, ignore_unknown_fields=True)
        return metadata_proto
    except json_format.ParseError as pbExc:
        raise KozmoMicroserviceException(f"Invalid metadata: {pbExc}")
...
def json_to_kozmo_messages(message_json: Dict) -> prediction_pb2.KozmoMessageList:
    message_proto = prediction_pb2.KozmoMessageList()
    try:
        json_format.ParseDict(message_json, message_proto)
        return message_proto
    except json_format.ParseError as pbExc:
...
def kozmo_message_to_json(message_proto: prediction_pb2.KozmoMessage) -> Dict:
    """
    Convert a KozmoMessage proto to JSON Dict
...
def kozmo_messages_to_json(message_protos: prediction_pb2.KozmoMessageList) -> Dict:
    """
    Convert a KozmoMessage proto list to JSON Dict
...
def get_data_from_proto(
    request: prediction_pb2.KozmoMessage,
) -> Union[np.ndarray, str, bytes, dict]:
    """
    Extract the data payload from the KozmoMessage
    ...
    if data_type == "data":
        ...
        raise KozmoMicroserviceException("Unknown data in KozmoMessage")


def grpc_datadef_to_array(datadef: prediction_pb2.DefaultData) -> np.ndarray:
    """
    Convert a KozmoMessage DefaultData to a numpy array.

    Parameters
    ----------
    datadef
       KozmoMessage DefaultData
...
def get_meta_from_proto(request: prediction_pb2.KozmoMessage) -> Dict:
    """
    Convert KozmoMessage proto meta into Dict
...
def array_to_rest_datadef(
    ...
    Returns
    -------
       Dict representing Kozmo payload
...
def array_to_grpc_datadef(
    ...
    """
    Convert numpy array and optional column names into a KozmoMessage DefaultData proto
    ...
    names
       column names
    data_type
       The KozmoMessage type to convert to
...
def construct_response_json(
    ...
    the KozmoMessage proto. This is necessary as the conversion using the KozmoMessage proto
...
def construct_response(
    user_model: KozmoComponent,
    is_request: bool,
    client_request: prediction_pb2.KozmoMessage,
    ...
    if isinstance(client_raw_response, np.ndarray) or isinstance(
        ...
        return prediction_pb2.KozmoMessage(data=data, meta=meta_pb)
    elif isinstance(client_raw_response, str):
        return prediction_pb2.KozmoMessage(strData=client_raw_response, meta=meta_pb)
    elif isinstance(client_raw_response, dict):
        jsonDataResponse = ParseDict(
            client_raw_response, prediction_pb2.KozmoMessage().jsonData
        )
        return prediction_pb2.KozmoMessage(jsonData=jsonDataResponse, meta=meta_pb)
    elif isinstance(client_raw_response, (bytes, bytearray)):
        return prediction_pb2.KozmoMessage(binData=client_raw_response, meta=meta_pb)
    elif isinstance(client_raw_response, any_pb2.Any):
        return prediction_pb2.KozmoMessage(
...
def extract_request_parts_json(
    ...
    if not isinstance(request, dict):
        raise KozmoMicroserviceException(f"Invalid request data type: {request}")
