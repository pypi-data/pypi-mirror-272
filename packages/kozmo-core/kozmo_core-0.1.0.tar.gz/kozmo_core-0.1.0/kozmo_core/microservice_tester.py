...
class KozmoTesterException(Exception):
...
def reconciliate_cont_type(feature: np.ndarray, dtype: str) -> np.ndarray:
    ...
    if dtype == "FLOAT":
        ...
        raise KozmoTesterException(f"Unknown dtype in reconciliate_cont_type {dtype}")
...
def generate_batch(contract: Dict, n: int, field: str) -> np.ndarray:
    ...
    for feature_def in contract[field]:
        ...
        if feature_def["ftype"] == "continuous":
            ...
            raise KozmoTesterException(f"Unknown feature type {feature_def['ftype']}")
...
def run_send_feedback(args):
    ...
    sc = KozmoClient(microservice_endpoint=endpoint)
