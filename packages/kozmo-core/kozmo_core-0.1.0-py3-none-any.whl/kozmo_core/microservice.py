...
from kozmo_core import __version__
from kozmo_core import wrapper as kozmo_microservice
...
from kozmo_core.gunicorn_utils import (
...
from kozmo_core.utils import getenv_as_bool, setup_tracing

# This is related to how multiprocessing is implemeneted on MacOS
# See https://github.com/kozmoai/kozmo-core/issues/3410 for discussion.
...
class MetricsEndpointFilter(logging.Filter):
    def filter(self, record):
        return kozmo_microservice.METRICS_ENDPOINT not in record.getMessage()
...
def setup_logger(log_level: str, debug_mode: bool) -> logging.Logger:
    ...
    kozmo_microservice.logger.setLevel(log_level_num)
...
def parse_args() -> Tuple[argparse.Namespace, List[str]]:
    ...
    parser.add_argument(
        ...
        help="Set http port of kozmo service",
    )
    ...
    parser.add_argument(
        ...
        help="Set grpc port of kozmo service",
    )
    ...
    parser.add_argument(
        ...
        help="Set metrics port of kozmo service",
    )
...
def _make_rest_server_debug(
    ...
    Args:
        user_object: an instance of user-defined class, inherited from user_model.KozmoComponent.
        kozmo_metrics: a KozmoMetrics instance.
        args: parsed args from commandline.
        jaeger_extra_tags:
    """

    def server():
        app = kozmo_microservice.get_rest_microservice(user_object, kozmo_metrics)
        try:
            user_object.load()
...
def _make_rest_server_prod(
    ...
    def server() -> None:
        ...
        options = {
            ...
            "worker_exit": partial(worker_exit, kozmo_metrics=kozmo_metrics),
...
def _run_grpc_server(
    ...
    server = kozmo_microservice.get_grpc_server(
        user_object,
        kozmo_metrics,
...
def _make_rest_metrics_server(
    ...
    def server() -> None:
        app = kozmo_microservice.get_metrics_microservice(kozmo_metrics)
        if args.debug:
...
def main():
    ...
    logger.info(f"Kozmo Core version: {__version__}")
    ...
    kozmo_metrics = KozmoMetrics(worker_id_func=os.getpid)
    ...
    if args.debug:
        ...
        server_rest_func = _make_rest_server_debug(
            user_object, kozmo_metrics, args, jaeger_extra_tags=jaeger_extra_tags
        )
    else:
    ...
    if args.grpc_workers > 0:
        server_grpc_func = _make_grpc_server(
            user_object, kozmo_metrics, args, annotations=annotations
        )

    logger.info("REST metrics microservice running on port %i", args.metrics_port)
    server_metrics_func = _make_rest_metrics_server(kozmo_metrics, args)
