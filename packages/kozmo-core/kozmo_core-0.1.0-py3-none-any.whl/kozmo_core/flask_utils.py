...
def get_multi_form_data_request() -> Dict:
    ...
    all the keys under KozmoMessage are accepted as form input
...
def get_request(skip_decoding=False) -> Union[Dict, bytes]:
    ...
    if skip_decoding:
        try:
            ...
            raise KozmoMicroserviceException("Can't find data")
        if data is None:
            raise KozmoMicroserviceException("Can't find data")
    ...
    try:
        ...
        raise KozmoMicroserviceException("Can't find JSON in data")
    if message is None:
        raise KozmoMicroserviceException("Can't find JSON in data")
...
class KozmoMicroserviceException(Exception):
    status_code = 400
...
ANNOTATION_GRPC_MAX_MSG_SIZE = "kozmo.io/grpc-max-message-size"
DEFAULT_ANNOTATION_REST_TIMEOUT = 5000
ANNOTATION_REST_TIMEOUT = "kozmo.io/rest-timeout"
