...
FEEDBACK_KEY = "kozmo_api_model_feedback"
FEEDBACK_REWARD_KEY = "kozmo_api_model_feedback_reward"
...
def split_image_tag(tag: str) -> Tuple[str]:
    ...
    ----------
    tag
        Fully qualified docker image tag. Eg. kozmoai/sklearn-iris:0.1
...
legacy_mode = os.environ.get("KOZMO_EXECUTOR_ENABLED", "true").lower() == "false"
...
# Compatibility layer of tags until Kozmo-Core 1.3
DEFAULT_LABELS["kozmo_deployment_name"] = DEFAULT_LABELS["deployment_name"]
...
class KozmoMetrics:
    ...
    def update(self, custom_metrics: List[Dict], method: str):
        ...
        for metrics in custom_metrics:
            ...
            tags["method"] = method
            key = (metrics_type, metrics["key"], KozmoMetrics._generate_tags_key(tags))
    ...
    def collect(self):
        ...
        logger.debug("KozmoMetrics.collect called")
        with self._lock:
            data = dict(self.data)
        logger.debug("Read current metrics data from shared memory")
