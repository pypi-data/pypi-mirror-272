...
from kozmo_core.utils import (
    ...
    kozmo_messages_to_json,
)

logger = logging.getLogger(__name__)


class KozmoClientException(Exception):
    """
    Kozmo Client Exception
    """
...
class KozmoClientPrediction:
    """
    Data class to return from Kozmo Client
    """

    def __init__(
        self,
        request: Optional[Union[prediction_pb2.KozmoMessage, Dict]],
        response: Optional[Union[prediction_pb2.KozmoMessage, Dict]],
        success: bool = True,
        msg: str = "",
    ):
...
class KozmoClientFeedback:
    """
    Data class to return from Kozmo Client for feedback calls
    """
...
class KozmoClientCombine:
    """
    Data class to return from Kozmo Client for aggregate calls
    """

    def __init__(
        self,
        request: Optional[prediction_pb2.KozmoMessageList],
        response: Optional[prediction_pb2.KozmoMessage],
        success: bool = True,
        msg: str = "",
    ):
        self.request = request
...
class KozmoClient:
    ...
    def __init__(
        ...
        channel_credentials: KozmoChannelCredentials = None,
        call_credentials: KozmoCallCredentials = None,
        debug: bool = False,
        ...
        ----------
        gateway
           API Gateway - either ambassador, istio or kozmo
        transport
           API transport - grpc or rest
        namespace
           k8s namespace of running deployment
        deployment_name
           name of kozmo deployment
        payload_type
    ...
    def _validate_args(
        ...
        **kwargs,
        ...
        """
        if not (gateway == "ambassador" or gateway == "kozmo" or gateway == "istio"):
            raise KozmoClientException(
                "Valid values for gateway are 'ambassador', 'istio', or 'kozmo'"
            )
        ...
        if not (data is None or isinstance(data, np.ndarray)):
            raise KozmoClientException("Valid values for data are None or numpy array")
    ...
    def predict(
        ...
        if k["gateway"] == "ambassador" or k["gateway"] == "istio":
            if k["transport"] == "rest":
                ...
                raise KozmoClientException("Unknown transport " + k["transport"])
        elif k["gateway"] == "kozmo":
            if k["transport"] == "rest":
                return rest_predict_kozmo(**k)
            elif k["transport"] == "grpc":
                return grpc_predict_kozmo(**k)
            else:
                raise KozmoClientException("Unknown transport " + k["transport"])
        else:
            raise KozmoClientException("Unknown gateway " + k["gateway"])

    def feedback(
        self,
        prediction_request: prediction_pb2.KozmoMessage = None,
        prediction_response: prediction_pb2.KozmoMessage = None,
        prediction_truth: prediction_pb2.KozmoMessage = None,
        reward: float = 0,
        gateway: str = None,
        transport: str = None,
        deployment_name: str = None,
    ...
    def explain(
        ...
        if k["gateway"] == "ambassador" or k["gateway"] == "istio":
            if k["transport"] == "rest":
                ...
                raise KozmoClientException("gRPC not supported for explain")
            else:
                raise KozmoClientException("Unknown transport " + k["transport"])
        else:
            raise KozmoClientException("Unknown gateway " + k["gateway"])
    ...
    def microservice(
        ...
        custom_data: any_pb2.Any = None,
        names: Iterable[str] = None,
    ) -> Union[KozmoClientPrediction, KozmoClientCombine]:
        """
        ...
        if k["transport"] == "rest":
            if (
                ...
                return microservice_api_rest_kozmo_message(**k)
            elif k["method"] == "aggregate":
                return microservice_api_rest_aggregate(**k)
            else:
                raise KozmoClientException("Unknown method " + k["method"])
        elif k["transport"] == "grpc":
            if (
                k["method"] == "predict"
                or k["method"] == "transform-input"
                ...
                return microservice_api_grpc_kozmo_message(**k)
            elif k["method"] == "aggregate":
    ...
    def microservice_feedback(
        ...
        reward
           A reward to send in feedback
        gateway
           API Gateway - either Gateway or kozmo
        transport
           API transport - grpc or rest
        deployment_name
           name of kozmo deployment
...
def microservice_api_rest_kozmo_message(
    method: str = "predict",
    microservice_endpoint: str = "localhost:5000",
    shape: Tuple = (1, 1),
    data: object = None,
    ...
    **kwargs,
    ...
    """
    Call Kozmo microservice REST API
    ...
    Returns
    -------
      A KozmoClientPrediction data response
    """
    if bin_data is not None:
        request = prediction_pb2.KozmoMessage(binData=bin_data)
    elif str_data is not None:
        request = prediction_pb2.KozmoMessage(strData=str_data)
    elif json_data is not None:
        request = json_to_kozmo_message({"jsonData": json_data})
    else:
        if data is None:
            data = np.random.rand(*shape)
        datadef = array_to_grpc_datadef(payload_type, data, names=names)
        request = prediction_pb2.KozmoMessage(data=datadef)
    payload = kozmo_message_to_json(request)
    ...
    try:
        response = json_to_kozmo_message(response_raw.json())
        return KozmoClientPrediction(request, response, success, msg)
    except Exception as e:
        return KozmoClientPrediction(request, None, success, str(e))
...
def microservice_api_rest_aggregate(
    ...
    **kwargs,
    ...
    """
    Call Kozmo microservice REST API aggregate endpoint
    ...
    for data in datas:
        if isinstance(data, (bytes, bytearray)):
            msgs.append(prediction_pb2.KozmoMessage(binData=data))
        elif isinstance(data, str):
            msgs.append(prediction_pb2.KozmoMessage(strData=data))
        else:
            datadef = array_to_grpc_datadef(payload_type, data, names)
            msgs.append(prediction_pb2.KozmoMessage(data=datadef))
    request = prediction_pb2.KozmoMessageList(kozmoMessages=msgs)
    payload = kozmo_messages_to_json(request)
    response_raw = requests.post(
    ...
    try:
        response = json_to_kozmo_message(response_raw.json())
        return KozmoClientCombine(request, response, success, msg)
    except Exception as e:
        return KozmoClientCombine(request, None, success, str(e))
...
def microservice_api_rest_feedback(
    ...
    **kwargs,
    ...
    """
    Call Kozmo microserice REST API to send feedback
    ...
    try:
        response = json_to_kozmo_message(response_raw.json())
        return KozmoClientFeedback(request, response, success, msg)
    except Exception as e:
        return KozmoClientFeedback(request, None, success, str(e))


def microservice_api_grpc_kozmo_message(
    method: str = "predict",
    microservice_endpoint: str = "localhost:5000",
    shape: Tuple = (1, 1),
    data: object = None,
    ...
    **kwargs,
    ...
    """
    Call Kozmo microservice gRPC API
    ...
    if bin_data is not None:
        ...
        request = prediction_pb2.KozmoMessage(customData=custom_data)
    else:
        if data is None:
            data = np.random.rand(*shape)
        datadef = array_to_grpc_datadef(payload_type, data, names=names)
    ...
    try:
        if method == "predict":
            ...
            raise KozmoClientException("Unknown method:" + method)

        channel.close()
        return KozmoClientPrediction(request, response, True, "")
    except Exception as e:
        channel.close()
        return KozmoClientPrediction(request, None, False, str(e))
...
def microservice_api_grpc_aggregate(
    ...
    **kwargs,
    ...
    """
    Call Kozmo microservice gRPC API aggregate
    ...
    for data in datas:
        if isinstance(data, (bytes, bytearray)):
            ...
            msgs.append(prediction_pb2.KozmoMessage(customData=data))
        else:
            datadef = array_to_grpc_datadef(payload_type, data, names=names)
            msgs.append(prediction_pb2.KozmoMessage(data=datadef))
    request = prediction_pb2.KozmoMessageList(kozmoMessages=msgs)
    try:
        ...
        channel.close()
        return KozmoClientCombine(request, response, True, "")
    except Exception as e:
        return KozmoClientCombine(request, None, False, str(e))
...
def microservice_api_grpc_feedback(
    ...
    **kwargs,
    ...
    try:
        ...
        channel.close()
        return KozmoClientFeedback(request, response, True, "")
    except Exception as e:
        return KozmoClientFeedback(request, None, False, str(e))
...
def rest_predict_kozmo(
    ...
    **kwargs,
    ...
    """
    Call Kozmo API Gateway using REST
    ...
    if raw_data:
        request = json_to_kozmo_message(raw_data)
        payload = raw_data
    else:
        if bin_data is not None:
            request = prediction_pb2.KozmoMessage(binData=bin_data, meta=metaKV)
        elif str_data is not None:
            request = prediction_pb2.KozmoMessage(strData=str_data, meta=metaKV)
        elif json_data is not None:
            request = json_to_kozmo_message({"jsonData": json_data})
        else:
            if data is None:
                data = np.random.rand(*shape)
            datadef = array_to_grpc_datadef(payload_type, data, names=names)
            request = prediction_pb2.KozmoMessage(data=datadef, meta=metaKV)
        payload = kozmo_message_to_json(request)
        ...
        if len(response_raw.text) > 0:
            try:
                if client_return_type == "proto":
                    ...
                    KozmoClientException("Invalid client_return_type")
            except:
                response = None
        else:
            response = None
...
def grpc_predict_kozmo(
    ...
    **kwargs,
    ...
    """
    Call Kozmo gRPC API Gateway endpoint
    ...
    Returns
    -------
       A KozmoMessage proto

    """
    if isinstance(raw_data, prediction_pb2.KozmoMessage):
        request = raw_data
    elif raw_data:
        request = json_to_kozmo_message(raw_data)
    else:
    ...
    stub = prediction_pb2_grpc.KozmoStub(channel)
    try:
        response = stub.Predict(request=request)
        channel.close()
        if client_return_type == "dict":
            request = kozmo_message_to_json(request)
            response = kozmo_message_to_json(response)
        elif client_return_type != "proto":
            raise KozmoClientException("Invalid client_return_type")
        return KozmoClientPrediction(request, response, True, "")
    except Exception as e:
        channel.close()
        return KozmoClientPrediction(request, None, False, str(e))
...
def rest_predict_gateway(
    ...
    **kwargs,
    ...
    ----------
    deployment_name
       The name of the Kozmo Deployment
    namespace
       k8s namespace of running deployment
    gateway_endpoint
    ...
    names
       Column names
    call_credentials
       Call credentials - see KozmoCallCredentials
    channel_credentials
       Channel credentials - see KozmoChannelCredentials
    http_path
       Custom http path
    meta
       Custom meta map
...
def explain_predict_gateway(
    ...
    **kwargs,
    ...
    gateway_endpoint
       The host:port of gateway
    gateway
       The type of gateway which can be kozmo or ambassador/istio
    transport
    ...
    """
    if transport != "rest":
        raise KozmoClientException("Only supported transport is REST for explanations")
    ...
    if response_raw.status_code == 200:
        if client_return_type == "dict":
            ...
            raise KozmoClientException("Invalid client_return_type")
        return KozmoClientPrediction(ret_request, ret_response, True, "")
    else:
        return KozmoClientPrediction(
            payload,
            response_raw,
            False,
...
def grpc_predict_gateway(
    ...
    **kwargs,
    ...
    ----------
    deployment_name
       Deployment name of Kozmo Deployment
    namespace
       The namespace the Kozmo Deployment is running in
    gateway_endpoint
    ...
    if isinstance(raw_data, prediction_pb2.KozmoMessage):
        ...
        if bin_data is not None:
            ...
            request = prediction_pb2.KozmoMessage(customData=custom_data, meta=metaKV)
        else:
            if data is None:
                data = np.random.rand(*shape)
            datadef = array_to_grpc_datadef(payload_type, data, names=names)
    ...
    stub = prediction_pb2_grpc.KozmoStub(channel)
    if namespace is None:
        metadata = [("kozmo", deployment_name)]
    else:
        metadata = [("kozmo", deployment_name), ("namespace", namespace)]
    if not headers is None:
        for k in headers:
...
def rest_feedback_kozmo(
    ...
    **kwargs,
    ...
    """
    Send Feedback to Kozmo API Gateway using REST
...
def grpc_feedback_kozmo(
    ...
    **kwargs,
    ...
    """
    Send feedback to Kozmo API gateway via gRPC
...
def rest_feedback_gateway(
    ...
    **kwargs,
    ...
    """
    Send Feedback to Kozmo via gateway using REST
    ...
    reward
       A reward to send in feedback
    deployment_name
       The name of the running Kozmo deployment
    namespace
       k8s namespace of running deployment
    gateway_endpoint
