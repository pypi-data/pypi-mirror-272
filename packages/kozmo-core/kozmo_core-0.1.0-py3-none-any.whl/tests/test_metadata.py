...
from kozmo_core.utils import json_to_kozmo_model_metadata
from kozmo_core.wrapper import KozmoModelGRPC, get_rest_microservice
...
def test_default_values_with_env():
    with patch("kozmo_core.metadata.MODEL_IMAGE", "kozmoai/sklearn-iris:0.1"):
        meta = validate_model_metadata({})
    assert meta == {
        "name": "kozmoai/sklearn-iris",
        "versions": ["0.1"],
        "platform": "",
        "inputs": [],
        "outputs": [],
...
def test_default_values_with_colon_in_env():
    with patch("kozmo_core.metadata.MODEL_IMAGE", "localhost:32000/sklearn-iris:0.1"):
        meta = validate_model_metadata({})
    assert meta == {
...
def test_v1_only_messagetype(messagetype):
    ...
    meta_proto = json_to_kozmo_model_metadata(meta_json)
    assert meta_json == {
        "name": "my-model-name",
        "versions": ["my-model-version-01"],
        "platform": "kozmo",
...
def test_v1_invalid_inputs(invalid_input):
    ...
    with pytest.raises(KozmoInvalidMetadataError):
...
def test_validate_model_metadata():
    ...
    with patch("kozmo_core.metadata.MODEL_IMAGE", None):
