...
import kozmo_core.utils as scu
from kozmo_core.env_utils import (
    ENV_MODEL_IMAGE,
    ENV_MODEL_NAME,
    ENV_PREDICTOR_LABELS,
    ENV_PREDICTOR_NAME,
    ENV_KOZMO_DEPLOYMENT_NAME,
...
def test_symmetric_json_conversion():
    ...
    datadef = scu.array_to_rest_datadef("ndarray", request_data)
    json_request = {"jsonData": datadef}
    kozmo_message_request = scu.json_to_kozmo_message(json_request)
    result_json_request = scu.kozmo_message_to_json(kozmo_message_request)
...
def test_json_to_kozmo_message_normal_data():
    data = {"data": {"tensor": {"shape": [1, 1], "values": [1]}}}
    requestProto = scu.json_to_kozmo_message(data)
...
def test_json_to_kozmo_message_ndarray():
    data = {"data": {"ndarray": [[1]]}}
    requestProto = scu.json_to_kozmo_message(data)
...
def test_json_to_kozmo_message_bin_data():
    a = np.array([1, 2, 3])
...
def test_json_to_kozmo_message_str_data():
...
def test_json_to_kozmo_message_json_data():
...
def test_json_to_kozmo_message_bad_data():
    with pytest.raises(KozmoMicroserviceException):
        data = {"foo": "bar"}
        scu.json_to_kozmo_message(data)
...
def test_json_to_kozmo_messages():
    data = {
        "kozmoMessages": [
    ...
    requestProto = scu.json_to_kozmo_messages(data)
    assert requestProto.kozmoMessages[0].data.tensor.values == [1]
    assert requestProto.kozmoMessages[1].data.tensor.values == [2]
    assert len(requestProto.kozmoMessages) == 2


def test_kozmo_message_to_json():
    arr = np.array([1, 2])
    datadef = prediction_pb2.DefaultData(
        tensor=prediction_pb2.Tensor(shape=(2, 1), values=arr)
    )
    request = prediction_pb2.KozmoMessage(data=datadef)
    dict = scu.kozmo_message_to_json(request)
