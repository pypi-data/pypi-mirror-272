...
class Station:
    ...
    def follow_trace(self, trace, gain: float=1.0):
        ...
        self._trace_cols.append(f'znle.{chan_name.lower()}.frequency')
    ...
    def sweep_vna_1d(self, delay: float=0.0):
        with db.Writer(self._basedir, fsync_every=1000) as w, self._plotter as p:
            ...
            for i in range(len(self._traces)):
                fast_vs.append(self._traces[i][0]._parent.params.get_freq_setpoints())
            ...
            for trace, fast_v in zip(self._traces, fast_vs):
                gain = trace[1]
                trace = trace[0]
                trace.set_as_active()
                ...
                processed_data = [fast_v, i_data, q_data, noises]
            ...
            traces_points = list(map(list, zip(*traces_data)))
            ...
            for j in range(len(traces_points)):
                ...
                point_data = list(chain.from_iterable(traces_points[j]))
                data = [time.time()] + self._measure()
    ...
    def sweep_vna_2d(self, slow_param, slow_v, slow_delay=0):
        with db.Writer(self._basedir, fsync_every=5001 if slow_delay >= 10.0 else 10002) as w, self._plotter as p:
            ...
            self._print(f'Starting run with ID {w.id}')
            min_duration = len(slow_v) * len(fast_vs[0]) * fast_delay + len(slow_v) * slow_delay
            self._print(f'Minimum duration {_sec_to_str(min_duration)}')
            ...
            w.metadata['slow_param'] = slow_param.full_name
            w.metadata['trace_information'] = trace_infos
            w.metadata['columns'] = ['time', slow_param.full_name] + self._col_names()
            ...
            for i in range(len(slow_v)):
                self._traces[0][0].root_instrument.freedom(False)
                ov = slow_v[i]
                slow_param(ov)
                time.sleep(slow_delay)
                wait_time_vna_finish = 0
                for trace in self._traces:
                    chn = trace[0].parent
                    wait_time_vna_finish += chn.params.sweep_time.get()
                self._traces[0][0].root_instrument.trigger_sweep_restart()
                time.sleep(wait_time_vna_finish + 2)
                ...
                for j in range(len(traces_points)):
                    ...
                    point_data = list(chain.from_iterable(traces_points[j]))
                    data = [time.time(), ov] + slow_measurement
                    data.extend(point_data)
            ...
            self._traces[0][0].root_instrument.freedom(True)
