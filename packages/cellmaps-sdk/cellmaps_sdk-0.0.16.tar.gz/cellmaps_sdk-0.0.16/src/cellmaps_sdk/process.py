...
from .data import (
    DearrayedTissueMicroArray, DearrayedTissueMicroArrayCellSegmentationMask, DearrayedTissueMicroArrayMissileFCS, MembraneMarkers, MissileClusters, MissileExpressionCounts, MissileExpressionSpatialData, MissileMetadata, MissileNeighbourhoods, NuclearMarkers, NuclearStain, Plot, PredictedROIs, ProteinChannelMarkers, 
    TissueMicroArray, WholeSlideImage, WholeSlideImageCellSegmentationMask,ROIs, WholeSlideImageMissileFCS)
...
# How k8s populates environment variables
# $SERVICE_NAME$_SERVICE_HOST
# $SERVICE_NAME$_SERVICE_PORT

# NEED TO IMPLEMENT INTERFACE FOR DE-SERIALIZING INPUTS FROM k8s Environment Variables
# CINCODEBIO_DATA_PAYLOAD
# CINCODEBIO_JOB_ID
# CINCODEBIO_ROUTING_KEY
# CINCODEBIO_BASE_URL
...
class Automated(Service, metaclass=EnforceTypeMetaClass):
    ...
    def _do_work(self,routing_key, body):
        ...
        if _re.match(fr"\S*\.{self._a}\b$",routing_key):
            ...
            # Only make requests to update JMS if being used in production
            ...
            args = self.process(
                prefix=self._get_root_prefix(workflow_id=job_obj['workflow']),
                input=args)
...
# Tool Developer has to provide concrete attributes / properties for routing keys.etc..
# Tool developer has to implement process / prepare template methods
# Need to find a way to inject type dependency through a form of mix (i.e. for cell segmentqation, etc..)
# How will I serialise / deserialise data?


# Automated -

# Process Input*: What is modelled in the IME (sent from execution ENV to DPS)

# Process Output*: What is modelled in the IME (sent from DPS to Execution ENV)

# Interactive - 

# Template Input*: What is modelled in the IME (sent from execution ENV to DPS)
    # Return a Job Object to execution ENV

# Template Output: This is the HTML frontend for the DPS
    # Retrieved from Client front-end using Job.id

# Process Input: This is what the Frontend Sends to the DPS (after user-interaction)
    # Uses parameterised endpoint injected to front-end to submit

# Process Output*: What is modelled in the IME (sent from DPS to Execution ENV)
    # Retrieved for the execution ENV using Job.id
    

"""
    routing_key = what service it goes to;
    body = {
        job_id: str,
        base_url: str, (only in the case of interactive (also needs to be replaced))
        args : {
            system_parameters : {data_flow: {bool} #which dataflow edges are used} # these are internal variables (user doesn't care)
            workflow_parameters : {} # these are parameters which do not point to data in the form of files
            tool_paramters : {} # these are parameters specific to a DPS if it is parameterised (not currently supported)
            data: {} # these are symbolic links which point to data (i.e. need to be written/retrieved in DPS)
            control: enum # this is logic only used for output (i.e. if the DPS makes some decision) 
            metadata : {} # this is metadata generated about the processing of the DPS only used for output (not currently supported)
        }   
    }
"""

# How do I handle optional?
class Start(Service):
    models: _List = [
        (set(),{TissueMicroArray,typing.Optional[NuclearMarkers],typing.Optional[NuclearStain],typing.Optional[MembraneMarkers],ProteinChannelMarkers}),
        (set(),{WholeSlideImage,typing.Optional[NuclearMarkers],typing.Optional[NuclearStain],typing.Optional[MembraneMarkers],ProteinChannelMarkers})]
    

# DeArray - Tools from cropping TMAs
class DeArray(Service):
    models: _List = [
        ({TissueMicroArray, NuclearStain},{ROIs}), #Predict ROIs (single Prediction) - Manual
        ({TissueMicroArray, NuclearStain},{PredictedROIs}), # Predict ROIs (multiple predictions) - SegArray
        ({TissueMicroArray, NuclearStain, ROIs},{ROIs}), # Adjust Single Predictions
        ({TissueMicroArray, NuclearStain, PredictedROIs},{ROIs}), # Edit Predicted ROIs - Select result of Segarray
        ({TissueMicroArray, ROIs},{DearrayedTissueMicroArray}), # Crop out Cores from ROIs
        ]

class TechnicalVarianceCorrection(Service):
    models: _List = [ 
        ({TissueMicroArray},{TissueMicroArray}), # Correct TMA (Slide Level)
        ({DearrayedTissueMicroArray},{DearrayedTissueMicroArray}), # Correct TMA Core Level
        ({WholeSlideImage},{WholeSlideImage}), # Correct Whole Slide Image
    ]


class CellSegmentation(Service):
    models: _List = [
    ({DearrayedTissueMicroArray,NuclearStain},{DearrayedTissueMicroArrayCellSegmentationMask}), # Nucleus only predicition
    ({DearrayedTissueMicroArray,NuclearStain,MembraneMarkers},{DearrayedTissueMicroArrayCellSegmentationMask}), # Nucleus & Membrane Prediction
    ({WholeSlideImage,NuclearStain},{WholeSlideImageCellSegmentationMask}), # Nucleus only predicition
    ({WholeSlideImage,NuclearStain,MembraneMarkers},{WholeSlideImageCellSegmentationMask}), # Nucleus & Membrane Prediction
    ]

class FeatureExtraction(Service):
    models: _List = [
        ({DearrayedTissueMicroArray,DearrayedTissueMicroArrayCellSegmentationMask,NuclearMarkers,ProteinChannelMarkers},{DearrayedTissueMicroArrayMissileFCS}), # Extract Spatial / Proteomic Features from DTMA
        ({WholeSlideImage,WholeSlideImageCellSegmentationMask,NuclearMarkers,ProteinChannelMarkers},{WholeSlideImageMissileFCS}), # Extract Spatial / Proteomic Features from WSI
        ]


class DataTransformation(Service):
    models: _List = [
        ({DearrayedTissueMicroArrayMissileFCS,ProteinChannelMarkers},{MissileMetadata,MissileExpressionCounts,MissileExpressionSpatialData}), # Extract Spatial / Proteomic Features from DTMA
        ({WholeSlideImageMissileFCS,ProteinChannelMarkers},{MissileMetadata,MissileExpressionCounts,MissileExpressionSpatialData}),
        ]
    

class Plotting(Service):
    models: _List = [
        ({MissileExpressionCounts,MissileClusters,ProteinChannelMarkers},{Plot}), 
        ({MissileNeighbourhoods,MissileClusters},{Plot}), 
        ]
    

class Clustering(Service):
    models: _List = [
        ({MissileExpressionCounts,ProteinChannelMarkers},{MissileClusters}),
        ({MissileExpressionCounts,MissileMetadata,ProteinChannelMarkers},{MissileClusters}),
        ({MissileClusters,MissileMetadata,MissileExpressionSpatialData},{MissileNeighbourhoods})
    ]
