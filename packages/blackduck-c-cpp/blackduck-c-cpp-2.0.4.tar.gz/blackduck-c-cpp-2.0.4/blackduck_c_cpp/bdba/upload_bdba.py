...
class BDBAApi:
    ...
    def upload_binary_part(self, project_name, version_name, codelocation_name, binary_file, file_chunk):
        ...
        with tqdm.tqdm(desc="BDBA Upload",
            ...
            try:
                csrf_token = self.hub.session.auth.__getattribute__('csrf_token');
                bearer_token = self.hub.session.auth.__getattribute__('bearer_token');
                headers['X-CSRF-TOKEN'] = csrf_token;
                headers['Authorization'] = 'Bearer {}'.format(bearer_token);

                ## not using session.post to have more control over headers
                response = requests.post('{}/api/uploads'.format(self.hub_api.bd_url),
                                         headers=headers,
                                         data=multipart_monitor,
                                         verify=not self.hub_api.insecure, timeout=600)
                if not response.ok:
