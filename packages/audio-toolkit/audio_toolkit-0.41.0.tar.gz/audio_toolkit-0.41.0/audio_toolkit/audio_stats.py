...
class AudioStatsV4:
    ...
    def __enter__(self) -> AudioStatsV4:
        ...
        if os.path.exists(self.cache_path):
            ...
            row_list = handle.fetchall()
            t1 = time.time()
            print(f"load cache time {self.cache_path}: {t1 - t0}", file=sys.stderr)

            for path, sample_rate, frame_count in tqdm(row_list, desc=f"indexing cache {self.cache_path} ..."):
