...
class AudioStatsV4:
    def __init__(self, cache_path: str = "/tmp/audio_stats.csv"):
        ...
        self.cache = pkm.str_int64.create()
        self.row_list = []
        self.f = None
    ...
    def __enter__(self) -> AudioStatsV4:
        ...
        if os.path.exists(self.cache_path):
            ...
            while True:
                ...
                if len(row_list) == 0:
                    break
                s_i = len(self.row_list)
                self.row_list.extend(row_list)
                for d_i, row in enumerate(row_list):
                    i = s_i + d_i
    ...
    def get(self, path: str) -> Dict[str, Union[int, float]]:
        ...
        if path not in self.cache:
            ext = os.path.splitext(path)[1]
            frame_count, sample_rate = get_duration[ext.lower()](path)
            self.row_list.append((path, sample_rate, frame_count))
            self.cache[path] = len(self.row_list) - 1
            self.f.write(f"{path}|{sample_rate}|{frame_count}\n")

        return self.cache[path][1:3]
