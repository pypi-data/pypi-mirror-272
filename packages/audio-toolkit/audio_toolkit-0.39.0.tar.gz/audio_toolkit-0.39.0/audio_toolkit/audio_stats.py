...
class AudioStatsV4:
    ...
    def __enter__(self) -> AudioStatsV4:
        ...
        if os.path.exists(self.cache_path):
            # load cache file
            def fetch(handle, batch_size) -> Iterator[list]:
                while True:
                    row_list = handle.fetchmany(batch_size)
                    if len(row_list) == 0:
                        return
                    yield from row_list

            handle = duckdb.read_csv(self.cache_path, sep="|", header=False)
            for row in tqdm(fetch(handle, FETCH_BATCH_SIZE), desc=f"loading and indexing cache {self.cache_path} ..."):
