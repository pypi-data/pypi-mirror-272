...
class Database(auth.Authentication):
    ...
    def get_relevant_solder_ids(self, institute_id, strict=True):
        ...
        def ss_valid(sold, dt_now, strict):
            """
            ...
            return dt_now <= dt_exp and dt_now <= dt_rtd_p30 if strict else True

        valid = functools.partial(
            ss_valid, dt_now=datetime.datetime.now(), strict=strict
        )
    ...
    def get_vtile_qc(self, qrcode, grade=None):
        ...
        def check_grade(grade):
            ...
            limits = json.loads(self.get('vtile_qc', grade=grade).data.qc.iloc[-1])
            ...
            for variable in variables:
                ...
                if v < limits[f'{variable}_min'] or v > limits[f'{variable}_max'] or math.isnan(v):
                    # variable already out of the limits or NaN, so so could
