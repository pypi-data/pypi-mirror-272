...
import numpy as np
#import qedata
from qdsdk import is_valid_trade_time
...
class qeContextBase(object):
    ...
    def getCommission(self, action, price, vol, yesvol, instid):
        ...
        if tempdata != "NULL":
            ...
            if action == 'open':
                ...
                return totalrate * (
                            tempdata['closefee'] * vol + tempdata['closefeerate'] * vol * price * tempdata['volmult'])
        else:
    ...
    def crossDay(self):
        ...
        for inst in self.instid:
            if inst in self.position:
                ...
                self.position[inst] = {'long': {'yesvol': 0, 'volume': 0, 'poscost': 0},
                                       'short': {'yesvol': 0, 'volume': 0, 'poscost': 0}}
    ...
    def checkCrossDay(self):
        ...
        if self.lasttday != '' and self.tradingday != self.lasttday:
            return True
                ...
                self.lasttime.hour == 14 or self.lasttime.hour == 15) and self.curtime.hour != 14 and self.curtime.hour != 15:
            return True
        return False

    def dayStatistics(self):
        logger.info(
            f'{self.stratName}{self.curday}: orders-{self.stat.dayorders},cancels-{self.stat.daycancels},opens-{self.stat.dayopens},closes-{self.stat.daycloses}')
        logger.info(f"{self.stratName}: totalpnl-" + str(round(self.stat.totalpnl, 2)) + ",totalfees-" + str(
            round(self.stat.totalfees, 2)) + \
                    ',balance-' + str(round(self.stat.balance, 2)) + ',accumlated return-' + str(
            round(100 * self.stat.accret, 2)) + '%')

    def totalStatistics(self):
        logger.info(f"Total statistics of {self.stratName}:")
        logger.info(f"{self.stratName} totalpnl-" + str(round(self.stat.totalpnl, 2)) + ",totalfees-" + str(
            round(self.stat.totalfees, 2), ) + ',maxmargin-' + \
    ...
    def updateFrozenMarg(self):
        ...
        for i in range(len(self.instid)):
            if self.longpendvol.get(self.instid[i]) and self.shortpendvol.get(self.instid[i]):
                self.frozenMarg += self.getMargin(self.longpendvol[self.instid[i]]['poscost'], True,
                                                  self.longpendvol[self.instid[i]]['volume'],
                                                  self.instid[i]) + self.getMargin(
            ...
            if direction > 0:
                ...
                if volume > 0 and action != 'close':
                    if self.position[instid]['short']['volume'] == 0:
                        ...
                        self.position[instid]['short']['poscost'] = getAvgCost(
    ...
    def getRelativeClosePnl(self, instid, price, dirstr, vol, isToday, presett):
        ...
        if self.recmode:
            logger.info(
                f'closeProf: {volMult}, {self.position[instid]}, {dirstr},{price}, {vol}, {poscost} {self.stratName}')
        if dirstr == 'long':
    ...
    def simuTrade(self, trade):
        ...
        try:
            ...
            if action == 'open':
                ...
                if tradeinstid in self.position:
                    if dirstr in self.position[tradeinstid]:
                        ...
                        self.position[tradeinstid][dirstr]['poscost'], self.position[tradeinstid][dirstr]['volume'] = \
                            updatePos(self.position[tradeinstid][dirstr]['poscost'],
                                      self.position[tradeinstid][dirstr]['volume'], \
                                      tradeprice, tradevol)
                    else:
                        #print(2)
                ...
                closePnl = self.getRelativeClosePnl(tradeinstid, tradeprice, dirstr, tvol - yvol, True,
                                                    trade.get('presett', 0))
                closePnl += self.getRelativeClosePnl(tradeinstid, tradeprice, dirstr, yvol, False,
                                                     trade.get('presett', 0))
