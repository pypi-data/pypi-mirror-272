...
class DataPipeLine:
    ...
    def run_presto_to_df(
        ...
        # verbose
        start = perf_counter()
        logger.info(f"{self.status} [Start] Query, Count: {self.count_rows}")
        if verbose:
        ...
        message = f"{self.status} [Finish] Time: {str(duration).split('.')[0]}, "
        if self.count_rows:
            message += f"Count rows: {df.to_numpy()[0][0]:,.0f} "
        else:
