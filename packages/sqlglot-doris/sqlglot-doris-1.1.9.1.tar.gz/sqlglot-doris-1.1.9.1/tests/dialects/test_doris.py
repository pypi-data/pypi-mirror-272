...
class TestDoris(Validator):
    ...
    def test_time(self):
        ...
        self.validate_all(
            "DATE_ADD(TO_DATE('2018-01-01'), INTERVAL '3' YEAR)",
            read={
                "clickhouse": "addDate(toDate('2018-01-01'), INTERVAL 3 YEAR)",
            },
        )
        self.validate_all(
    ...
    def test_rewrite_rule(self):
        ...
        self.validate_all(
            "SELECT CAST(SUM(bytes) AS STRING), SUM(bytes) AS s, t FROM (SELECT MIN(CASE WHEN xeventid = '1234' THEN phone_time ELSE NULL END) AS start_event_mint, MAX(MIN(CASE WHEN xeventid = '1234' THEN phone_time ELSE NULL END)) AS m) GROUP BY t, SUM(bytes) ORDER BY SUM(bytes)",
            read={
        ...
        self.validate_all(
            "SELECT computeId, EXPLODE(split_events) AS event_chain_, ARRAY_SLICE(ARRAY_COMPACT(EXPLODE(split_events).2), 1, 10) AS event_chain",
            read={
                "clickhouse": "select computeId, arrayJoin(split_events) as event_chain_,arraySlice(arrayCompact(event_chain_.2), 1, 10) as event_chain",
            },
        )
        self.validate_all(
    ...
    def test_mysql2doris_create(self):
        ...
        self.validate_all(
            'CREATE TABLE IF NOT EXISTS t2 (a INT NOT NULL)\nUNIQUE KEY(`a`)\nDISTRIBUTED BY HASH(`a`) BUCKETS AUTO\nPROPERTIES (\n    "replication_allocation" = "tag.location.default: 3"\n)',
            read={
        ...
        self.validate_all(
            'CREATE TABLE B (pk INT, int_key INT NOT NULL)\nUNIQUE KEY(`pk`)\nDISTRIBUTED BY HASH(`pk`) BUCKETS AUTO\nPROPERTIES (\n    "replication_allocation" = "tag.location.default: 3"\n)',
            read={
    ...
    def test_clickhouse2doris_create(self):
        self.maxDiff = None
        self.validate_all(
            'CREATE TABLE dwd_log_pub_behavior_analysis_v3_di_local (`app_name` VARCHAR, `_sys_insert_time` DATETIME)\nDUPLICATE KEY(`app_name`)\nDISTRIBUTED BY HASH(`app_name`) BUCKETS AUTO\nPROPERTIES (\n    "replication_allocation" = "tag.location.default: 3"\n)',
            read={
                "clickhouse": "CREATE TABLE dwd_log_pub_behavior_analysis_v3_di_local ( `app_name` LowCardinality(String), `_sys_insert_time` DateTime MATERIALIZED now(), INDEX index_clientid clientid TYPE bloom_filter GRANULARITY 3 ) PARTITION BY dt ORDER BY (eventid, app_name, city_id, hour) TTL _sys_insert_time + toIntervalDay(31)"
            },
        )
        ...
        self.validate_all(
            "CREATE TABLE block_chain.bc_playrecord_month_analysis_local (`companycode` BIGINT COMMENT '商家号', `encryptcode` STRING COMMENT '加密商家号', `songid` STRING COMMENT '歌曲id', `playnum` SMALLINT COMMENT '点播次数', `hashdir` STRING COMMENT 'hash地址', `playmonth` DATE COMMENT '日期分区键')\nDUPLICATE KEY(`companycode`)\nDISTRIBUTED BY HASH(`companycode`) BUCKETS AUTO\nPROPERTIES (\n    \"replication_allocation\" = \"tag.location.default: 3\"\n)",
            read={
    ...
    def test_oracle2doris_create(self):
        ...
        #     read={
        #         "oracle": "create table AA/B (id INTEGER, name number(10))",
        #     },
        # )
