...
class Functions(Parameters):
    ...
    def get_data_resource(self, key: str):
        ...
        if value is None:
            self.LoggingFeature(f"{Functions.color_message('YELLOW', 'ADVERTENCIA:')} La clave {key} consultada no existe en el diccionario de resources.").send_log()
    ...
    def set_data_cache(self, key: str, value: Any):
        ...
        self.LoggingFeature(f"{Functions.color_message('BLUE', 'Dato guardado en caché')}: key: {key} -> valor: {value}").send_log()
    ...
    class LoggingFeature():
        def __init__(self, mensaje, enable_logging=True):
