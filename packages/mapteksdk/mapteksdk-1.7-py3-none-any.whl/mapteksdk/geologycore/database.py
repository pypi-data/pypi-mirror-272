...
from ..capi import DrillholeModel, DataEngine
...
class DrillholeDatabase(VisualContainer, TablesMixin):
  ...
  def _save(self):
    ...
    if DataEngine().version > (1, 8):
      DataEngine().Checkpoint(self._lock.lock)
    else:
  ...
  @property
  def holes(self) -> typing.Sequence[ObjectID[Drillhole]]:
