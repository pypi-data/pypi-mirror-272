...
from ..capi.util import (register_dll_directory, disable_dll_loading,
                         CApiDllLoadFailureError, CApiFunctionNotSupportedError)
from ..data import (Surface, EdgeNetwork, Marker, Text2D, Text3D, Container,
                    ...
                    Ellipsoid, FilledPolygon, ChangeReasons)
...
class Project:
  ...
  def __init__(
      self,
      options: typing.Optional[ProjectOptions]=None,
  ...
  @contextmanager
  def undo(self, _send: bool=True) -> Generator[UndoStack, None, None]:
    ...
    undo_stack = UndoStack(_can_send=_send)
  ...
  def add_objects(
      ...
      new_objects: typing.Iterable[tuple[str, ObjectID[DataObject]]] |
    ...
    for _, class_type in self.__types_for_data:
      try:
        ...
        if capi.DataEngine().TypeIsA(object_type, type_index):
          return class_type
      except (CApiDllLoadFailureError, CApiFunctionNotSupportedError):
  ...
  def set_selected(
      ...
      object_ids_or_paths:
        typing.Optional[
          Selection |
          typing.Sequence[str | ObjectID[DataObject] | DataObject] |
          str |
  ...
  def recycle_bin_id(self) -> ObjectID:
    ...
    if capi.DataEngine().version >= (1, 8):
      recycle_bin = ObjectID(capi.DataEngine().RecycleBin(self.backend.index))
    else:
  ...
  def project_path(self) -> pathlib.Path:
    ...
    """
    path_string = capi.DataEngine().ProjectPath(self.backend.index)
    if path_string:
  ...
  def _get_before_state(
    ...
    if (oid.is_a((VisualContainer, StandardContainer))
      ...
      with self.read(oid) as container:
        ...
        return ObjectID(
          capi.DataEngine().ShallowCloneObject(container._lock.lock)
          ), primary_children
  ...
  def _run_with_undo(
      ...
      function_to_run: typing.Callable[[WriteLock], None]):
    ...
    with WriteLock(oid.handle, rollback_on_error=True) as w_lock:
      function_to_run(w_lock)
      change_reasons = ChangeReasons(capi.DataEngine().Checkpoint(w_lock.lock))
