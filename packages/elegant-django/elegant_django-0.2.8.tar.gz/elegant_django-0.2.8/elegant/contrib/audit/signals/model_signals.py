...
from ..settings import CRUD_DIFFERENCE_CALLBACKS
from ..settings import DATABASE_ALIAS
from ..settings import LOGGING_BACKEND
from ..settings import REGISTERED_CLASSES
from ..settings import UNREGISTERED_CLASSES
from ..settings import WATCH_MODEL_EVENTS
from ..utils import get_m2m_field_name
from ..utils import model_delta
from ..utils import should_propagate_exceptions
    ...
    try:
        ...
        with transaction.atomic(using=using):
            ...
            if create_crud_event and not created:
                ...
                def crud_flow():
                    try:
                        # atomicity based on the audit database alias
                        ...
                        try:
                            logger.exception(
                                f"easy audit had a pre_save exception on CRUDEvent creation. "
                                f"instance: {instance}, instance pk: {instance.pk}"
                            )
                        except Exception:
                            pass
            ...
            if create_crud_event and created:
                ...
                def crud_flow():
                    try:
                        ...
                        try:
                            logger.exception(
                                f"easy audit had a post_save exception on CRUDEvent creation. "
                                f"instance: {instance}, instance pk: {instance.pk}"
                            )
                        except Exception:
                            pass
        ...
        with transaction.atomic(using=using):
            ...
            def crud_flow():
                try:
                    ...
                    try:
                        logger.exception(
                            f"easy audit had a m2m_changed exception on CRUDEvent creation. "
                            f"instance: {instance}, instance pk: {instance.pk}"
                        )
                    except Exception:
                        pass
        ...
        with transaction.atomic(using=using):
            ...
            def crud_flow():
                try:
                    ...
                    try:
                        logger.exception(
                            f"easy audit had a post_delete exception on CRUDEvent creation. "
                            f"instance: {instance}, instance pk: {instance.pk}"
                        )
                    except Exception:
                        pass
