...
class PEDANT:
    ...
    def get_score(self, reference, candidate, question):
        ...
        curr_input_text = "[CLS] " + candidate + " [SEP] " + reference + " [SEP] " + remove_punctuation(normalize_answer(str(question))) + " [SEP]"
    ...
    def get_scores(self, reference, candidate, question):
        ...
        if isinstance(reference, list) and isinstance(candidate, list):
            ...
            for reference in references:
                ...
                confidece_scores[references[i]][candidate] = score
            ...
            input_texts, tagged_texts = [], []
            f1_scores, precisions, recalls = [], [], []
            for candidate in candidates:
                curr_input_text = "[CLS] " + remove_punctuation(normalize_answer(str(candidate))) + " [SEP] " + remove_punctuation(normalize_answer(str(reference))) + " [SEP] " + remove_punctuation(normalize_answer(str(question))) + " [SEP]"
                input_texts.append(curr_input_text)
                f, p, r = calculate_f1_score_with_precision(str(candidate), str(reference))
                f1_scores.append(f)
                precisions.append(p)
