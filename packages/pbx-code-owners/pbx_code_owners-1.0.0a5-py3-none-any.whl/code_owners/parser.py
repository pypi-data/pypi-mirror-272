...
def notify_owner(owner: str, section_name: str) -> None:
    content = f'Section "{section_name}" requires {owner} approval'
...
def main() -> None:
    ...
    for section in codeowners["sections"]:
        if "." in section["paths"] or match_any_path(changed_paths, section["paths"]):
            for owner in section["owners"]:
                notify_owner(owner, section["name"])
