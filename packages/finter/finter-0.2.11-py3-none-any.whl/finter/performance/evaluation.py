...
def get_bench_cm(bench):
    ...
    if upper_bench in ['CSI300', 'DAX', 'DJIA', 'HANGSENG', 'KOSDAQ', 'KOSPI', 'KOSPI200',
                       'KOSPI_LARGECAP', 'KOSPI_MIDCAP', 'KOSPI_SMALLCAP', 'MSCI_US_REIT',
                       'NASDAQ100', 'NASDAQ_COMPOSITE', 'S&P500', 'S&P_GSCI', 'S&P_GSCI_CRUDE',
                       'S&P_GSCI_PM', 'STOXX50', 'STOXX600', 'US_DOLLAR', 'VIX']:
        bench_cm = 'content.factset.api.price_volume.world-index-price_close.1d'
    elif upper_bench in ['CANADA_S_P_TSX', 'DOWJONES_INDUSTRIAL', 'EURO_STOXX_50',
                         'FTSE_100_INDEX', 'HANG_SENG_CHINA_ENTERPRISES_INDEX',
                         'HANG_SENG_INDEX', 'HO_CHI_MINH_STOCK_INDEX', 'KOREA_KOSPI',
                         'NASDAQ_100', 'NASDAQ_COMPOSITE', 'NIKKEI_225_INDEX',
                         'PHLX_SEMICONDUCTOR_S', 'RUSSELL_2000', 'S&P_500',
                         'SHANGHAI_COMPOSITE_INDEX', 'US_DOLLAR_INDEX']:
        ...
        raise ValueError("Not supported benchmark! If you want to add it, please contact maintainer")
...
def plot_with_plotly(data, model_name):
    ...
    for series_name, series_data in data.items():
        if series_name == model_name:
            ...
            line_width = 1  
        fig.add_trace(go.Scatter(x=series_data.index, y=series_data, mode='lines', name=series_name.split('.')[-1],
...
def plot_with_matplotlib(data, model_name):
    ...
    for series_name, series_data in data.items():
        if series_name == model_name:
            ...
            line_width = 1  
        plt.plot(series_data.index, series_data, label=series_name.split('.')[-1], linewidth=line_width)
    plt.axhline(0, color='grey', linestyle='--')
    plt.title(f'Comparison {model_name.split('.')[-1]} with benchmark{'s' if len(data.keys()) > 1 else ''}')
...
class Evaluator:
    ...
    @staticmethod
    def compare_with_bm(model: str, bms, start = None, end = None, interest = False, interactive = False):
        """
        ...
        if interactive:
            if not module_exists('plotly'):
                raise ImportError("Plotly is not installed. Install it by running 'pip install plotly'.")
            import plotly.graph_objects as go
        else:
            if not module_exists('matplotlib'):
                raise ImportError("Matplotlib is not installed. Install it by running 'pip install matplotlib'.")
            import matplotlib.pyplot as plt
        ...
        bms = [bm.upper() if '.' not in bm else bm for bm in bms ]  # Convert all benchmark identifiers to upper case
        ...
        start, end = Evaluator._parse_edge_dates(start, end)
        start_dates = [cum_model_ret.ne(0).idxmax()] + [benchmarks[bm].ne(0).idxmax() for bm in bms]
    ...
    @staticmethod
    def get_cum_ret(model: str, interest):
        """
        ...
        position = position.loc[:, abs(position).sum() != 0]
        if all(isinstance(col, str) for col in position.columns) and not model_info[2] == "compustat":
            position.columns = position.columns.astype(int)
