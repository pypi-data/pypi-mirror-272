    ...
    def get_agent(agent_id):
        with Session(dbmanager.engine) as session:
            agent: Agent = dbmanager.get_items(
                Agent, filters={"id": agent_id}, session=session).data[0]
            ...
            models = [model.model_dump(
                mode="json", exclude=model_exclude) for model in agent.models]
            ...
            agent_dict["agents"] = [
                get_agent(agent.id) for agent in agent.agents]
            return agent_dict
...
def init_db_samples(dbmanager: Any):
    ...
    if "Default Workflow" in workflow_names and "Travel Planning Workflow" in workflow_names:
        ...
        return
    logger.info(
        "Initializing database with Default and Travel Planning Workflows")
    # models
    gpt_4_model = Model(
    ...
    default_workflow = Workflow(
        name="Default Workflow", description="Default workflow", user_id="guestuser@gmail.com")
    ...
    with Session(dbmanager.engine) as session:
        ...
        dbmanager.link(link_type="agent_model",
                       primary_id=painter_assistant.id, secondary_id=gpt_4_model.id)
        dbmanager.link(link_type="agent_skill", primary_id=painter_assistant.id,
        ...
        dbmanager.link(link_type="agent_agent",
                       primary_id=travel_groupchat_agent.id, secondary_id=planner_assistant.id)
        dbmanager.link(link_type="agent_agent",
                       primary_id=travel_groupchat_agent.id, secondary_id=local_assistant.id)
        dbmanager.link(
            link_type="agent_agent", primary_id=travel_groupchat_agent.id, secondary_id=language_assistant.id
        )
        dbmanager.link(link_type="agent_agent",
                       primary_id=travel_groupchat_agent.id, secondary_id=user_proxy.id)
        dbmanager.link(link_type="agent_model",
                       primary_id=travel_groupchat_agent.id, secondary_id=gpt_4_model.id)
        dbmanager.link(link_type="agent_model",
                       primary_id=planner_assistant.id, secondary_id=gpt_4_model.id)
        dbmanager.link(link_type="agent_model",
                       primary_id=local_assistant.id, secondary_id=gpt_4_model.id)
        dbmanager.link(link_type="agent_model",
                       primary_id=language_assistant.id, secondary_id=gpt_4_model.id)
