...
valid_link_types = ["agent_model", "agent_skill",
                    "agent_agent", "workflow_agent"]
...
class DBManager:
    ...
    def __init__(self, engine_uri: str):
        ...
        self.engine = create_engine(
            engine_uri, connect_args=connection_args)
    ...
    def get_items(
        ...
        try:
            ...
            if return_json:
                result = [self._model_to_dict(row)
    ...
    def get(
        ...
        with Session(self.engine) as session:
            response = self.get_items(
                model_class, session, filters, return_json, order)
        return response
            ...
            try:
                if link_type == "agent_model":
                    ...
                    agent = self.get_items(
                        Agent, filters={"id": primary_id}, session=session).data[0]
                ...
                status = False
            if return_json:
                linked_entities = [self._model_to_dict(
                    row) for row in linked_entities]
                ...
                try:
                    if link_type == "agent_model":
                        primary_model = session.exec(
                            select(Agent).where(Agent.id == primary_id)).first()
                        secondary_model = session.exec(
                            select(Model).where(Model.id == secondary_id)).first()
                        ...
                        primary_model = session.exec(
                            select(Agent).where(Agent.id == primary_id)).first()
                        secondary_model = session.exec(
                            select(Agent).where(Agent.id == secondary_id)).first()
                        if primary_model is None or secondary_model is None:
                            status = False
                            status_message = "One or both entity records do not exist."
                        else:
                        ...
                        primary_model = session.exec(
                            select(Agent).where(Agent.id == primary_id)).first()
                        secondary_model = session.exec(
                            select(Skill).where(Skill.id == secondary_id)).first()
                        if primary_model is None or secondary_model is None:
                            status = False
                            status_message = "One or both entity records do not exist."
                        else:
                        ...
                        primary_model = session.exec(
                            select(Workflow).where(Workflow.id == primary_id)).first()
                        secondary_model = session.exec(
                            select(Agent).where(Agent.id == secondary_id)).first()
                        if primary_model is None or secondary_model is None:
                            status = False
