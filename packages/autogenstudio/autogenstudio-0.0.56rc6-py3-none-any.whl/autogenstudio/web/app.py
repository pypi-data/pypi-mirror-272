...
high_priority_loop = asyncio.new_event_loop()


def run_high_priority_loop():
    asyncio.set_event_loop(high_priority_loop)
    high_priority_loop.run_forever()


high_priority_thread = threading.Thread(
    target=run_high_priority_loop, daemon=True)
    ...
    while True:
        ...
        for connection, socket_client_id in websocket_manager.active_connections:
            if message["connection_id"] == socket_client_id:
                print(
                    f"**** Sending message from queue to client {socket_client_id} ***** ")
                # asyncio.run(websocket_manager.send_message(
                #     message, connection))
                future: Future = asyncio.run_coroutine_threadsafe(
                    websocket_manager.send_message(
                        message, connection), high_priority_loop
                )
                future.result()
