...
def message_handler():
    while True:
        ...
        message_connection_exists = message["connection_id"] in active_connection_ids
        if not message_connection_exists:
            logger.error(
                f"Connection ID: {message['connection_id']} not found in active connections: {active_connection_ids}"
            )
