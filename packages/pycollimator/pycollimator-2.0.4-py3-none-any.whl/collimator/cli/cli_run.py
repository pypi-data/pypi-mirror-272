...
@click.option(
    ...
    show_default=True,
    help="Directory which contains all model.json & submodel.json files",
)
@click.option(
    "--model",
    default="model.json",
...
def collimator_run(
    ...
    click.echo(
        "collimator run inputs:\n"
        "\tcwd={}\n\tmodeldir={}\n\tmodel={}\n\tsignalsdir={}\n\tlogsdir={}".format(
            cwd, modeldir, model, signalsdir, logsdir
        )
    )
    ...
    loaded_model = collimator.load_model(
        modeldir, model=model, npydir=signalsdir, logsdir=logsdir
    )

    if end_time is not None:
        loaded_model.model.configuration.stop_time = float(end_time)

    _ = loaded_model.simulate()
