...
class BDFSolver(ODESolverImpl):
    ...
    def _initialize_state(self, func, t0, xc0, unravel, *args, dt=None):
        ...
        M = jnp.eye(len(xc0), dtype=xc0.dtype)
    ...
    def solve_newton_system(self, func, t, y, c, psi, LU, scale):
        ...
        def _body_fun(carry):
            ...
            pred_residual = rate ** (NEWTON_MAXITER - k) / (1 - rate) * dy_norm
            exit_flag = jnp.isfinite(rate) & ((rate >= 1.0) | (pred_residual > tol))

            y = jnp.where(exit_flag, y, y + dy)
            d = jnp.where(exit_flag, d, d + dy)

            converged = (dy_norm == 0.0) | (
                jnp.isfinite(rate) & (rate / (1 - rate) * dy_norm < tol)
            )

            dy_norm_old = dy_norm
            k += 1
            exit_flag = exit_flag | (k >= NEWTON_MAXITER) | converged
    ...
    def newton_iteration(self, state, func, boundary_time):
        ...
        def _body_fun(carry):
            ...
            converged, n_iter, y_new, d = self.solve_newton_system(
                func, t_new, y_predict, c, psi, LU, scale
            )
    ...
    def attempt_bdf_step(self, func, boundary_time, carry):
        ...
        error = ERROR_CONST[state.order] * d
