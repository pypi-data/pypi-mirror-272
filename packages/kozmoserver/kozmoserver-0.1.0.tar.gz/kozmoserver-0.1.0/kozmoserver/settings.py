...
ENV_PREFIX_SETTINGS = "KOZMOSERVER_"
ENV_PREFIX_MODEL_SETTINGS = "KOZMOSERVER_MODEL_"
...
class Settings(BaseSettings):
    ...
    """Grace timeout to wait until the workers shut down when stopping KozmoServer."""
    ...
    ``kozmoserver.repository.repository.SchemalessModelRepository``)."""

    # Model repository settings
    model_repository_root: str = "."
    """Root of the model repository, where we will search for models."""

    # Model Repository parameters are meant to be set directly by the KozmoServer runtime.
    ...
    kafka_topic_input: str = "kozmoserver-input"
    kafka_topic_output: str = "kozmoserver-output"
...
class ModelParameters(BaseSettings):
    ...
    This path must be either absolute or relative to where KozmoServer is running.
    """
...
class ModelSettings(BaseSettings):
    ...
    implementation_: str = Field(
        alias="implementation", env="KOZMOSERVER_MODEL_IMPLEMENTATION"
    )
    """*Python path* to the inference runtime to use to serve this model (e.g.
    ``kozmoserver_sklearn.SKLearnModel``)."""

    # Model parameters are meant to be set directly by the KozmoServer runtime.
