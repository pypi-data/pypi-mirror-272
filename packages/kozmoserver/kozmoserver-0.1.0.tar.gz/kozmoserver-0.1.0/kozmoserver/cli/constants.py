DefaultBaseImage = "kozmoai/kozmoserver:{version}-slim"
...
ARG KOZMOSERVER_ENV_NAME="kozmoserver-custom-env" \\
    KOZMOSERVER_ENV_TARBALL="./envs/base.tar.gz"
...
COPY \\
    ...
    ./conda.yam[l] \\
    .
RUN mkdir $(dirname $KOZMOSERVER_ENV_TARBALL); \\
    for envFile in environment.yml environment.yaml conda.yml conda.yaml; do \\
        if [[ -f $envFile ]]; then \\
            conda env create \
                --name $KOZMOSERVER_ENV_NAME \\
                --file $envFile; \\
            conda-pack --ignore-missing-files \
                -n $KOZMOSERVER_ENV_NAME \\
                -o $KOZMOSERVER_ENV_TARBALL; \\
        fi \\
    done; \\
    chmod -R 776 $(dirname $KOZMOSERVER_ENV_TARBALL)
...
# Override KozmoServer's own `CMD` to activate the embedded environment
# (optionally activating the hot-loaded one as well).
CMD source ./hack/activate-env.sh ./envs/base.tar.gz && \\
    kozmoserver start $KOZMOSERVER_MODELS_DIR
"""
