...
'''
5.visualize_grid_cells(image_path, W, H, X, Y)
将图像划分的grid cell进行可视化
image_path：图像的路径
W:设置图像的宽
H：设置图像的高
X、Y：设置grid cell的数量为X×Y个
'''
def visualize_grid_cells(image_path, W, H, X, Y):
    # 读取图像
    ...
    # 可视化grid cell
    # 创建一个与原图同样大小的空图像用于绘制grid cell
    grid_image = np.zeros_like(resized_image, dtype=np.uint8)
    ...
    for i in range(X):
        for j in range(Y):
            ...
            # 计算grid cell的右下角坐标
            end_x = (i + 1) * cell_width
            end_y = (j + 1) * cell_height

            # 绘制grid cell边界
            cv2.rectangle(grid_image, (start_x, start_y), (end_x, end_y), (0, 255, 0), 2)
    ...
    axs[0].axis('off')
    axs[1].imshow(cv2.cvtColor(grid_image, cv2.COLOR_BGR2RGB))
    axs[1].set_title('Grid Cells')
    axs[1].axis('off')
    plt.show()
