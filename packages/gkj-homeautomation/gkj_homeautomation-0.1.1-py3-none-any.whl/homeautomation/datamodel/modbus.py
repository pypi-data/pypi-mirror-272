...
import homeautomation.kostal_plenticore as kp
...
class CDevice:
    ...
    def set_register(self):
        if self.device_type == CDeviceType.INVERTER:
            return read_register_values_from_csv(in_str = kp.get_register_str())
        return []
...
def read_register_values_from_csv(in_str: str) -> list[CRegister]:
    r_table = []
    lines = in_str.split('\n')
    # print(lines)
    for line in lines[:-1]:
        l_values=line.strip().split(';')
        if l_values:
            r_val = CRegister(
