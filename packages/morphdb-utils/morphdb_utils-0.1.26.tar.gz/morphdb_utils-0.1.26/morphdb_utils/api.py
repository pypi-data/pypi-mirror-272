...
def _load_file_data_impl(
    ...
    try:
        response = requests.get(url_sql, headers=headers, timeout=10)
    except Timeout:
...
def _execute_sql_impl(
    ...
    try:
        response = requests.post(url_sql, headers=headers, json=request, verify=True, timeout=10)
    except Timeout:
...
def generate_report(
    ...
    try:
        response = requests.post(url, headers=headers, json=request, verify=True, timeout=29)
    except Timeout:
