...
def get_iterator_outputs(output, handler):
    """
    ...
    def process(iterator_output_result):
        handler.postprocess(iterator_output_result)
        ...
        processed_output = worker_pb2.PredictionResponse(
            data=encoded_output, status=worker_pb2.Status.STATUS_SUCCEEDED, stop=False
        )
    ...
    yield worker_pb2.PredictionResponse(
        data=b"", status=worker_pb2.Status.STATUS_SUCCEEDED, stop=True
    )
