...
import sys
from ..handling.file_handling import Handler, get_global_handler
...
class PredictionRunner:
    ...
    def __init__(
        ...
        self.file_handler = get_global_handler()
        ...
        try:
            with run_lock:
                ...
                with StdoutCapturing(self.metadata), StderrCapturing(self.metadata):
                    ...
                    # Postprocess output
                    if self.iter_output:
                        o = get_iterator_outputs(output, self.file_handler)
                        for i in o:
                            yield i
                    else:
                        yield get_single_output(output, self.file_handler)
