    ...
    def __init__(
        ...
        if path_or_url is not None:
            ...
            if isinstance(path_or_url, Path):
                super().__init__(_path=str(path_or_url))
            else:
                try:
                    TypeAdapter(HttpUrl).validate_python(path_or_url)
                    super().__init__(url=path_or_url)
                except ValidationError:
                    super().__init__(_path=path_or_url)
        else:
            if path and url:
                raise ValueError("Please provide either path or url, not both")
            super().__init__(_path=path, url=url)
