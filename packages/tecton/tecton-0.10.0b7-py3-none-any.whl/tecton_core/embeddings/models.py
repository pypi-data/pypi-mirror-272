...
class ModelContainer:
    ...
    # Not currently in use. For future use in adding speculative parameters for tokenization
    ...
    def load(cls, filepath: str, device_name: str) -> ModelContainer:
        with torch.device(device_name):
            return cls(
                ...
                # TODO: Extract token limits and model load args from the config
                # config=PretrainedConfig.from_pretrained(filepath, local_files_only=True, trust_remote_code=False),
            )
...
def _embed_pytorch_core_logic(
    *,
    ...
    encoded_input = tokenizer(
        strings, padding=True, truncation=True, return_tensors="pt", max_length=_MAX_TOKENIZATION_LENGTH
    )
    with torch.no_grad():
...
def _embed_pytorch_core_logic_with_bisect(
    *,
    ...
    return torch.cat(
        [
            _embed_pytorch_core_logic(strings=first_chunk, tokenizer=tokenizer, model=model),
            _embed_pytorch_core_logic(strings=second_chunk, tokenizer=tokenizer, model=model),
        ]
    )
...
def _embed_pytorch_core_logic_with_retries(
    *,
    ...
    while max_attempts > 0:
        max_attempts -= 1
        try:
            return _embed_pytorch_core_logic(strings=strings, tokenizer=tokenizer, model=model)
    ...
    return _embed_pytorch_core_logic_with_bisect(strings=strings, tokenizer=tokenizer, model=model)
    ...
    *,
    ...
    if null_index == 0:
        ...
        sentence_embeddings = _embed_pytorch_core_logic_with_retries(
            strings=input_strings, tokenizer=tokenizer, model=model
        )
...
def embed_pytorch(
    ...
    *,
    ...
    execution_utils.gc_all(cuda_device)
    return results
