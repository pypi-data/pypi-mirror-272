...
from anyscale.commands.job_commands import status, submit
...
class FakeJobSDK:
    ...
    @property
    def submitted_id(self) -> Optional[str]:
    ...
    def submit(self, config: JobConfig):
        ...
        self._submitted_id = str(uuid.uuid4())
        self._submitted_name = (
            self._submitted_config.name
            if self._submitted_config.name is not None
    ...
    def status(self, name: Optional[str], job_id: Optional[str]):
        ...
        if (
            self._submitted_config is not None
            and self._submitted_name is not None
            and (name == self._submitted_name or job_id == self._submitted_id)
        ):
            return JobStatus(
                id=self._submitted_id if self._submitted_id else "",
                name=self._submitted_name,
                state=JobState.SUCCEEDED,
                config=self._submitted_config,
            )
...
class TestSubmit:
    ...
    def test_follow(self, fake_job_sdk, mock_job_controller, inlined_entrypoint: bool):
        ...
        if inlined_entrypoint:
            assert "`--follow` is deprecated, use `--wait` instead." in result.stdout

        assert result.exit_code == 0

        if inlined_entrypoint:
            mock_job_controller.logs.assert_called_once_with(
                fake_job_sdk.submitted_id, should_follow=True
            )
        else:
            mock_job_controller.logs.assert_called_once_with(
                DEFAULT_JOB_ID, should_follow=True
            )
    ...
    def test_wait(self, fake_job_sdk, mock_job_controller, inlined_entrypoint: bool):
        ...
        if inlined_entrypoint:
            mock_job_controller.wait.assert_called_once_with(
                job_id=fake_job_sdk.submitted_id,
            )
        else:
            mock_job_controller.wait.assert_called_once_with(job_id=DEFAULT_JOB_ID,)
