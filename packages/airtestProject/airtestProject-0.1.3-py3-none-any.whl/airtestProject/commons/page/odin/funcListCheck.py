...
class FuncListCheckPage:
    ...
    def _open_ui_from_ui_list(self, ui, ui_view, swipe_point, first_ui, final_ui, fun_name="air"):
        """
        ...
        def __open_ui_from_ui_list():
            ...
            if operate(fun_name).wait_next_element(ui, ui_view) is True:
                ...
                log.log_adb_out(f"从列表打开ui界面报错{adb_log}")
            else:
                air.sleep(3)
                log.info(f"打开{ui}失败")
        ...
        __open_ui_from_ui_list()

    def _open_ui(self, ui, ui_view, fun_name="air"):
        ...
        def __open_ui():
            ...
            if operate(fun_name).wait_next_element(ui, ui_view) is True:
                ...
                log.info(f"成功点击打开{ui}")
                adb_log = self.adb.shell("logcat -d Unity:E *:S")
                log.log_adb_out(f"打开ui界面报错{adb_log}")
            else:
                air.sleep(5)
                log.info(f"打开{ui}失败")
                adb_log = self.adb.shell("logcat -d Unity:W *:S")
    ...
    def _close_ui(self, ui_view, close_pos, fun_name='air'):
        """
        ...
        def __close_ui():
            if operate(fun_name).wait_element_appear(close_pos):
                ...
                if operate(fun_name).wait_disappear_element(ui_view):
                    ...
                    log.log_adb_out(f"关闭ui界面报错{adb_out}")
                    log.info("关闭ui界面")
                else:
    ...
    def _click_ui(self, ui, ocrPlus=None, fun_name='air'):
        ...
        def __click_ui():
            ...
            air.sleep(3)
            adb_out = self.adb.shell("logcat -d Unity:W *:S")
            log.log_adb_out(f"点击ui报错{adb_out}")
            log.info("点击ui")

        __click_ui()

    def _click_ui_from_ui_list(self, ui, swipe_point, first_ui, final_ui, fun_name='air'):
        self.adb.shell("logcat -c")

        @log.wrap(f"从列表点击{ui}")
        def __click_ui_from_ui_list():
            while operate(fun_name).exists(ui) is False:
                operate(fun_name).swipe(swipe_point, vector_direction=[0, -0.2])
                if operate(fun_name).exists(final_ui) is not False:
                    start_time = time.time()
            ...
            air.sleep(3)
            adb_out = self.adb.shell("logcat -d Unity:W *:S")
            log.log_adb_out(f"从列表点击ui报错{adb_out}")
            log.info("点击ui")

        __click_ui_from_ui_list()

    def _open_list(self, list_pos, next_pos, fun_name="air"):
    ...
    def _check_scenes(self, scenes_pos, wait_pos, fun_name="air"):
        self.adb.shell("logcat -c")
        ...
        def __check_scenes():
            ...
            if operate(fun_name).exists(wait_pos):
                ...
                air.sleep(0.5)
                adb_out = self.adb.shell("logcat -d Unity:W *:S")
                log.log_adb_out(f"点击场景出现的报错{adb_out}")
            else:
                log.info("点击无反应，可能是已处于对应场景")
                # air.snapshot(filename="screenshot{}点击无反应的界面.png".format(scenes_pos))
                air.sleep(0.5)
                adb_out = self.adb.shell("logcat -d Unity:W *:S")
                log.log_adb_out(f"点击场景无反应的报错{adb_out}")
                print(adb_out)
                return False

        __check_scenes()

    def _swip_ui(self, swipe_pos, fun_name="air"):
        ...
        def __swip_ui():
            ...
            air.sleep(2)
            adb_out = self.adb.shell("logcat -d Unity:W *:S")
            log.log_adb_out(f"滑动界面报错{adb_out}")
            log.info("滑动界面")

        __swip_ui()

    # TODO: 队列版本后续开发一个识别黑框的会方便点（poco其实不用这么麻烦，我后续想想怎么整合(poco可以知道ui节点也就知道了黑框存在)）
    def _Plot(self, dialogue_ui_list, stop_ui, choice_ui=None, jump_ui=None, start_ui=None, log_path=None,
             fun_name="air"):
        """
        ...
        def __Plot():
            ...
            air.sleep(2)
            adb_out = self.adb.shell("logcat -d Unity:W *:S")
            log.log_adb_out(f"剧情报错{adb_out}")
            log.info("过完剧情")
    ...
    def scenes_jump(self):
        self._open_list("场景", "场景跳转")
        self._check_scenes("斗罗场景", "等待条")
        self._open_list("场景", "场景跳转")
        self._check_scenes("TestShader", "等待条")
        self._open_list("场景", "场景跳转")
        self._check_scenes("斗罗场景 (流式加载)", "等待条")
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("设置", "设置页", "功能列表滑动点", "设置", "测试剧情2")
        self._close_ui("设置页", "关闭设置")

    def hua_zhi_view(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("画质", "画质页", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui("极低")
        self._click_ui("应用")
        self._click_ui("低")
        self._click_ui("应用")
        ...
        self._click_ui("应用")
        self._click_ui("其它")
        self._click_ui("画质", ocrPlus=True)
        self._close_ui("画质页", "返回键")

    def ji_neng_view(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("技能装配UI", "技能界面", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui("技能界面攻击键")
        self._close_ui("技能界面", "返回键")

    def demo_ui_view(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("demoUI", "demoUI界面", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui("点击发事件")
        self._click_ui("列表组件")
        self._click_ui("基础组件")
        self._close_ui("demoUI界面", "返回键")

    def sound_setting_view(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("soudsetingview", "音乐设置界面", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui("使用Wwise", ocrPlus=True)
        self._click_ui("Destroy Audio")
        self._click_ui("使用 UnityAudio")
        self._click_ui("创建机关")
        self._close_ui("音乐设置界面", "关闭音乐界面")

    def DemoRTShowHighQualityShadowView(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("DemoRTShowHighQualityShadowView", "高清阴影模型界面", "功能列表滑动点", "设置",
                                  "测试剧情2")
        self._close_ui("高清阴影模型界面", "返回键")

    def DemoRTShowOrthographicView(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("DemoRTShowOrthographicView", "正交相机模型界面", "功能列表滑动点", "设置",
                                  "测试剧情2")
        self._close_ui("正交相机模型界面", "返回键")

    def DemoRTShowPlanarShadowView(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("DemoRTShowPlanarShadowView", "平面阴影模型界面", "功能列表滑动点", "设置",
                                  "测试剧情2")
        self._close_ui("平面阴影模型界面", "返回键")

    def SRPBatcherProfiler(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("SRPBatcherProfiler", "关闭SRP Batcher", "功能列表滑动点", "设置",
                                  "测试剧情2")
        self._click_ui("关闭SRP Batcher")
        self._click_ui("开启SRP Batcher")
        self._open_list("功能", "功能列表")
        self._close_ui("关闭SRP Batcher", "SRPBatcherProfiler")

    def TestUIMoldeClipView(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("TestUIMoldeClipView", "TestMolde界面", "功能列表滑动点", "设置",
                                  "测试剧情2")
        self._swip_ui("TestMolde滑动点")
        self._click_ui("打开新")
        self._click_ui("TestUIMoldeClipView")
        self._close_ui("TestMolde界面", "TestMolde关闭按钮")

    def xie_yi_view(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("协议工具", "测试发送", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui("恢复监听发送")
        self._click_ui("恢复监听接收")
        self._click_ui("测试发送")
        self._click_ui("清空")
        self._close_ui("测试发送", "协议工具关闭按钮")

    def shi_pin(self):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("视频", "视频界面", "功能列表滑动点", "设置", "测试剧情2")
        self._close_ui("视频界面", "视频界面关闭按钮")

    def ce_shi_ju_qin1(self, log_path):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("测试剧情1", "测试剧情1黑幕", "功能列表滑动点", "设置", "测试剧情2")
        self._Plot(["最近怪物横行，请少侠前往击杀"], "功能", ["挑战Boss"], start_ui="测试剧情1开始", log_path=log_path)

    def ce_shi_ju_qin2(self, log_path):
        self._open_list("功能", "功能列表")
        self._open_ui_from_ui_list("测试剧情2", "测试剧情2黑幕", "功能列表滑动点", "设置", "测试剧情2")
        self._Plot(["最近怪物横行，请少侠前往击杀", "这个小怪有点强，你要不要和我一起去？", "不是吧，小怪还要我帮忙啊！",
                  "两个人一起，不是打的快些嘛", "好吧，陪你走一遭"], stop_ui="功能", start_ui="测试剧情2开始", log_path=log_path)
    ...
    def run_step(self):
        ...
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("reloadall", "功能列表滑动点", "设置", "测试剧情2")

        self.ji_neng_view()

        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("testloadrawdata", "功能列表滑动点", "设置", "测试剧情2")
        ...
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("检查更新", "功能列表滑动点", "设置", "测试剧情2")
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("镜头切换", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("镜头切换", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("镜头切换", "功能列表滑动点", "设置", "测试剧情2")
        ...
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("是否显示ObjID", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("是否显示ObjID", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("记录协议", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("显示ping", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("报错提示界面", "功能列表滑动点", "设置", "测试剧情2")
        self._click_ui_from_ui_list("DamageText Test", "功能列表滑动点", "设置", "测试剧情2")
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("GCTest", "功能列表滑动点", "设置", "测试剧情2")
        self._open_list("功能", "功能列表")
        self._click_ui_from_ui_list("测试报错", "功能列表滑动点", "设置", "测试剧情2")
...
if __name__ == '__main__':
    ...
    # output_html_path = r"G:\pyProject\odin-testautomation\TestAutomation\airtestProject\commons\page\odin\log\log.html"
