...
class NeptuneManager:
    ...
    def log_dataframe(self, 
        ...
        if df_format:
            NeptuneManager.project[namespace][df_name].upload(File.as_html(dataframe))
        if csv_format:
            # create the temporary folder if it doesn't exist
            csv_buffer = StringIO()
            dataframe.to_csv(csv_buffer, **to_csv_kwargs)
            NeptuneManager.project[namespace][df_name].upload(File.from_stream(csv_buffer, extension="csv"))
        ...
        if profile_report_name is not None and profile_report_title is not None:
            ...
            NeptuneManager.project[namespace][profile_report_name].upload(
