...
class Session:
    ...
    def process_event_trace(self, session_id: str, request_body: operations.ProcessEventTraceRequestBody) -> operations.ProcessEventTraceResponse:
        r"""Process an event trace for a given session"""
        hook_ctx = HookContext(operation_id='processEventTrace', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        request = operations.ProcessEventTraceRequest(
            session_id=session_id,
        ...
        url = utils.generate_url(base_url, '/session/{session_id}/traces', request)
        ...
        res = operations.ProcessEventTraceResponse(status_code=http_res.status_code, content_type=http_res.headers.get('Content-Type') or '', raw_response=http_res)
