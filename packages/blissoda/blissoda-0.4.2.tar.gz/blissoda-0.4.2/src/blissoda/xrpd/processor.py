...
class XrpdProcessor(
    ...
    def ensure_workflow_accessible(self, scan) -> None:
        ...
        if wfilename.startswith("/data/visitor/") or "inhouse" in wfilename:
            return  # visitor or inhouse folder
        dataset_filename = self.get_filename(scan)
        root = self._get_accessible_workflows_dir(dataset_filename)
        os.makedirs(root, exist_ok=True)
        dest = os.path.join(root, os.path.basename(wfilename))
        try:
            shutil.copy(wfilename, dest)
        except shutil.SameFileError:
            pass
        self.set_workflow(scan, dest)
