...
# Cleans / Removes about 60% of the content
...
def content_type_is_text(content_type) -> bool:
    """
...
def crawl_website(url: str, how_many_pages = 30, memory_store = None) -> dict:
    """
    Crawl website starting from a given URL. Stores data in-memory. 
    
    Returns the dictionnary with all the content.
    """
    submitted = 0
    if not memory_store:
...
def remove_reviews(soup : BeautifulSoup) -> BeautifulSoup:
    """
    Slightly risky function which removes the reviews from a Shopify store (for now - add more cases)
    """
...
def get_local_domain(from_url) -> str:
    """
