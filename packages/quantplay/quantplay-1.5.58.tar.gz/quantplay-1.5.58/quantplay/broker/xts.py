...
class XTS(Broker):
    ...
    def margins(self):
        ...
        if (
            ...
            if "description" in data and "max limit" in data["description"].lower():
                time.sleep(1)
                raise RetryableException("Rate Limit Exceeded")
    ...
    def positions(self):
        ...
        if "type" in api_response and api_response["type"] == "error":
            if (
                "description" in api_response
                and "authorization" in api_response["description"].lower()
            ):
                raise TokenException(f"[XTS_Error] {api_response['description']}")
            raise Exception("[XTS_Error]: " + api_response["description"])
