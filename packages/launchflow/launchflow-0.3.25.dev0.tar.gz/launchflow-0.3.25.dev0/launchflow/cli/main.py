...
async def create(
    ...
    local_mode: bool = typer.Option(False, "--local", help="Create only local resources."),
    ...
    if local_mode:
        local_resources = [resource for resource in resources if is_local_resource(resource)]

        if local_resources:
            await create_resources("local", "local", *local_resources, prompt=not auto_approve, api_key=api_key)
        else:
    ...
    try:
        async with async_launchflow_client_ctx(api_key=api_key) as client:
            ...
            remote_resources = [resource for resource in resources if not is_local_resource(resource)]
...
async def destroy(
    project: Optional[str] = typer.Option(None, help=PROJECT_HELP),
    environment: Optional[str] = typer.Option(None, help=ENVIRONMENT_HELP),
    local_mode: bool = typer.Option(False, "--local", help="Only destroy local resources."),
):
...
async def promote(
    ...
    async with async_launchflow_client_ctx(api_key=api_key) as client:
        try:
            ...
            launchflow.environment = to_environment_info.name
            service_infos = _get_service_infos(service)
            services: List[Service] = []
            service_names = set()
            # Load services from config
