...
class SupabaseCache:
    ...
    def set(self, key, value, expire=None, tag=None):
        ...
        exists_id = exists_key.data[0]["id"] if exists_key.data else None
        if exists_id:
            data["id"] = exists_id
            response = self.supabase.table("cache_entries").upsert(data).execute()

        else:
            response = self.supabase.table("cache_entries").insert(data).execute()
        logger.info(f"Set cache response: {response}")
        # print(response)
        return True
        # if exists_key.data:
        #     data.pop("store_time")
        #     data.pop("access_time")
        #     data.pop("access_count")
        #     response = self.supabase.table("cache_entries").update(data).eq(
        #         "key", serialized_key
        #     ).execute()
        # else:
    ...
    def delete(self, key):
        ...
        if response.count < 1:
            raise Exception(f"Failed to delete cache entry: {serialized_key}")
