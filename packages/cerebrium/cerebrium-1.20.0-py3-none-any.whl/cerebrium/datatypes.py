...
class Hardware:
    ...
    def get_provider(self, name: str) -> ProviderOption:
        """Check if there's a provider with the given name.
...
class CerebriumBuild:
    def __init__(
        ...
        shell_commands: Union[List[str], str] = [],
    ):
...
class CerebriumDeployment:
    def __init__(
        self,
        name: str = "my-model",
        python_version: PythonVersionType = constants.DEFAULT_PYTHON_VERSION,
...
class CerebriumConfig:
    ...
    def __to_json__(self, indent: int = 4):
        return json.dumps(self.to_dict(), indent=indent, sort_keys=False)  # type: ignore
