...
def init(
    ...
    disable_confirmation: Annotated[
        Optional[bool],
        typer.Option(
            help="Whether to disable confirmation prompts.",
        ),
    ] = False,
...
def deploy(
    ...
    cpu: Annotated[
        ...
        typer.Option(
            ...
            max=constants.MAX_CPU,
            help=(
                "Number of vCPUs to use for the Cortex deployment. Defaults to {constants.DEFAULT_CPU}. "
...
def package_app(
    ...
    if (
        ...
        return "Failed to deploy, no main.py found", None
    ...
    if cerebrium_config.partial_upload:
        ...
        setup_response = _setup_request(
            cerebrium_config=cerebrium_config, params=params
        )
    ...
    build_status = _stream_logs(
        build_id=build_id,
        cerebrium_config=cerebrium_config,
