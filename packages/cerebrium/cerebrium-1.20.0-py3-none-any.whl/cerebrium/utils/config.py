...
def update_config_from_files(
    ...
    if has_changed and warning_message:
        files = [e for e in constants.INTERNAL_FILES if os.path.exists(e)]
        cerebrium_log(
            level="WARNING",
    ...
    if archive_files:
        for file in constants.INTERNAL_FILES:
            if file is not constants.SHELL_COMMANDS_FILE:
                archive_file(file)
    else:
        for file in constants.INTERNAL_FILES:
            if (
                os.path.exists(file)
                and file.strip("./") != constants.SHELL_COMMANDS_FILE
            ):
                os.remove(file)
