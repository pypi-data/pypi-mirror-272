...
def make_cortex_util_files(
    ...
    files_to_remove = constants.INTERNAL_FILES.copy()
    files_to_remove.extend(f"./{f}" for f in constants.INTERNAL_FILES)
    ...
    predict_file = os.path.join(
        working_dir, constants.PREDICT_FILE
    )  # use a file to avoid storing large files in the model objects in ddb
    if not source == "serve" and not config.build.disable_predict:
        if config.build.predict_data is not None and not os.path.exists(
            constants.PREDICT_FILE
        ):
            with open(predict_file, "w") as f:
                f.write(
                    config.build.predict_data
                )  # predict data has been validated as a json string previously
        elif config.build.predict_data is None and os.path.exists(
            constants.PREDICT_FILE
        ):
            # Use the existing predict file
            config.file_list.append(constants.PREDICT_FILE)

    # Create files temporarily for upload
    requirements_files = [
        (constants.PIP_REQUIREMENTS_FILE, config.dependencies.pip),
        (constants.APT_REQUIREMENTS_FILE, config.dependencies.apt),
        (constants.CONDA_REQUIREMENTS_FILE, config.dependencies.conda),
        (constants.SHELL_COMMANDS_FILE, config.build.shell_commands),
    ]

    for file_name, reqs in requirements_files:
        if reqs:
            if file_name == constants.SHELL_COMMANDS_FILE:
                if isinstance(reqs, str):
                    if os.path.exists(reqs):
                        if reqs.startswith("./"):
                            reqs = reqs[2:]
                        if reqs != constants.SHELL_COMMANDS_FILE:
                            shutil.copy(
                                reqs,
                                os.path.join(
                                    working_dir, constants.SHELL_COMMANDS_FILE
                                ),
                            )
                        else:
                            config.file_list.append(constants.SHELL_COMMANDS_FILE)
                    else:
                        cerebrium_log(
                            message=f"Shell commands file {reqs} does not exist. If you would like to use a command, please add it to your toml in brackets. e.g. shell_commands = [echo 'hello world']",
                            level="ERROR",
                        )

                else:
                    shell_commands_to_file(reqs, os.path.join(working_dir, file_name))
                    #  else, it's a path to a file that we need to upload
            else:
                requirements_to_file(
                    reqs,
                    os.path.join(working_dir, file_name),
                    is_conda=file_name == constants.CONDA_REQUIREMENTS_FILE,
                )
