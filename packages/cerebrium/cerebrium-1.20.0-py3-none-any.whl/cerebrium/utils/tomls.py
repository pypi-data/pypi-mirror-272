...
def legacy_to_toml_structure(
    ...
    build = datatypes.CerebriumBuild(
        ...
        disable_confirmation=legacy_config.get(
    ...
    if os.path.exists(requirements_file):
        if os.stat(requirements_file).st_size != 0:
            dependencies = update_from_file(
                ...
                key="pip",
                confirm=(legacy) and (not disable_confirmation),
            )
    else:
        dependencies["pip"] = pip
    pkglist_file = os.path.join(dir_path, "pkglist.txt")
    if os.path.exists(pkglist_file):
        if os.stat(pkglist_file).st_size != 0:
            dependencies = update_from_file(
                pkglist_file,
                dependencies,
                "apt",
    ...
    conda_pkglist_file = os.path.join(dir_path, "conda_pkglist.txt")
    if os.path.exists(conda_pkglist_file):
        if os.stat(conda_pkglist_file).st_size != 0:
            dependencies = update_from_file(
                conda_pkglist_file,
                dependencies,
                "conda",
...
def save_config_to_toml_file(
    ...
    if not config_dict.get("build", {}).get("shell_commands"):
        config_dict["build"].pop("shell_commands")
        if os.path.exists(constants.SHELL_COMMANDS_FILE):
            if os.path.getsize(constants.SHELL_COMMANDS_FILE) > 0:
                config_dict["build"]["shell_commands"] = constants.SHELL_COMMANDS_FILE
    ...
    if "region_name" in config_dict.get("hardware", {}):
        # region_name is the real name of the region for the backend region is the name of the region for the user
        config_dict["hardware"].pop("region_name")

    if "build" in config_dict:
        if "force_rebuild" in config_dict["build"]:
            # Never want to save force_rebuild to the toml.
            # It should be a cli only flag.
            config_dict["build"].pop("force_rebuild")
