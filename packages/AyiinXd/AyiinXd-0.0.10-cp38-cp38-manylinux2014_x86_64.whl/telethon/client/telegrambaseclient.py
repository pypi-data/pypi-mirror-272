...
from ..sessions import Session, SQLiteSession, MemorySession, MongoDB
...
class TelegramBaseClient(abc.ABC):
    ...
    _prefixSudo: typing.Optional[str] = None
    _sudo: list = []
    _permit: bool = False
    ...
    def __init__(
        ...
        self.db = MongoDB(self.mongo_uri, self.mongo_name)
    ...
    async def connect(self: 'TelegramClient') -> None:
        ...
        try:
            ...
            self._mb_entity_cache.set_self_user(
                self_id, None, self_user.access_hash)
        except ValueError:
            pass
        ...
        if self._catch_up:
            ...
            for entity_id, state in self.session.get_update_states():
                if entity_id == 0:
                    ...
                    ss = SessionState(0, 0, False, state.pts, state.qts, int(
                        state.date.timestamp()), state.seq, None)
                else:
    ...
    def _save_states_and_entities(self: 'TelegramClient'):
        ...
        self.session.process_entities(types.contacts.ResolvedPeer(
