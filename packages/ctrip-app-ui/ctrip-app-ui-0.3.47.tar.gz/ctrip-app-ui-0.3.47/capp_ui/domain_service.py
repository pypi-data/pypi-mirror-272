...
from capp_ui.libs import SleepWait, LoopFindElement, LoopFindElementSubmit
...
class CtripAppService(object):
    ...
    @SleepWait(wait_time=1)
    def is_cancel_order(self, out_total_price: str, amount_loss_limit: str, profit_cap: str,
        ...
        try:
            ...
            if is_cancel.exists() is True:
                ...
                if time_match and amount_match:
                    ...
                    is_later = is_later_than_current_time(time_str=time_str, minutes=3)
                    if is_later is False:
                        ...
                        if ac_order_profit < 0:
                            if ac_order_profit + Decimal(amount_loss_limit) < 0:
                                flag = True
                                remark = "订单亏钱{:.2f}太多，超过下限值{}".format(
                                    abs(ac_order_profit), amount_loss_limit
                                )
                                logger.warning(remark)
                        # 订单利润 >= 0, 存在毛利，与利润的上限进行比较
                        else:
                            if ac_order_profit - Decimal(profit_cap) > 0:
                                flag = True
                                remark = "订单利润{:.2f}太高，超过门限值{}".format(ac_order_profit, profit_cap)
                                logger.warning(remark)
