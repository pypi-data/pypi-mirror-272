...
class NativeS3Converter:
            ...
            try:
                ...
                if "." in field_name:
                    self._handle_nested_fields(
                        field_name, field_dtype, nullable, fields, df[field_name]  # type: ignore
                    )
                else:
                    gable_type = self._parse_type(
                        field_dtype, field_name, nullable[field_name], df[field_name]  # type: ignore
                    )
    ...
    def _parse_type(
        self, dtype: str, field_name: str, nullable: bool, column: pd.Series
    ) -> dict:
        """
            ...
            if found_recap_type is not None:
                ...
                if index == len(field_name_parts) - 1:
                    ...
                    subfield_recap_type = self._parse_type(
                        field_type, subfield_name, nullable[field_name], column
                    )
                else:
                    subfield_recap_type = {
                        "fields": [],
