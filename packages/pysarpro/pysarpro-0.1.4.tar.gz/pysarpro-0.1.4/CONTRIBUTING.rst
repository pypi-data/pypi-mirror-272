...
How to contribute to pysarpro
=================================

Developing Open Source is great fun!
If you're looking for something to implement or to fix, you can browse the
`open issues on GitHub <https://github.com/Pol-InSAR/pysarpro/issues?q=is%3Aopen>`__.
...
.. warning::
   ...
   of any contributions to pysarpro.
...
can be contributed to pysarpro.

1. If you are a first-time contributor:

   * Go to `https://github.com/Pol-InSAR/pysarpro
     <https://github.com/Pol-InSAR/pysarpro>`_ and click the
     "fork" button to create your own copy of the project.

   * Clone (download) the repository with the project source on your local computer::

      git clone https://github.com/your-username/pysarpro.git
   ...
   * Now, you have remote repositories named:
     ...
     - ``upstream``, which refers to the ``pysarpro`` repository, and
...
3. To submit your contribution:
   ...
   * If you want, post on the `developer forum
     <https://discuss.scientific-python.org/c/contributor/pysarpro>`_ to explain your changes or
     to ask for review.

For a more detailed discussion, read these :doc:`detailed documents
<../gitwash/index>` on how to use Git with ``pysarpro`` (:ref:`using-git`).
...
* No changes are ever merged without review and approval by two core team members.
  ...
  `developer forum <https://discuss.scientific-python.org/c/contributor/pysarpro>`_ if
...
* Use relative module imports, i.e. ``from .._shared import xyz`` rather than
  ``from pysarpro._shared import xyz``.
...
``pysarpro/submodule/tests`` folders.

Testing requirements are listed in `requirements/test.txt`.
Run:

- **All tests**: ``spin test``
- Tests for a **submodule**: ``spin test pysarpro/morphology``
- Run tests from a **specific file**: ``spin test pysarpro/morphology/tests/test_gray.py``
- Run **a test inside a file**:
  ``spin test pysarpro/morphology/tests/test_gray.py::test_3d_fallback_black_tophat``
- Run tests with **arbitrary ``pytest`` options**:
  ``spin test -- any pytest args you want``.
- Run all tests and **doctests**:
  ``spin test -- --doctest-modules pysarpro``
...
``pysarpro_TEST_STRICT_WARNINGS`` to `0`.
...
This will print a report with one line for each file in `pysarpro`,
detailing the test coverage::

  Name                                             Stmts   Exec  Cover   Missing
  ------------------------------------------------------------------------------
  pysarpro/color/colorconv                             77     77   100%
  pysarpro/filter/__init__                              1      1   100%
  ...
...
Output is in ``pysarpro/doc/build/html/``.  Add the ``--clean``
...
``pysarpro`` raises ``FutureWarning``\ s to highlight changes in its
API, e.g.:
...
instead of being emitted by files in the pysarpro library:

* **Good**: ``ipython:1: UserWarning: ...``
* **Bad**: ``pysarpro/pysarpro/measure/_structural_similarity.py:155: UserWarning:``

Deprecating Keywords and Functions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When removing keywords or entire functions, the
``pysarpro._shared.utils.deprecate_kwarg`` and
``pysarpro._shared.utils.deprecate_func`` utility functions can be used
to perform the above procedure.

Adding Data
-----------
While code is hosted on `github <https://github.com/Pol-InSAR/>`_,
example datasets are on `gitlab <https://gitlab.com/Pol-InSAR/data>`_.
These are fetched with `pooch <https://github.com/fatiando/pooch>`_
when accessing `pysarpro.data.*`.

New datasets are submitted on gitlab and, once merged, the data
registry ``pysarpro/data/_registry.py`` in the main GitHub repository
can be updated.
...
at the following `website <https://pandas.pydata.org/speed/pysarpro/>`_.
...
development environment, then if using ``venv`` you may install the requirement with::
  ...
  source pysarpro-dev/bin/activate
  pip install asv
...
.. code-block:: python
  ...
  import numpy as np
  from pysarpro import transform

  class TransformSuite:
      """Benchmark for transform routines in pysarpro."""
...
of pysarpro. So if anything from pysarpro is imported at the top level,
it should be done as:

.. code-block:: python

    try:
        from pysarpro import metrics
    except ImportError:
        pass
...
.. code-block:: python
    ...
    try:
        from pysarpro import registration
    except ImportError:
...
of time since building pysarpro can be a time consuming task::
...
``pysarpro`` repository. The command ``asv continuous`` is of help here::
...
The output may look something like::
  ...
  [  0.00%] · For pysarpro commit 37c764cb <benchmark_docs~1> (round 1/2):
  [...]
...
the pysarpro.filter module benchmarks on release v0.17.2::
