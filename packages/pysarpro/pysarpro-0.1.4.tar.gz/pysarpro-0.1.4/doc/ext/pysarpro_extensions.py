"""Custom Sphinx extensions for pysarpro's docs.
...
def write_random_js(app, exception):
    ...
    if app.builder.format != "html":
        logger.debug(
            "[pysarpro_extensions] skipping generation of random.js for non-html build"
        )
        return
    ...
    if tutorial_urls == 0:
        logger.error(
            "[pysarpro_extensions] did not find any gallery examples while creating %s",
            random_js_path,
        )
        return
    ...
    with open(random_js_path, 'w') as file:
        file.write(content)
    logger.info(
        "[pysarpro_extensions] created %s with %i possible targets",
        random_js_path,
        len(tutorial_urls),
    )
