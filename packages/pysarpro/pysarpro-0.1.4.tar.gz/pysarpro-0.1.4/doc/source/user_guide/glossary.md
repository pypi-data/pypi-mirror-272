...
array
    ...
    ``pysarpro``, images are NumPy arrays with dimensions that
...
channel
    ...
    ``channel_axis=None``. Aside from the functions in ``pysarpro.color``, most
...
contrast
    ...
    image are available in {mod}`pysarpro.exposure`. See {ref}`exposure`.
...
float
    ...
    exponential prefactors in {func}`pysarpro.filters.gaussian`), so that
...
histogram
    ...
    {func}`pysarpro.exposure.histogram`.
...
int
    ...
    which needs to be represented. In ``pysarpro``, the most common
...
labels
    ...
    segmentation is an image of labels. {func}`pysarpro.measure.label` labels
    connected components of a binary image and returns an image of
    labels. Labels are usually contiguous integers, and
    {func}`pysarpro.segmentation.relabel_sequential` can be used to relabel
...
pixel
    ...
    images. In ``pysarpro``, pixels are the individual elements of
