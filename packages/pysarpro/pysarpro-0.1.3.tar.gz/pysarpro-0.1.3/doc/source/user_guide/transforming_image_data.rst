...
:mod:`pysarpro.color` provides utility functions to convert images
...
from RGB to grayscale::
    ...
    >>> sarprocolor.rgb2gray(red_pixel)
    array([[ 0.2125]])
    >>> green_pixel = np.array([[[0, 255, 0]]], dtype=np.uint8)
    >>> sarprocolor.rgb2gray(green_pixel)
    array([[ 0.7154]])
...
.. currentmodule:: pysarpro.exposure
...
:mod:`pysarpro.exposure` provides functions that spread the intensity
...
:func:`pysarpro.exposure.histogram`::
    ...
    >>> sarproexposure.histogram(image)
...
this operation)::
    ...
    >>> v_min, v_max
    (10.0, 186.0)
    >>> better_contrast = sarproexposure.rescale_intensity(moon, in_range=(v_min, v_max))
