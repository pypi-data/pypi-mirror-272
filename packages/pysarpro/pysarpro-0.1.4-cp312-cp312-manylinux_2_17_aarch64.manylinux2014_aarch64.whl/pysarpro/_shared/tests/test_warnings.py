...
def setup():
    ...
    old_strictness = os.environ.pop('pysarpro_TEST_STRICT_WARNINGS', None)
    yield
    # Add the user's desired strictness
    if old_strictness is not None:
        os.environ['pysarpro_TEST_STRICT_WARNINGS'] = old_strictness
...
def test_strict_warning_true(setup, strictness):
    os.environ['pysarpro_TEST_STRICT_WARNINGS'] = strictness
    with pytest.raises(ValueError):
        with expected_warnings(['some warnings']):
            pass
