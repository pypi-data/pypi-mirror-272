...
Images in ``pysarpro`` are represented by NumPy ndarrays. Hence, many
...
.. note::
    ...
    are not natively supported in ``pysarpro``. However, data stored in these types
...
types and how ``pysarpro`` treats them.
...
The pysarpro library functions supporting color images have a
...
Because ``pysarpro`` represents images using NumPy arrays, the
...
color channels, and is denoted by ``channel`` or ``ch``. Prior to pysarpro
...
.. table:: *Dimension name and order conventions in pysarpro*
...
Many functions in ``pysarpro`` can operate on 3D images directly::
    ...
    >>> ws = sarprosegmentation.watershed(im3d, seeds)

In many cases, however, the third spatial dimension has lower resolution
than the other two. Some ``pysarpro`` functions provide a ``spacing``
keyword argument to help handle this kind of data::

    >>> slics = sarprosegmentation.slic(im3d, spacing=[5, 1, 1], channel_axis=None)
...
syntax can be used::
    ...
    ...     edges[pln] = sarprofilters.sobel(image)
...
developing algorithms. In particular, ``pysarpro`` uses C-contiguous
arrays by default.
...
Although ``pysarpro`` does not currently provide functions to
...
.. table:: *Addendum to dimension names and orders in pysarpro*
