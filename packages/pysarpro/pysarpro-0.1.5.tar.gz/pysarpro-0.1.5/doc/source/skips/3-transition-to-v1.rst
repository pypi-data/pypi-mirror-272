...
==========================================
SKIP 3 — Transitioning to pysarpro 1.0
==========================================
...
pysarpro is preparing to release version 1.0. This is potentially an
...
pysarpro version dependencies accordingly.
...
of projects depend on pysarpro and would thus be affected by backwards
...
unlikely that anyone used ``pysarpro<1.*`` in their dependency list (though
...
for pysarpro 1.0, many of which have yet to be decided upon. Indeed, the
...
pysarpro/pysarpro#5439 [7]_. Some examples are briefly included below
...
pysarpro dependency to 0.19.x. The warning would also point users to a
...
correctly, in sync with pysarpro.
...
- pysarpro 0.19 will be the final *true* 0.x release. It contains some new
...
- shortly after 0.19, we release 0.20, which is identical except that it emits
  ...
  "pysarpro 1.0 will be released later this year and will contain breaking
  changes. To ensure your code keeps running, please install
  ``pysarpro<=0.19.*``. To silence this warning but still depend on
  pysarpro after 1.0 is released, install ``pysarpro!=0.20.*``." The
...
- After 0.20, we make all the API changes we need, without deprecation cycles.
  Importantly, for every API change, we add a line to a "pysarpro 1.0
...
- Once the transition has happened in the repository, we release 1.0.0a0, an
  ...
  authors who chose to pin to ``pysarpro!=0.20.*`` a chance to make the
  migration to 1.0.
...
Instead of breaking compatibility in the ``pysarpro`` package, we could
leave that package at 0.19, and release a *new* package, e.g.
``pysarpro1``, which starts at 1.0 and imports as ``pysarpro1``. This would
obviate the need for users to pin their pysarpro version — users depending
on pysarpro 0.x would be able to use that library "in perpetuity."
...
would be decreased, as everyone installing ``pysarpro`` would still get the
old version.
...
approach. The minutes of this meeting are in the pysarpro meeting notes
repository [9]_.
...
sufficient to protect users [15]_:
    ...
    In pysarpro, we adopted the semantic versioning as it
...
pysarpro "documentation", such as a decade's worth of accumulated
...
as pointed out by Josh Warner [16]_:
    ...
    pysarpro teaching material out there. The majority we do not control,
...
.. [13] https://mail.python.org/archives/list/pysarpro@python.org/message/UYARUQM5LBWXIAWBAPNHIQIDRKUUDTEK/
.. [14] https://mail.python.org/archives/list/pysarpro@python.org/message/63ZGG7DY5SWVM62XASHMCPFAG6KPJCMT/
.. [15] https://mail.python.org/archives/list/pysarpro@python.org/message/HXI7YVCN6IFF5TL54JBP5QRUDHKTTYRR/
.. [16] https://mail.python.org/archives/list/pysarpro@python.org/message/HRZGMOJLD2WDIO3JXQV3PRWKIUOVOF7P/
.. [17] https://mail.python.org/archives/list/pysarpro@python.org/message/GFXBQYKDACDCH7BGNEGOU7LKHR2LPFX6/
.. [18] https://mail.python.org/archives/list/pysarpro@python.org/message/5J4W63BXFQTT4GHPTZFH3AM4QHAXOW5R/
