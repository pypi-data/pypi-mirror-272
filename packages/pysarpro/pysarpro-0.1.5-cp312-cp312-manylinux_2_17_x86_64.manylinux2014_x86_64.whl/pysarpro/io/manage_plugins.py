...
can be multiple states for a given plugin:
    ...
    available: Defined in an *ini file located in ``pysarpro.io._plugins``.
        See also :func:`pysarpro.io.available_plugins`.
    ...
    available but not on this system: Defined in ``pysarpro.io._plugins``, but
...
plugin_provides = {}
# The module names for the plugins in `pysarpro.io._plugins`.
plugin_module_name = {}
...
def _scan_plugins():
    ...
    for filename in config_files:
        ...
        if 'provides' not in meta_data:
            warnings.warn(
                f'file {filename} not recognized as a pysarpro io plugin, skipping.'
            )
            continue
    ...
    *args, **kwargs : arguments and keyword arguments
    ...
    if len(plugin_funcs) == 0:
        msg = (
            ...
            "You may load I/O plugins with the `pysarpro.io.use_plugin` "
            "command.  A list of all available plugins are shown in the "
            "`pysarpro.io` docstring."
        )
...
def use_plugin(name, kind=None):
    ...
    ----------
    name : str
        Name of plugin. See ``pysarpro.io.available_plugins`` for a list of available
        plugins.
    ...
    To see a list of available plugins run ``pysarpro.io.available_plugins``. Note
...
def _load(plugin):
    ...
    if plugin not in plugin_module_name:
        ...
        plugin_module = __import__(
            'pysarpro.io._plugins.' + modname, fromlist=[modname]
        )
