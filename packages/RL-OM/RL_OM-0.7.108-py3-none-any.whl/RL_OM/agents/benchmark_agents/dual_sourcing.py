...
class SingleIndexPolicy():
    ...
    def __init__(self,
        ...
        self.S = 0.53 #0.97 # initial value
        self.S_e = 0.47 #0.86 # initial value
...
class CappedDualIndexPolicy(SingleIndexPolicy):
    ...
    def set_and_calculate_grid_with_cuts(self, S, S_e, cap, step_size, plot=False):
        ...
        initial_S_e_candidates = np.arange(S_e, S_e + step_size*(self.initial_steps+1), step_size)
        initial_cap_candidates = np.arange(cap, cap + step_size*(self.initial_steps+1), step_size)
        ...
        for i, S_candidate in enumerate(initial_S_candidiates):
            for j, S_e_candidate in enumerate(initial_S_e_candidates):
                if S_e_candidate > S_candidate:
                    cost_value = np.inf
                else:
                    for k, cap_candidate in enumerate(initial_cap_candidates):
                        cost_value = self.run_simulation(S_candidate, S_e_candidate, cap_candidate)
