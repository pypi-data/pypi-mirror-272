"""
Command-line interface to manage KozmoML models.
"""
...
async def start(folder: str):
    """
    Start serving a machine learning model with KozmoML.
    """
    settings, models_settings = await load_settings(folder)

    server = KozmoML(settings)
...
async def build(folder: str, tag: str, no_cache: bool = False):
    """
    Build a Docker image for a custom KozmoML runtime.
    """
...
@click.option(
    ...
    envvar="KOZMOML_INFER_URL",
    help=(
        "URL of the KozmoML to send inference requests to. "
...
@click.option(
    ...
    type=str,
    required=True,
    envvar="KOZMOML_INFER_MODEL_NAME",
...
@click.option(
    ...
    required=True,
    type=click.Path(),
    envvar="KOZMOML_INFER_INPUT_DATA_PATH",
...
@click.option(
    ...
    required=True,
    type=click.Path(),
    envvar="KOZMOML_INFER_OUTPUT_DATA_PATH",
    help="Local path to the output file for the inference responses to be  written to.",
)
@click.option("--workers", "-w", default=10, envvar="KOZMOML_INFER_WORKERS")
@click.option("--retries", "-r", default=3, envvar="KOZMOML_INFER_RETRIES")
@click.option(
    "--batch-size",
    "-s",
    default=1,
    envvar="KOZMOML_INFER_BATCH_SIZE",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    envvar="KOZMOML_INFER_BINARY_DATA",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    envvar="KOZMOML_INFER_EXTRA_VERBOSE",
    help="Extra verbose mode (shows detailed requests and responses).",
)
@click.option(
    "--transport",
    "-t",
    envvar="KOZMOML_INFER_TRANSPORT",
...
@click.option(
    "--request-headers",
    "-H",
    envvar="KOZMOML_INFER_REQUEST_HEADERS",
    type=str,
    multiple=True,
    help=(
...
@click.option(
    "--timeout",
    default=60,
    envvar="KOZMOML_INFER_CONNECTION_TIMEOUT",
...
@click.option(
    ...
    default=0,
    type=float,
    envvar="KOZMOML_INFER_BATCH_INTERVAL",
...
@click.option(
    ...
    default=0,
    type=float,
    envvar="KOZMOML_INFER_BATCH_JITTER",
