...
KOZMOML_MODEL_NAME_HEADER = "kozmoml-model"
KOZMOML_MODEL_VERSION_HEADER = "kozmoml-version"
...
class KafkaHandlers:
    ...
    def _get_model_details(self, request: KafkaMessage) -> Tuple[str, Optional[str]]:
        ...
        if KOZMOML_MODEL_NAME_HEADER not in headers:
            raise InvalidMessageHeaders(KOZMOML_MODEL_NAME_HEADER)

        model_name = headers[KOZMOML_MODEL_NAME_HEADER]
        model_version = headers.get(KOZMOML_MODEL_VERSION_HEADER, None)
