DefaultBaseImage = "kozmoai/kozmoml:{version}-slim"
...
ARG KOZMOML_ENV_NAME="kozmoml-custom-env" \\
    KOZMOML_ENV_TARBALL="./envs/base.tar.gz"
...
COPY \\
    ...
    ./conda.yam[l] \\
    .
RUN mkdir $(dirname $KOZMOML_ENV_TARBALL); \\
    for envFile in environment.yml environment.yaml conda.yml conda.yaml; do \\
        if [[ -f $envFile ]]; then \\
            conda env create \
                --name $KOZMOML_ENV_NAME \\
                --file $envFile; \\
            conda-pack --ignore-missing-files \
                -n $KOZMOML_ENV_NAME \\
                -o $KOZMOML_ENV_TARBALL; \\
        fi \\
    done; \\
    chmod -R 776 $(dirname $KOZMOML_ENV_TARBALL)
