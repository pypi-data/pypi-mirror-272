...
ENV_PREFIX_SETTINGS = "KOZMOML_"
ENV_PREFIX_MODEL_SETTINGS = "KOZMOML_MODEL_"
...
class Settings(BaseSettings):
    ...
    """Grace timeout to wait until the workers shut down when stopping KozmoML."""
    ...
    ``kozmoml.repository.repository.SchemalessModelRepository``)."""

    # Model repository settings
    model_repository_root: str = "."
    """Root of the model repository, where we will search for models."""

    # Model Repository parameters are meant to be set directly by the KozmoML runtime.
    ...
    kafka_topic_input: str = "kozmoml-input"
    kafka_topic_output: str = "kozmoml-output"
...
class ModelParameters(BaseSettings):
    ...
    This path must be either absolute or relative to where KozmoML is running.
    """
...
class ModelSettings(BaseSettings):
    ...
    implementation_: str = Field(
        alias="implementation", env="KOZMOML_MODEL_IMPLEMENTATION"
    )
    """*Python path* to the inference runtime to use to serve this model (e.g.
    ``kozmoml_sklearn.SKLearnModel``)."""

    # Model parameters are meant to be set directly by the KozmoML runtime.
