...
from pyastroweatherio.helper_functions import ConversionFunctions, AstronomicalRoutines, Atmosphere
...
class AstroWeather:
    ...
    async def _get_location_data(self) -> None:
        ...
        if seventimer_index < len(self._weather_data_seventimer):
            ...
            details_seventimer = {"timepoint": 0, "seeing": _NOT_AVAILABLE, "transparency": _NOT_AVAILABLE, "lifted_index": _NOT_AVAILABLE}
    ...
    async def _get_forecast_data(self, forecast_type, hours_to_show) -> None:
        ...
        for metno_index, datapoint in enumerate(self._weather_data_metno):
            ...
            item["cloud_area_fraction_high"] = details_metno.get("cloud_area_fraction_high")
            item["cloud_area_fraction_medium"] = details_metno.get("cloud_area_fraction_medium")
            item["cloud_area_fraction_low"] = details_metno.get("cloud_area_fraction_low")
            ...
            if self._weather_data_metno[metno_index].get("data", {}).get("next_1_hours") is None:
                # No more hourly data
                _LOGGER.debug("No more hourly data at %s", self._weather_data_metno[metno_index].get("time", {}))
                break
            if self._weather_data_metno[metno_index].get("data", {}).get("next_6_hours") is None:
    ...
    async def _calc_condition_percentage(
        ...
        if not all(v is not None for v in [cloudcover_high, cloudcover_medium, cloudcover_low, seeing, transparency, wind_speed]):
            return None
            ...
            if astro_dataseries != {} and not self._experimental_features:
                ...
                for index in range(0, 20):
                    self._weather_data_seventimer.append(
                        {"timepoint": index * 3, "seeing": _NOT_AVAILABLE, "transparency": _NOT_AVAILABLE, "lifted_index": _NOT_AVAILABLE}
                    )
    ...
    def _get_data_seventimer_timer(self, anchor_timestamp, datetime):
        ...
        for index, row7 in enumerate(self._weather_data_seventimer):
            ...
            # index += 1
        if seventimer_index >= len(self._weather_data_seventimer):
            return {"timepoint": row7["timepoint"], "seeing": _NOT_AVAILABLE, "transparency": _NOT_AVAILABLE, "lifted_index": _NOT_AVAILABLE}
        else:
