...
from supervisor_detect.saving import load_detector

import kozmoserver
from kozmoserver.batching import BatchedRequests
from kozmoserver.types import InferenceRequest, InferenceResponse
from kozmoserver.settings import ModelSettings
...
from kozmoserver.errors import KozmoServerError, InferenceError
from kozmoserver.logging import logger

ENV_PREFIX_SUPERVISOR_DETECT_SETTINGS = "KOZMOSERVER_MODEL_SUPERVISOR_DETECT_"

SUPERVISOR_MODULE_NAMES = {"cd": "drift", "od": "outlier", "ad": "adversarial"}


class SupervisorDetectSettings(BaseSettings):
    """
    Parameters that apply only to supervisor detect models
    """

    class Config:
        env_prefix = ENV_PREFIX_SUPERVISOR_DETECT_SETTINGS
    ...
    Note that this is different to KozmoServer's adaptive batching, since the rest
...
class SupervisorDetectRuntime(MLModel):
    """
    Implementation of the MLModel interface to load and serve `supervisor-detect` models.
    """

    def __init__(self, settings: ModelSettings):
        if settings.parameters is None:
            self._ad_settings = SupervisorDetectSettings()
        else:
            extra = settings.parameters.extra
            self._ad_settings = SupervisorDetectSettings(**extra)  # type: ignore
    ...
    async def load(self) -> bool:
        ...
        try:
            ...
            kozmoserver.register("kozmo_model_drift", "Drift metrics")
    ...
    def _log_metrics(self, current_pred: dict) -> None:
        ...
        if "is_drift" in data:
            ...
            for is_drift_instance in is_drift:
                kozmoserver.log(kozmo_model_drift=is_drift_instance)
    ...
    def _encode_response(self, y: dict) -> InferenceResponse:
        ...
        # Add headers
        y["meta"]["headers"] = {
            "x-kozmo-supervisor-type": self.supervisor_type,
            "x-kozmo-supervisor-method": self.supervisor_method,
        }
        return InferenceResponse(
    ...
    @cached_property
    def supervisor_method(self) -> str:
        module: str = type(self._model).__module__
        return module.split(".")[-1]

    @cached_property
    def supervisor_type(self) -> str:
        module: str = type(self._model).__module__
        method = module.split(".")[-2]
        if method in SUPERVISOR_MODULE_NAMES:
            return SUPERVISOR_MODULE_NAMES[method]
        else:
            return "unknown"
