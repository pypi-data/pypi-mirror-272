    ...
    async with httpx.AsyncClient() as client:
        ...
        while retry_count < max_retries and status_code != 200:
            ...
            if response.status_code == 409 and retry_count <= max_retries:
                ...
                logger.error(f"Status code {response.status_code} for {ATLAS_SERVICE_URL}. Tried {retry_count} time(s)")
                raise ValueError(f"Status code {response.status_code} for {ATLAS_SERVICE_URL}. Tried {retry_count} time(s). Failed to get a token: {response.content}")
            else:
