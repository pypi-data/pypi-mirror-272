...
class FileClient:
    ...
    def predict(self, file_path: str, prediction_timeout: float = 180.0) -> FileResult:
        # """Predicts the given file.
        #
        # Args:
        #     file_path: Path to the file. For example, "/Users/user/file.mp3"
        #     predictioon_timeout: Maximum amount of time to wait until prediction of the uploade file is done.
        # """
        ...
        # get results
        results = self._get_all_results(session_id, prediction_timeout)
    ...
    def _create_session(self, file_path: str) -> str:
        ...
        default_sensitivity = DefaultSensitivity(
            self.api_config.sensitivity.default.value
        )
        tags_sensitivity = {}
    ...
    def _get_all_results(self, session_id: str, timeout: float) -> FileResult:
        retry_count = 0
        data_count = 0
        ...
        while not is_timed_out:
            ...
            if state == "error":
                raise CochlSenseException(
                    "Inference failed because the account reached the maximum usage limit or an unexpected error."
                )
        ...
        if is_timed_out:
            # when server inference not complete within timeout period
            raise CochlSenseException(f"prediction (session_id={session_id}) has timed out '{timeout}s'")
    ...
    def _get_file_format(cls, file_path: str) -> str:
        ...
        try:
            ...
            raise ValueError(
                f"invalid file format '{file_path}', supported formats: {_supported_formats}"
            )
