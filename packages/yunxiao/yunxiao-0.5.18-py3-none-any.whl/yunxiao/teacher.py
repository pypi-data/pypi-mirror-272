...
class TeacherLite(BaseModel):
    ...
    orgStructureList: Optional[List[orgStructureList]] = [
        {"orgStructureName": "兼职", "leaderStatus": 0, "orgStructureId": 49186449}
    ]
    dataRole: int = 2
    id: int = None
...
def edit(client: YunXiao, teacher: TeacherLite):
    query = client.request(
            method="POST",
            url=f"https://{client.host}/api/cs-pc-crm/teacher/editTeacher",
            json=TeschersQueryPayload(queryKey=teacher.name).model_dump()
    )["data"]
    if not query:
        res = client.request(
            method="POST",
...
def create(client: YunXiao, teacher: TeacherLite):
    query = client.request(
            method="POST",
            url=f"https://{client.host}/api/cs-pc-crm/teacher/editTeacher",
            json=TeschersQueryPayload(queryKey=teacher.name).model_dump()
    ...
    if not query:
        data = teacher.model_dump().pop("id")
        res = client.request(
            method="POST",
            url=f"https://{client.host}/api/cs-pc-crm/teacher/editTeacher",
            json=data
...
def leave(client: YunXiao, teache_id: int):
    res = client.request(
        method="POST",
        url=f"https://{client.host}/api/cs-pc-crm/teacher/leaveOffice",
        json={
