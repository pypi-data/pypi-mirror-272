...
class Rudholm:
    ...
    def __fetch_data(self, base_url: str, method: str, body: dict = None, read_multiple: bool = False) -> list:
        ...
        if read_multiple:
            ...
            response = requests.request(
                method, base_url + f"?currentPage={page}", headers=self.headers, json=body, timeout=self.timeout
            )
    ...
    def __check_headers(
        ...
        if response.status_code == 503:
            ...
            logging.debug("BODY SENT: %s", response.request.body)
    ...
    def __wait_for_retry(self, retry_after: int) -> None:
        ...
        retry_after = (retry_after - current_time) + 2 # Add two seconds as a buffer
    ...
    def fetch_one(self, endpoint_id: str, item_id: str = "", body: dict = None) -> list:
        ...
        return self.__fetch_data(
            request_url["partial_url"], request_url["method"], body, True
        )
