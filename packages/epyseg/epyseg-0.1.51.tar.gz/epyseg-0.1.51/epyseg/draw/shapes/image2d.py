...
set_UI()
from epyseg.serialization.tools import create_objects_from_dict
import copy
...
class Image2D(Rectangle2D):
    ...
    def __init__(self, *args, x=None, y=None, width=None, height=None, data=None, dimensions=None, filename=None, fraction_of_parent_image_width_if_image_is_inset=0.25, opacity=1., theta=None, crop_left=0, crop_right=0, crop_top=0, crop_bottom=0,fill_color=None, border_color=0xFFFFFF,border_size=None, placement=Position('Top-Right'),annotations=None, custom_loading_script=None, allow_unknown_files=True, scale=1, **kwargs):
            ...
            if self.filename.lower().endswith('.svg'):
                ...
                try:
                    ...
                    if not allow_unknown_files:
                        ...
                        self.img = None # this is the indication that an error occured and that I cannot do much
        ...
        if draw:
            ...
            try:  # trick to force equal save and restore even upon error !!!
                ...
                if self.img is not None and not isinstance(self.img, plt.Figure) and not isinstance(self.img, QSvgRenderer):
                    ...
                    text_height = font_metrics.height()
                    text_x = rect_to_plot.x() + rect_to_plot.width() / 2. - text_width / 2.
                    text_y = rect_to_plot.y() + rect_to_plot.height() / 2. + text_height / 2.+text_height # to make it more visible
                    ...
                    try:
                        ...
                        if pos:
                            ...
                            if extra.placement.check_position('right'):
                                extra.setTopLeft(self.x() + self.width(scale=True) - extra.width(scale=True), extra.y())
                            if extra.placement.check_position('bottom'):
                                extra.setTopLeft(extra.x(), self.topLeft().y() + self.height(scale=True) - extra.height(
                                    scale=True))
                            ...
                            if extra.placement.check_position('center_h'):
                                extra.setTopLeft(
                                    self.x() + self.width(scale=True) / 2. - extra.width(scale=True) / 2., extra.y())
                            if extra.placement.check_position('center_v'):
                                extra.setTopLeft(extra.x(), self.y() + self.height(scale=True) / 2. - extra.height(
                                    scale=True) / 2.)
    ...
    def draw(self, painter, draw=True, parent=None):
        if draw:
            ...
            painter.restore()
                ...
                # scale_bar.setTopLeft(self.get_P1())
                # alignLeft(top_left, scale_bar)
                # alignTop(top_left, scale_bar)
                # # scale_bar.setTopLeft(scale_bar.get_P1().x()-extra_space, scale_bar.get_P1().x()+extra_space)
 ...
 # en fait pas good besoin de prendre les crops et le scale en compte
    ...
    qualities = [QPainter.SmoothPixmapTransform, QPainter.TextAntialiasing, QPainter.SmoothPixmapTransform, None, QPainter.Antialiasing] # None sets antialiasing to False -−> see code after # painter.setRenderHint  QPainter.HighQualityAntialiasing,
