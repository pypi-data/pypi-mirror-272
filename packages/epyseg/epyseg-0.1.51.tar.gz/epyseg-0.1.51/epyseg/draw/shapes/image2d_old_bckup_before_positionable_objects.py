...
class Image2D(Rectangle2D):
    ...
    def __init__(self, *args, x=None, y=None, width=None, height=None, data=None, dimensions=None, opacity=1.,fill_color=None, **kwargs):
        Image2D._count += 1
    ...
    def set_rotation(self, theta, bg_color=0x000000): # by default set it to black or to None --> white could also be useful
    ...
    def draw(self, painter, draw=True):
        if draw:
            painter.save()
            painter.setClipRect(self.getRect(scale=True)) # required first to fraw the stuff at the very beginning of it !!! --> it must be done before save

            # add a black bg behind the images if needed

            if self.fill_color is not None:
                painter.setBrush(QBrush(QColor(self.fill_color)))
                painter.drawRect(self.getRect(scale=True))
            ...
            #     rect_to_plot = self.boundingRect(scaled=True) #scaled=True #self.adjusted(self.__crop_left, self.__crop_top, self.__crop_right, self.__crop_bottom) # need remove the crops with that
                rect_to_plot = self.getRect(scaled=True)
