...
#         last = reference.y()+reference.height(scale=True) # NB I COULD REPLACE topLeft with .topleft
#     else:
#         last = reference.x() + reference.width(scale=True)
#
#     # print('last',last, space, reference.y(), reference.height(), reference.height(scale=True))
#
...
def pack2(space=3, mode=packing_modes[0], extra_border_mode=False, *objects_to_pack): # a cleaned and better version of packX
    ...
    if orientation == packing_modes[1]:
        last = reference.y()+reference.height(scale=True) if not '-' in mode else 0 # NB I COULD REPLACE topLeft with .topleft
        ...
        last = reference.x() + reference.width(scale=True) if not '-' in mode else 0
        if extra_border_mode and isinstance(reference, Image2D):
            extra = reference.get_extra_border_size()
            if '-' in mode:
                last-=extra
    ...
    for img in objects_to_pack[1:]:
            ...
            if orientation == packing_modes[1]:
                if not '-' in mode:
                    ...
                    last = previous_coords.y() - img.height(scale=True)
                    if extra_border_mode and isinstance(img, Image2D):
                        last-=img.get_extra_border_size()
                    img.setTopLeft(img.x(), last)  # here again it could be replaced by setTopLeft
                    if extra_border_mode and isinstance(img, Image2D):
                ...
                if not '-' in mode:
                    ...
                    if extra_border_mode and isinstance(img, Image2D):
                        last-=img.get_extra_border_size()
                    last = previous_coords.x() - img.width(scale=True)
                    if extra_border_mode and isinstance(img, Image2D):
                        last-=img.get_extra_border_size()
                    img.setTopLeft(last, img.y())  # here again it could be replaced by setTopLeft
...
def align2(position=None,*objects_to_align_with_respect_to_ref):
    ...
    if position == align_positions[0] or position is None:
        ...
        last_y = reference.y() + reference.height(scale=True)
    elif position == align_positions[2]:
        last_x = reference.x()
    elif position == align_positions[3]:
        last_x = reference.x()+reference.width(scale=True)
    elif position == align_positions[4]:
        last_y = reference.y() + reference.height(scale=True) / 2.  # should be the end point of every stuff
    elif position == align_positions[5]:
        last_x = reference.x() + reference.width(scale=True) / 2.
    else:
        pass
