...
class JobSequence(BaseModel):
    ...
    def get_history_log(self, log_type=None, log_level=None):
        ...
        Returns:
            An :obj:`list` object.
        """
        log_type = self.LogType(log_type.upper()).value if log_type else None
        log_level = self.LogLevel(log_level.upper()).value if log_level else None

        history_logs_json = self._control_hub.api_client.get_job_sequence_log_history(
            self.id, log_type, log_level
        ).response.json()

        history_logs = [JobSequenceHistoryLog(log_json) for log_json in history_logs_json]
...
class JobSequenceHistoryLog(BaseModel):
    ...
    _ATTRIBUTES_TO_IGNORE = ['id']
    _REPR_METADATA = ['timestamp', 'logMessage', 'logType', 'logLevel']

    def __init__(self, history_log):
        super().__init__(
            history_log,
            attributes_to_ignore=JobSequenceHistoryLog._ATTRIBUTES_TO_IGNORE,
