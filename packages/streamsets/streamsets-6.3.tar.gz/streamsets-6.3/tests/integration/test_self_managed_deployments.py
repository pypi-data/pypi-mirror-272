...
def test_valid_install_mechanism(sch, sample_deployments, install_mechanism):
    sch.start_deployment(*sample_deployments)

    # Installation script runs in a docker container with -d flag as background process
    docker_command = 'docker run'
    mechanism_to_flags = {
        'TARBALL': {
            SelfManagedDeployment.InstallMechanism.DEFAULT: '--foreground',
            SelfManagedDeployment.InstallMechanism.BACKGROUND: '--background',
            SelfManagedDeployment.InstallMechanism.FOREGROUND: '--foreground',
        },
        'DOCKER': {
            SelfManagedDeployment.InstallMechanism.DEFAULT: docker_command + ' -d',
            SelfManagedDeployment.InstallMechanism.BACKGROUND: docker_command + ' -d',
            SelfManagedDeployment.InstallMechanism.FOREGROUND: docker_command + '',
        },
    }

    for sample_deployment in sample_deployments:
        install_script = sample_deployment.install_script(install_mechanism=install_mechanism.value)
        install_type = sample_deployment.install_type
        assert mechanism_to_flags[install_type][install_mechanism] in install_script


def test_invalid_install_mechanism(sch, sample_deployments):
    sch.start_deployment(*sample_deployments)

    invalid_install_mechanism = "foo"
    for sample_deployment in sample_deployments:
        with pytest.raises(ValueError):
            sample_deployment.install_script(install_mechanism=invalid_install_mechanism)
...
def test_install_script(sample_deployments):
    for deployment in sample_deployments:
