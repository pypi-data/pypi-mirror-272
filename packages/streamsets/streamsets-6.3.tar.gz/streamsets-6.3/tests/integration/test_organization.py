...
def test_update_org_config(org_config, sch):
    original_config = copy.deepcopy(org_config)

    MAX_JOB_RUN_KEY = 'Maximum number of job runs'
    # increasing instead of decreasing to not break other tests running parallely
    NEW_NUM_OF_JOBS_POSSIBLE = org_config[MAX_JOB_RUN_KEY] + 5

    org_config[MAX_JOB_RUN_KEY] = NEW_NUM_OF_JOBS_POSSIBLE

    # checking with updated info from Control Hub to make sure it went through properly
    another_org_config = sch.organizations.get(id=sch.organization).configuration
    assert another_org_config[MAX_JOB_RUN_KEY] == NEW_NUM_OF_JOBS_POSSIBLE

    # make sure nothing else has changed
    for key, value in original_config.items():
        if key == MAX_JOB_RUN_KEY:
            continue

        assert another_org_config[key] == value
