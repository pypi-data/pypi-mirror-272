...
class WordDoc:
    ...
    def insert_paragraph(self, text="", **args):
        start =  self.doc.add_paragraph(text, **args)
        start.paragraph_format.space_before = Cm(0)
        start.paragraph_format.space_after = Cm(0)
        return start

    def add_run(self, text, bold=False):
        self.insert_paragraph().add_run(text).bold = bold
    ...
    def traverse(self, indict, bold=True):
        self.walk(indict, origin=self.doc, bold=bold, depth=0, indent=self.indent)

    def walk(self, indict, origin, bold, depth=0, indent=0):
        new = indent
...
#            depth = depth+1
    ...
    #                header = origin.add_paragraph()
                    ...
                    header.paragraph_format.space_before = Cm(0)
                    header.paragraph_format.space_after = Cm(0)
                    self.walk(v, origin, bold, depth=depth+1, indent=indent)
                elif isinstance(v, list):
                    for i in v:
                        self.walk(i, origin, bold,  depth+1, indent=indent)
                else:
...
#                    header.paragraph_format.left_indent = Cm((depth+1) * indent)
            ...
            for i in indict:
                self.walk(i, origin, bold,  depth, indent=indent)
        else:
