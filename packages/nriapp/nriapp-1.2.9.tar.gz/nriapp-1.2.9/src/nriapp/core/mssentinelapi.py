...
#                        self.load_session()
    ...
    def get_incidents(self, alertStatus=['New','InProgress', 'Resolved'], severity=[256, 128, 64, 32], pageIndex= 1, lookBackInDays = 60, pageSize = 50, sourceFilter=[], titleFilter=[], incident_list=[]):  #July 25, 2023 Changed the PageSize. It seems like the Defender changed the limit from 3000 to 0-50 only.
        ...
        if len(json.loads(response.text)) == pageSize:
            self.get_incidents(alertStatus=alertStatus, pageIndex=pageIndex+1,lookBackInDays = lookBackInDays, pageSize=pageSize, incident_list=incident_list)
#        else:                                                          #July 26, 2023
#            incident_list.extend(json.loads(response.text))            #Removed. Causes the duplication of data.
...
#                } for i in json.loads(response.text)['Items']]
    ...
    def get_mail_metadata(self, startUtc, endUtc, networkMessageId):                #Email Explorer only
        ...
        if object["ResultData"] != None:
            ...
            "urls"                 : urlFilters,
            "returnPath"           : resultData['P1Sender'],
            "networkMessageId"     : resultData['NetworkMessageId'],
            "language"             : resultData['MailLanguage'],
            "senderIpLocation"     : resultData['SenderIpLocation'] if resultData.get('SenderIpLocation') else '',
            "compositeAuth"        : resultData['CompositeAuthResult'] if resultData.get('CompositeAuthResult') else 'Unknown',
            "dkimAuth"             : resultData['DkimAuthResult'] if resultData.get('DkimAuthResult') else 'Unknown',
            "spfAuth"              : resultData['SpfAuthResult'] if resultData.get('SpfAuthResult') else 'Unknown',
            "dmarcAuth"            : resultData['DmarcAuthResult'] if resultData.get('DmarcAuthResult') else 'Unknown',
            "urlData"              : resultData['UrlDatas'],
            "urlCount"             : resultData['UrlCount']
            }
        else:
            {}

    def get_mail_timeline(self, startUtc, endUtc, networkMessageId):                        #Email Explorer only
        metadata_search = "https://security.microsoft.com/apiproxy/di/Find/MailMetaData"
...
#   def get_normalized_url_value(self, hash)
...
#                        dbgPrint.info("ISP: %s" % isp)
                        ...
                        if logonType:
                            data.update(logonType)
                        if accountDisplayName:
                            data.update(accountDisplayName)
                        if objectId:
                            data.update(objectId)
                        if iPAddress:
                            data.update(iPAddress)
                        if userAgent:
                            data.update(userAgent)
                    else:
...
#                                networkMessageId = i.get("networkMessageId")
                                metadata = self.parentClass.get_mail_metadata(i["receivedDate"], i["receivedDate"], i["networkMessageId"])

                                if metadata:
                                    mailMessage["language"] = metadata["language"]
                                    mailMessage["returnPath"] = metadata["returnPath"].replace("https", "hxxps").replace("http", "hxxp").replace(".", "[.]").replace(":","[:]")
                                    mailMessage["networkMessageId"] = metadata["networkMessageId"]
                                    mailMessage["Authentications"] = {
                                        "DKIM" :  metadata["dkimAuth"],
                                        "SPF" : metadata["spfAuth"],
                                        "DMARC" : metadata["dmarcAuth"],
                                        "Composite" : metadata["compositeAuth"]                                        
                                        }
    ...
    #                                    if x["Verdict"] != None:
                                ...
                                for a in file_list:
                                    file = {}
                                    if a["type"] == "file":
                                        file["filename"] = a["name"]
                                        file["malwareFamily"] = a["MalwareFamily"]
                                        file["sha256"] = [i["value"].lower() for i in a["fileHashes"] if i["algorithm"] == "SHA256"][0] if [i for i in a["fileHashes"] if i.get("$id")] else ""
...
#                                other_data = self.parentClass.get_mail_metadata(i["receivedDate"], i["receivedDate"], i["networkMessageId"])
            ...
            def parse_mcas_alert_story(self, queue=None):
                if hasattr(self, "items"):
                    ...
                    if self.description["type"] == "ITPAlertStoryDescription":
                        ...
                        logonRequest = self.items[0]["cloudLogonRequest"] if self.items[0].get("cloudLogonRequest") else None
                        if logonRequest:
                            data["type"] = logonRequest["type"]
                            data["requestId"] = logonRequest["requestId"]
                        ip = self.items[0]["ip"]
...
#                           raise("URL WDATP duplicate!")
                ...
                if id not in id_list["registry"]:
                    ...
                    if object != current:
                        ...
                        displayName = [i["DisplayName"] for i in current["alertIds"]]
                        for i in current["alertIds"]:
            ...
            elif current["type"] == "identityLogonEvent":
                id = current["Account object id"]
                if id not in id_list["identityLogonEvent"]:
                    id_list["identityLogonEvent"].append(id)
                    accumulator.append(current)
                else:
                    dbgprint.warning("Identity Logon id duplicate")
