...
def _extract_latest_resolved_prompt(params):
    try:
...
class ChatCompletionWrapper:
    ...
    def create(self, *args, **kwargs):
        ...
        with self.tracer.start_as_current_span("chat-completion-span") as span:
            latest_resolved_prompt = _extract_latest_resolved_prompt(params)
            if latest_resolved_prompt:
                self.tracer.mark_rag_query_trace_event(
                    PromptResolved(fully_resolved_prompt=latest_resolved_prompt)
                )
                ...
                span.set_attributes(
                    {
                        ...
                        "choices": str(log_response["choices"]),
                        **params_flat,
                    }
                )
                try:
