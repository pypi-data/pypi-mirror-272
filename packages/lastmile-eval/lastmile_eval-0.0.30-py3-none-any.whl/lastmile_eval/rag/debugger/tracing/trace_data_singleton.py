...
class TraceDataSingleton(Singleton):
    ...
    def add_rag_event(
        ...
        if span_id is None:
            raise ValueError(
                "Error adding rag event: could not extract span_id from event payload"
            )
        ...
        if span_id in self._added_spans:
            raise ValueError(
                f"You have already added an event for span '{span_id}'. Please check for other calls to `add_rag_event()` within the same span and either remove them, or explicitly pass the `span_id` argument in `add_rag_event()`."
            )
    ...
    def log_unstructured_rag_events(self, lastmile_api_token: str) -> None:
        ...
        for event_payload in self.rag_event_sequence:
            ...
            if SHOW_DEBUG:
                print(
                    f"TraceDataSingleton.log_unstructured_rag_events: {payload=}"
                )
