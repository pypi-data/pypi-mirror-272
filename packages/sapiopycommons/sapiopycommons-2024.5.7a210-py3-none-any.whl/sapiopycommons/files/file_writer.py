...
class FieldColumn(ColumnDef):
    ...
    field_name: str
    record_key: str | None
    cascade_down: bool = False

    def __init__(self, field_name: str, record_key: str | None = None, cascade_down: bool = True,
                 may_skip: bool = False):
        """
        ...
        :param cascade_down: If true, check the RowBundle's fields if the given field name doesn't exist in the
            RowBundle's records.
        :param may_skip: If true, this ColumnDef will return an empty string for RowBundles where may_skip is true.
        """
        self.field_name = field_name
        self.record_key = record_key
        self.cascade_down = cascade_down
        self.may_skip = may_skip
    ...
    def _get_field(self, row: RowBundle) -> Any:
        ...
        if record is None:
            return row.fields.get(self.field_name) if self.cascade_down else None
        fields: dict[str, Any] = AliasUtil.to_field_map_lists([record])[0]
        if self.field_name not in fields and self.cascade_down:
...
class DateColumn(FieldColumn):
    ...
    def __init__(self, field_name: str, time_format: str, timezone: str | None = None, record_key: str | None = None,
                 cascade_down: bool = False, may_skip: bool = False):
        """
        :param field_name: The name of the field in the RowBundle to get the value of.
        ...
        :param may_skip: If true, this ColumnDef will return an empty string for RowBundles where may_skip is true.
        """
        super().__init__(field_name, record_key, cascade_down, may_skip)
