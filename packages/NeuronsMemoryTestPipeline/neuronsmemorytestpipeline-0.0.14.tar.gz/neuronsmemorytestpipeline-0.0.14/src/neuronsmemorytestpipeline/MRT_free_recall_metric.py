...
def process_recall(df, filter_task, model, alias_df, group=['group_id']):
    ...
    for group in sorted(df.group_id.unique().tolist(), reverse=False):
        ...
        print('Before Identification of brands: \n', df_group[df_group.given_response_label_presented.isin(target_brands)].given_response_label_presented.value_counts())
        df_group_tocorrect = df_group[~df_group.given_response_label_presented.isin(target_brands)].copy()
        df_group  = df_group[df_group.given_response_label_presented.isin(target_brands)].copy()
        df_group['recalled_brand'] = df_group['given_response_label_presented']
        df_group_tocorrect, corrected_counts, corrected_df = MRT_text_processing.clean_free_recall(df_group_tocorrect, target_brands, responses_to_clean, model)
        df_group = pd.concat([df_group, df_group_tocorrect], axis=0, ignore_index=True)
    ...
    print('After Identification of brands: \n', df_cleaned[df_cleaned.recalled_brand.isin(target_brands)].recalled_brand.value_counts())
