...
def create_jpeg_image(
    ...
    if resp.returncode != 0:
        logger.error(
            f"JPEG creation failed >> command: {' '.join(command)} with error: {resp.stderr.decode('utf-8')}")
...
def reproject(
    ...
    """
    logger.debug(
        f"Reprojecting {input_file} to {target_crs} and writing to {output_file}")
    ...
    if resp.returncode != 0:
        logger.error(
            f"Reprojection failed >> command: {' '.join(command)} with error: {resp.stderr.decode('utf-8')}")
    else:
        logger.debug(f"Reprojecting completed")
