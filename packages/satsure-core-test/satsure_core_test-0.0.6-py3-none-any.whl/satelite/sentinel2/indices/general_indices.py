...
class GeneralIndices:
    ...
    def create_s2_normalised_index(self,
                                   ...
                                   cloud_mask_bands: Iterable[str] = (
                                           "B02", "B03"),
                                   cloud_mask_threshold: Iterable[int] = (
                                           3200, 3000),
        ...
        for unique_tile in unique_tiles:
            ...
            if not filename_band_A or not filename_band_B:
                logger.error(
                    f"Index creation skipped, bands A or B missing - A: {filename_band_A}, B: {filename_band_B}"
                )
                continue
            ...
            if not cloud_mask_bands_path:
                logger.error(
                    f"Cloud masking skipped, mask band files: {cloud_mask_bands} not found in {input_folder}"
                )
