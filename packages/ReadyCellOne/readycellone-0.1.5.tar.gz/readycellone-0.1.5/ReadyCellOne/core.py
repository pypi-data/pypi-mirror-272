...
def transcriptome_rank_transform(df, num_random_samples=0):
  '''
  Assign transcriptome-wide rankings for each gene within each sample

  @param df: pd.DataFrame, training data
  @param num_random_samples: int (default 0), number of random expression profiles to create during training.
  ...
  if num_random_samples > 0:
    ranked_expression = pd.concat([ranked_expression, randomize(ranked_expression, num=num_random_samples)])
