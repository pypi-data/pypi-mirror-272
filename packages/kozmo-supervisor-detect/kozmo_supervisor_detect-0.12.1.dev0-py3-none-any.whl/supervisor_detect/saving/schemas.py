"""
Pydantic models used by :func:`~supervisor_detect.utils.validate.validate_config` to validate configuration dictionaries.
The `resolved` kwarg of :func:`~supervisor_detect.utils.validate.validate_config` determines whether the *unresolved* or
...
import numpy as np
from supervisor_detect.utils.pydantic_v1 import BaseModel, validator

from supervisor_detect.utils.frameworks import Framework
from supervisor_detect.utils._types import (Literal, supported_models_all, supported_models_tf,
...
from supervisor_detect.saving.validators import NDArray, validate_framework, coerce_int2list, coerce_2_tensor
...
class ModelConfig(CustomBaseModel):
    ...
    :class:`~supervisor_detect.cd.tensorflow.preprocess.HiddenOutput` or
    :class:`~supervisor_detect.cd.pytorch.preprocess.HiddenOutput` model is returned (dependent on `flavour`).
...
class EmbeddingConfig(CustomBaseModel):
    ...
    :class:`~supervisor_detect.models.tensorflow.embedding.TransformerEmbedding`.
    """
...
class PreprocessConfig(CustomBaseModel):
    ...
    if `src` specifies :func:`~supervisor_detect.cd.tensorflow.preprocess.preprocess_drift`
    ...
    :func:`~supervisor_detect.cd.tensorflow.preprocess.preprocess_drift`:
    ...
    :class:`~supervisor_detect.utils.schemas.ModelConfig`.
    """
    ...
    reference, or a :class:`~supervisor_detect.utils.schemas.EmbeddingConfig`. If `model=None`, the `embedding` is passed to
    :func:`~supervisor_detect.cd.tensorflow.preprocess.preprocess_drift` as `model`. Otherwise, the `model` is chained to
    ...
    :class:`~supervisor_detect.utils.schemas.TokenizerConfig`.
    """
...
class KernelConfig(CustomBaseModelWithKwargs):
    ...
    If `src` specifies a :class:`~supervisor_detect.utils.tensorflow.GaussianRBF` kernel, the `sigma`, `trainable` and
    `init_sigma_fn` fields are passed to it. Otherwise, all fields except `src` are passed as kwargs.

    Examples
    --------
    A :class:`~supervisor_detect.utils.tensorflow.GaussianRBF` kernel, with three different bandwidths:

    .. code-block :: toml

        [kernel]
        src = "@supervisor_detect.utils.tensorflow.GaussianRBF"
        trainable = false
        sigma = [0.1, 0.2, 0.3]
    ...
    should match :py:func:`~supervisor_detect.utils.tensorflow.kernels.sigma_median`. If `None`, it is set to
    :func:`~supervisor_detect.utils.tensorflow.kernels.sigma_median`.
    """
    # Validators
    _validate_flavour = validator('flavour', allow_reuse=True, pre=False)(validate_framework)
...
class DeepKernelConfig(CustomBaseModel):
    """
    Unresolved schema for :class:`~supervisor_detect.utils.tensorflow.kernels.DeepKernel`'s.

    Examples
    --------
    A :class:`~supervisor_detect.utils.tensorflow.DeepKernel`, with a trainable
    :class:`~supervisor_detect.utils.tensorflow.GaussianRBF` kernel applied to the projected inputs and a custom
    ...
    model, specified as an object registry reference, or a :class:`~supervisor_detect.utils.schemas.ModelConfig`.
    """
    ...
    :class:`~supervisor_detect.utils.tensorflow.kernels.GaussianRBF` with trainable bandwidth.
    """
    kernel_b: Optional[Union[str, KernelConfig]] = "@utils.tensorflow.kernels.GaussianRBF"
    """
    The kernel to apply to the raw inputs. Defaults to a :class:`~supervisor_detect.utils.tensorflow.kernels.GaussianRBF`
...
class DriftDetectorConfig(DetectorConfig):
    ...
    in `.dill` format, an object registry reference, or a :class:`~supervisor_detect.utils.schemas.PreprocessConfig`.
    """
...
class KSDriftConfig(DriftDetectorConfig):
    ...
    `KSDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/ksdrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.KSDrift` documentation for a description of each field.
    """
    p_val: float = .05
...
class KSDriftConfigResolved(DriftDetectorConfigResolved):
    ...
    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.KSDrift` documentation for a description of each field.
    Resolved schema for the :class:`~supervisor_detect.cd.KSDrift` detector.
    """
    p_val: float = .05
...
class ChiSquareDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `ChiSquareDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/chisquaredrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.ChiSquareDrift` documentation for a description of each field.
    """
    p_val: float = .05
    preprocess_at_init: bool = True
    update_x_ref: Optional[Dict[str, int]] = None
...
class TabularDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `TabularDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/tabulardrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.TabularDrift` documentation for a description of each field.
    """
    p_val: float = .05
    preprocess_at_init: bool = True
    update_x_ref: Optional[Dict[str, int]] = None
...
class CVMDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `CVMDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/cvmdrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.CVMDrift` documentation for a description of each field.
    """
    p_val: float = .05
    preprocess_at_init: bool = True
    update_x_ref: Optional[Dict[str, int]] = None
...
class FETDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `FETDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/fetdrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.FETDrift` documentation for a description of each field.
    """
    p_val: float = .05
    preprocess_at_init: bool = True
    update_x_ref: Optional[Dict[str, int]] = None
...
class MMDDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `MMDDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/mmddrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.MMDDrift` documentation for a description of each field.
    """
...
class LSDDDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `LSDDDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/lsdddrift.html>`_ detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.LSDDDrift` documentation for a description of each field.
    """
...
class ClassifierDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `ClassifierDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/classifierdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.ClassifierDrift` documentation for a description of each field.
    """
...
class SpotTheDiffDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `SpotTheDiffDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/spotthediffdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.SpotTheDiffDrift` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    p_val: float = .05
    binarize_preds: bool = False
...
class LearnedKernelDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `LearnedKernelDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/learnedkerneldrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.LearnedKernelDrift` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch', 'keops'] = 'tensorflow'
    p_val: float = .05
...
class ContextMMDDriftConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `ContextMMDDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/contextmmddrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.ContextMMDDrift` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    p_val: float = .05
    c_ref: str
...
class MMDDriftOnlineConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `MMDDriftOnline <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/onlinemmddrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.MMDDriftOnline` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    ert: float
    window_size: int
...
class LSDDDriftOnlineConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `LSDDDriftOnline <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/onlinelsdddrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.LSDDDriftOnline` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    ert: float
    window_size: int
...
class CVMDriftOnlineConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `CVMDriftOnline <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/onlinecvmdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.CVMDriftOnline` documentation for a description of each field.
    """
    ert: float
...
class FETDriftOnlineConfig(DriftDetectorConfig):
    """
    Unresolved schema for the
    `FETDriftOnline <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/onlinefetdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.FETDriftOnline` documentation for a description of each field.
    """
    ert: float
    window_sizes: List[int]
    n_bootstraps: int = 10000
...
class ClassifierUncertaintyDriftConfig(DetectorConfig):
    """
    Unresolved schema for the
    `ClassifierUncertaintyDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/modeluncdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.ClassifierUncertaintyDrift` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    x_ref: str
    model: Union[str, ModelConfig]
...
class RegressorUncertaintyDriftConfig(DetectorConfig):
    """
    Unresolved schema for the
    `RegressorUncertaintyDrift <https://docs.kozmoai.io/projects/supervisor-detect/en/stable/cd/methods/modeluncdrift.html>`_
    detector.

    Except for the `name` and `meta` fields, the fields match the detector's args and kwargs. Refer to the
    :class:`~supervisor_detect.cd.RegressorUncertaintyDrift` documentation for a description of each field.
    """
    backend: Literal['tensorflow', 'pytorch'] = 'tensorflow'
    x_ref: str
    model: Union[str, ModelConfig]
...
# Unresolved schema dictionary (used in supervisor_detect.utils.loading)
...
# Resolved schema dictionary (used in supervisor_detect.utils.loading)
