...
from supervisor_detect.saving.registry import registry
from supervisor_detect.saving._tensorflow import load_detector_legacy, load_embedding_tf, load_kernel_config_tf, \
    load_model_tf, load_optimizer_tf, prep_model_and_emb_tf, get_tf_dtype
from supervisor_detect.saving._pytorch import load_embedding_pt, load_kernel_config_pt, load_model_pt, \
    load_optimizer_pt, prep_model_and_emb_pt, get_pt_dtype
from supervisor_detect.saving._keops import load_kernel_config_ke
from supervisor_detect.saving._sklearn import load_model_sk
from supervisor_detect.saving.validate import validate_config
from supervisor_detect.base import Detector, ConfigurableDetector, StatefulDetectorOnline
from supervisor_detect.utils.frameworks import has_tensorflow, has_pytorch, Framework
from supervisor_detect.saving.schemas import supported_models_tf, supported_models_torch
from supervisor_detect.utils.missing_optional_dependency import import_optional
get_device = import_optional('supervisor_detect.utils.pytorch.misc', names=['get_device'])
...
def _init_detector(cfg: dict) -> ConfigurableDetector:
    ...
    klass = getattr(import_module('supervisor_detect.cd'), detector_name)
...
def _load_preprocess_config(cfg: dict) -> Optional[Callable]:
    ...
    expected to match that generated by serialize_preprocess in supervisor_detect.utils.saving (also see pydantic schema).
...
def resolve_config(cfg: dict, config_dir: Optional[Path], enable_unsafe_loading: bool = False) -> dict:
    ...
    https://docs.kozmoai.io/projects/supervisor-detect/en/stable/overview/config_file.html.
            ...
            if src.startswith('@'):
                ...
                if src in registry.get_all():
                    ...
                    raise ValueError(
                        ...
                        "documentation at "
                        "https://docs.kozmoai.io/projects/supervisor-detect/en/stable/overview/getting_started.html."
                    )
