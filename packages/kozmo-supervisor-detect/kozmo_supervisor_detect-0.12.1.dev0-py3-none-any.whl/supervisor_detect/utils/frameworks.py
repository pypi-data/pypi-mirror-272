...
class BackendValidator:
    ...
    def verify_backend(self, backend: str):
        ...
        ----------
        backend
            Choice of backend the user wishes to initialize the supervisor-detect construct with. Must be one of the keys
        ...
        NotImplementedError
            ...
            implemented for an supervisor-detect object then it should not have a key on `self.backend_options`.
        ImportError
    ...
    def _raise_import_error(self, missing_deps: List[str], backend: str):
        ...
        pip_msg = '' if not optional_dependencies else \
            (f'The necessary missing dependencies can be installed using '
             f'`pip install supervisor-detect[{" ".join(optional_dependencies)}]`.')
