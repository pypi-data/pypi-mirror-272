...
import pandas as pd
import requests
from supervisor_detect.utils.data import Bunch
...
def fetch_ecg(return_X_y: bool = False) \
    ...
    """
    Xy_train = load_url_arff('https://storage.googleapis.com/kozmo-datasets/ecg/ECG5000_TRAIN.arff')
    X_train, y_train = Xy_train[:, :-1], Xy_train[:, -1]
    Xy_test = load_url_arff('https://storage.googleapis.com/kozmo-datasets/ecg/ECG5000_TEST.arff')
...
def fetch_cifar10c(corruption: Union[str, List[str]], severity: int, return_X_y: bool = False) \
    ...
    (corrupted data, target)
        Tuple if 'return_X_y' equals True.
    """
    url = 'https://storage.googleapis.com/kozmo-datasets/cifar10c/'
...
def corruption_types_cifar10c() -> List[str]:
    ...
    """
    url = 'https://storage.googleapis.com/kozmo-datasets/'
    folder = 'cifar10c'
    filetype = 'npy'
...
def load_genome_npz(fold: str, return_labels: bool = False) \
    ...
    url = 'https://storage.googleapis.com/kozmo-datasets/genome/'
...
def fetch_genome(return_X_y: bool = False, return_labels: bool = False) -> Union[Bunch, tuple]:
    ...
    found in the readme on https://console.cloud.google.com/storage/browser/kozmo-datasets/genome/.
    ...
    try:
        resp = requests.get('https://storage.googleapis.com/kozmo-datasets/genome/label_dict.json', timeout=TIMEOUT)
        resp.raise_for_status()
    except RequestException:
        logger.exception("Could not connect, URL may be out of service")
        raise
