...
import numpy as np
from supervisor_detect.utils.sampling import reservoir_sampling
...
def encompass_batching(
    ...
    if backend == Framework.TENSORFLOW:
        from supervisor_detect.cd.tensorflow.preprocess import preprocess_drift
    elif backend == Framework.PYTORCH:
        from supervisor_detect.cd.pytorch.preprocess import preprocess_drift  # type: ignore[assignment]
...
def get_input_shape(shape: Optional[Tuple], x_ref: Union[np.ndarray, list]) -> Optional[Tuple]:
    ...
    if isinstance(shape, tuple):
        ...
        logger.warning('Input shape could not be inferred. '
                       'If supervisor_detect.models.tensorflow.embedding.TransformerEmbedding '
