...
from supervisor_detect.utils._random import fixed_seed
from packaging import version
from pytest_cases import param_fixture, parametrize, parametrize_with_cases
from sklearn.model_selection import StratifiedKFold


from supervisor_detect.cd import (ChiSquareDrift, ClassifierUncertaintyDrift, RegressorUncertaintyDrift,
...
from supervisor_detect.models.pytorch import TransformerEmbedding as TransformerEmbedding_pt
from supervisor_detect.models.tensorflow import TransformerEmbedding as TransformerEmbedding_tf
from supervisor_detect.saving import (load_detector, read_config, registry,
                                 resolve_config, save_detector, write_config)
from supervisor_detect.saving.loading import (_get_nested_value, _replace,
                                         _set_dtypes, _set_nested_value, _prepend_cfg_filepaths)
from supervisor_detect.saving.saving import _serialize_object
from supervisor_detect.saving.saving import (_path2str, _int2str_keys, _save_kernel_config, _save_model_config,
                                        _save_preprocess_config)
from supervisor_detect.saving.schemas import DeepKernelConfig, KernelConfig, ModelConfig, PreprocessConfig
from supervisor_detect.utils.pytorch.kernels import DeepKernel as DeepKernel_pt
from supervisor_detect.utils.tensorflow.kernels import DeepKernel as DeepKernel_tf
from supervisor_detect.utils.frameworks import has_keops
if has_keops:  # pykeops only installed in Linux CI
    from pykeops.torch import LazyTensor
    from supervisor_detect.utils.keops.kernels import DeepKernel as DeepKernel_ke

if version.parse(scipy.__version__) >= version.parse('1.7.0'):
    from supervisor_detect.cd import CVMDrift
...
def test_version_warning(data, tmp_path):
    ...
    different supervisor_detect version, then saved, then loaded again (warning is still expected on final load).
...
def test_registry_get():
    """
    Unit test for supervisor_detect.utils.registry.get(). This will make more sense once we have a more automated
    process for pre-registering supervisor-detect objects, as then can compare against list of objects we wish to register.
    """
