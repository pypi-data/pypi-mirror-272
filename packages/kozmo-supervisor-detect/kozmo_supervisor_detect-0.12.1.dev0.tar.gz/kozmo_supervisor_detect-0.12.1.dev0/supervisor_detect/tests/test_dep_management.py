...
These tests import all the named objects from the public API of supervisor-detect and test that they throw the correct errors
...
1. The optional dependency relation hasn't been added to the test script. In this case, this test assumes that the
    functionality should work for the default supervisor-detect install. If this is not the case the exported object name
...
2. The relevant export in the public API hasn't been imported using `optional_import` from
    `supervisor_detect.utils.missing_optional_dependency`.
...
def check_correct_dependencies(
    ...
    dependencies
        ...
        should pass for the default supervisor-detect install.
    opt_dep
...
def test_cd_dependencies(opt_dep):
    ...
    from supervisor_detect import cd
...
def test_cd_torch_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.cd import pytorch as cd_pytorch
...
def test_cd_tensorflow_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.cd import tensorflow as tensorflow_cd
...
def test_ad_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect import ad
...
def test_od_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect import od
...
def test_od_backend_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.od import pytorch as od_pt_backend
...
def test_tensorflow_model_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.models import tensorflow as tf_models
...
def test_torch_model_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.models import pytorch as torch_models
...
def test_dataset_dependencies(opt_dep):
    ...
    for dependency, relations in []:
        dependency_map[dependency] = relations
    from supervisor_detect import datasets
...
def test_fetching_utils_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.utils import fetching
    check_correct_dependencies(fetching, dependency_map, opt_dep)


def test_saving_tf_dependencies(opt_dep):
    """Tests that the supervisor_detect.saving._tensorflow module correctly protects against uninstalled optional
    dependencies.
    """
    dependency_map = defaultdict(lambda: ['default'])
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.saving import _tensorflow as tf_saving
    check_correct_dependencies(tf_saving, dependency_map, opt_dep)


def test_saving_torch_dependencies(opt_dep):
    """Tests that the supervisor_detect.saving._pytorch module correctly protects against uninstalled optional
    dependencies.
    """
    dependency_map = defaultdict(lambda: ['default'])
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.saving import _pytorch as pt_saving
    check_correct_dependencies(pt_saving, dependency_map, opt_dep)


def test_saving_dependencies(opt_dep):
    """Tests that the supervisor_detect.saving module correctly protects against uninstalled optional dependencies."""
    dependency_map = defaultdict(lambda: ['default'])
    for dependency, relations in []:
        dependency_map[dependency] = relations
    from supervisor_detect import saving
...
def test_tensorflow_utils_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.utils import tensorflow as tensorflow_utils
...
def test_torch_utils_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.utils import pytorch as pytorch_utils
...
def test_keops_utils_dependencies(opt_dep):
    ...
    for dependency, relations in [
        ...
        dependency_map[dependency] = relations
    from supervisor_detect.utils import keops as keops_utils
