...
# reference: https://gist.github.com/mikeputnam/2820675
NOTE_B0 = 31
NOTE_C1 = 33
NOTE_CS1 = 35
NOTE_D1 = 37
...
def pinMode(pin, mode):
    ...
    # 2. https://mryslab.github.io/pymata4/pin_modes/#set_pin_mode_analog_input
        ...
        if mode == INPUT:
            board.set_pin_mode_analog_input(integer_only)

        else:
            digital_pin_equivalent = DIGITAL_PINS_COUNT + integer_only
            if mode == OUTPUT:
                board.set_pin_mode_digital_output(digital_pin_equivalent)

            elif mode == INPUT_PULLUP:
                board.set_pin_mode_digital_input_pullup(digital_pin_equivalent)
...
def analogWrite(pin, val):
    if isinstance(pin, str) and pin.startswith('A'):
        pin = analogToDigital(pin)  # convert analog pin to digital pin equivalent
    board.set_pin_mode_pwm_output(pin)  # set the pin's mode as a pwm output pin
    board.pwm_write(pin, val)  # write an output value
...
def digitalRead(pin):
    if isinstance(pin, str) and pin.startswith('A'):
        pin = analogToDigital(pin)  # convert analog pin to digital pin equivalent
        board.set_pin_mode_digital_input(pin)  # set the pin's mode as a digital input pin
    val = board.digital_read(pin)  # read the digital value
    return val
...
def analogRead(pin):
    if isinstance(pin, str) and pin.startswith('A'):
        pin = int(pin[1:])
    val = board.analog_read(pin)  # read the analog value
    return val
...
def ultrasonicRead(trigger_pin):
    val = board.sonar_read(trigger_pin)
    return val
...
def servoWrite(pin, position):
    board.servo_write(pin, position)
...
def tone(pin, frequency, duration=None):
    if duration is None:
        board.play_tone_continuously(pin, frequency)
    else:
        board.play_tone(pin, frequency, duration)
