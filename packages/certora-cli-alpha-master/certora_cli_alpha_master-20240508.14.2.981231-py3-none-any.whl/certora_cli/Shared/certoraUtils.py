        ...
        if PACKAGE_FILE.exists():
            try:
                with PACKAGE_FILE.open() as package_json_file:
                    ...
                    dependencies_list = []
                    devDependencies_list = []
                    if "dependencies" in package_json:
                        dependencies_list = list(package_json["dependencies"].keys())
                    if "devDependencies" in package_json:
                        devDependencies_list = list(package_json["devDependencies"].keys())
                    deps = dependencies_list + devDependencies_list
                    if len(set(deps)) < len(deps):
                        raise CertoraUserInputError(f"package.json includes duplicated dependencies in: {deps}")
