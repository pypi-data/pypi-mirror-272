...
def _load_model_v2(filename: str, classname: str) -> Tuple[_ModelServer, BaseModel, BaseModel, bool]:
...
def _run_model_server(model: _ModelServer, remote: bool, port: int = 8000, api_name: str = None, inputType: TypeVar = InputDataType, outputType: TypeVar = OutputDataType):
    """Boot simple server that serves the model."""
    from prometheus_client import Counter, Gauge, Histogram, make_asgi_app
    app = FastAPI(
        title=api_name,
        description="This is a simple model serving API. " 
        +" Add `X-AUTH-KEY` header you set as `SERVING_AUTH_KEY` in your environment. " if remote else ""+ "You can access the metrics at `/metrics`. "
