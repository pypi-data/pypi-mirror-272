...
def revision_show(service: ResponseModelServiceInfo, number: int, format: str):
    ...
    if format == "yaml":
        autoscaling = revision.revision_spec.autoscaling
        service_form = V1ServeRevisionServiceForm(
            expose=revision.revision_spec.expose,
            healthcheck=V1PortPath(path=revision.revision_spec.readiness_probe.path,port=int(revision.revision_spec.readiness_probe.port) if revision.revision_spec.readiness_probe.port else None) if revision.revision_spec.readiness_probe else None,
            monitoring=[V1PortPath(path=x.path,port=int(x.port) if x.port else None) for x in revision.revision_spec.monitoring] if revision.revision_spec.monitoring else None,
            autoscaling=V1Autoscaling(min=autoscaling.min, max=autoscaling.max, metric=autoscaling.metric, target=autoscaling.target) if autoscaling else None,
            auxiliary=V1AuxiliaryForm(
                runner_type=revision.revision_spec.runner_type,),
        ).to_dict()

        vops = _translate_vops(revision.volume_operations) if revision.volume_operations else None
        form = {
            "name":service.name,
            "message": revision.message,
            "env": _translate_envvars(revision.revision_spec.env_vars),
            "image": revision.kernel_image.image_url,
            "resources": V1ResourcesForm(
                cluster=service.kernel_cluster.name,
                preset=revision.kernel_resource_spec.name
            ).to_dict() if revision.kernel_resource_spec else None,
            "run": [
                V1Command(
                    command=x.command,
                    workdir=x.workdir,
                ).to_dict() for x in revision.revision_spec.command
            ] if revision.revision_spec.command else None,
            "ports": [V1PortPath(port=int(x.source_port)).to_dict() for x in revision.revision_spec.ports] if revision.revision_spec.ports else None,
            "service":service_form,
        }
        if vops:
            if "import" in vops:
                form["import"] = vops["import"]
            if "mount" in vops:
                form["mount"] = vops["mount"]
        sanitized = _sanitize(form)
