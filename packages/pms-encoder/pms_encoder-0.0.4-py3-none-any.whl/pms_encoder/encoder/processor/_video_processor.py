...
from ..utils._queue_object_generator import QueueObjectGenerator
from ..utils._video_ffmpeg_cmd_generator import *
from ..utils._calculate_progress import check_progress
from ..utils._sub_output_util import zip_directory
...
def split_frames(
    ...
    object_generator = QueueObjectGenerator(
        process,
        redis_data,
        meta_data,
    )
...
def merge_audio(
    ...
    if audio == "N":
        shutil.move(
            "/app/tmp/{}_tmp.{}".format(redis_data["fileKey"], redis_data["format"].lower()),
            "/app/tmp/{}.{}".format(redis_data["fileKey"], redis_data["format"].lower())
        )
    else:
...
def save_inferenced_meta(
    ...
    probe = ffmpeg.probe("/app/tmp/{}.{}".format(redis_data["fileKey"], redis_data["format"].lower()))
...
def downscale_sub_output(
    ...
    output_file_path = "/app/tmp/{}.{}".format(redis_data["fileKey"], redis_data["format"].lower())
    zip_file_path = "/app/tmp/{}.zip".format(redis_data["fileKey"])
    if redis_data.get("subOutput") == "Y":
        # make temp directory
        if not os.path.exists("/app/tmp/{}".format(redis_data["fileKey"])):
            os.makedirs("/app/tmp/{}".format(redis_data["fileKey"]))
        # generate sub_output
        last_dot_index = redis_data["fileName"].rfind('.')
        original_file_name = redis_data["fileName"][:last_dot_index] 
        sub_output_path = "/app/tmp/{}/{}_{}x{}.{}".format(
            redis_data["fileKey"],
            original_file_name,
        ...
        shutil.move(
            output_file_path,
            "/app/tmp/{}/{}.{}".format(redis_data["fileKey"], original_file_name, redis_data["format"])
        )
        
        # zip
        zip_directory("/app/tmp/{}".format(redis_data["fileKey"]), zip_file_path)
        
        # move download file
        download_zip_file_path = "/app/files/{}.zip".format(redis_data["fileKey"])
        shutil.move(
            zip_file_path,
        ...
        shutil.rmtree("/app/tmp/{}".format(redis_data["fileKey"]))
        
    else:
        download_file_path = "/app/files/{}.{}".format(redis_data["fileKey"], redis_data["format"].lower())
        shutil.move(
            output_file_path,
            download_file_path
        )
