...
class CompoundMessage(GenericCRUD, metaclass=MetaLogger, object=object_message):
    ...
    def get_queue(self):
        if not self.queue:
            self.queue = DatabaseQueue(schema_name="message", table_name="message_table",
                                   view_name="message_view", column_name="message_id",
    ...
    def _get_template_ids_by_campaign(self, campaign_id: int) -> list[int]:
        ...
        possible_template_ids = self.select_multi_value_by_where(
    ...
    def set_compound_message_after_text_template(
        ...
        if message_id:
            ...
            compound_message = self.select_one_value_by_column_and_value(select_clause_value="compound_message_json",
                                                           column_value=message_id)
        ...
        if body:  # TODO constract textblocks_and_attributes if body is given
            ...
            if campaign_id and not message_template_id:
                ...
                self.logger.info("Choosed random message_template_id", object={
                    "possible_template_ids": possible_template_ids, "message_template_id": message_template_id})
