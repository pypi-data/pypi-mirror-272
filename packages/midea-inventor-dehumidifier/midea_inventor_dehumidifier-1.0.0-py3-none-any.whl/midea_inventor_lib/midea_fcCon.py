...
class fcCon:
    ...
    def __init__(self):
        ...
        self.f2050b = 160
        self.DEHUMI = -95
        self.DEHUMI_STR = "0xA1"  # Dehumidification device
        self.HUMI = -3
        self.crc8_854_table = [0, 94, 188, 226, 97, 63, 221, 131, 194, 156, self.KEYCODE_MEDIA_PLAY, 32, 163, 253, 31,
                               65, 157, 195, 33, self.KEYCODE_MEDIA_PAUSE, 252, 162, 64, 30, 95, 1, 227, 189, 62, 96,
                               130, 220, 35, 125, 159, 193, 66, 28, 254, self.f2050b, 225, 191, 93, 3, 128, 222, 60, 98,
                               190, 224, 2, 92, 223, 129, 99, 61, 124, 34, 192, 158, 29, 67, 161, 255, 70, 24, 250, 164,
                               39, 121, 155, 197, 132, 218, 56, 102, 229, 187, 89, 7, 219, 133, 103, 57, 186, 228, 6,
                               88, 25, 71, 165, 251, 120, 38, 196, 154, 101, 59, 217, 135, 4, 90, 184, 230, 167, 249,
                               27, 69, 198, 152, 122, 36, 248, 166, 68, 26, 153, 199, 37, 123, 58, 100, 134, 216, 91, 5,
                               231, 185, 140, 210, 48, 110, 237, 179, 81, 15, 78, 16, 242, 172, 47, 113, 147, 205, 17,
                               79, 173, 243, 112, 46, 204, self.VERSION_CODE, 211, 141, 111, 49, 178, 236, 14, 80, 175,
                               241, 19, 77, 206, 144, 114, 44, 109, 51, 209, 143, 12, 82, 176, 238, 50, 108, 142, 208,
                               83, 13, 239, self.REQUEST_CONTROL_AVOID, 240, 174, 76, 18, 145, 207, 45, 115, 202, 148,
                               118, 40, 171, 245, 23, 73, 8, 86, 180, 234, self.SET_WEATHER_REQUEST_CODE, 55, 213, 139,
                               87, 9, 235, 181, 54, 104, 138, 212, 149, 203, 41, 119, 244, 170, 72, 22, 233, 183, 85,
                               11, self.BAR_HEIGHT, 214, 52, self.SET_WEATHER_RESULT_CODE, 43, 117, 151, 201, 74, 20,
        ...
        # Not used by Dehumidification device
    ...
    def stdAirConEx_pro2byte(self, devType, msgType, tmpBuf, dstLen):
        ...
        # DataBodyDeHumiRequest
            ...
            tmpBuf[0] = 72
            tmpBuf[1] = (((((((self.btnSound & 1) << 6) | ((self.quickChkSts & 1) << 5)) | (
                    (self.timingType & 1) << 4)) | ((self.childSleepMode & 1) << 3)) | ((self.iMode & 1) << 2)) | (
                                 (self.controlSource & 1) << 1)) | ((self.runStatus & 1) << 0)
            ...
            tmpBuf[9] = (((((self.displayClass & 7) | (self.pumpSwitch << 3)) | (self.pumpSwitch_flag << 4)) | (
                    self.sleepSwitch << 5)) | (self.ionSetSwitch << 6)) | (self.filterShow << 7)
    ...
    def toString(self):
        return "Dehumidifier Model Water tank = " + str(self.tankShow) + "  swing = " + str(
            self.upanddownSwing) + "  Negative ion = " + str(self.ionSetSwitch) + "  Power on state = " + str(
            self.runStatus) + "  Set humidity = " + str(self.humidity) + "  Set humidity fraction = " + str(
            self.humidity_set_dot) + "  Humidity received = " + str(
            self.humidity_cur) + "  Humidity fraction received = " + str(
            self.humidity_cur_dot) + "  Wind speed = " + str(self.windSpeed) + "  mode = " + str(
            self.mode_F1_return) + "  timing = " + str(self.timingIsValid) + "  Timed on = " + str(
            self.timingOnSwitch) + "  Timed H = " + str(self.timingOnHour) + "  Timed M = " + str(
            self.timingOnMinute) + "  Timing off = " + str(self.timingOffSwitch) + "  Timing off H = " + str(
            self.timingOffHour) + "  Timig off M = " + str(self.timingOffMinute)
