...
class CameraDevice(Protocol):
    ...
    # async def stream_video(self, source_name: str, options: Any) -> None: ...  # todo
    # async def record_to_file(
    #     self, source_name: str, output_path: str, duration: Optional[int] = 60  # todo
    # ) -> None: ...
    async def get_value(
        self, state_name: str
    ) -> Union[
...
class ToastMessage(TypedDict, total=False):
    toast: str
...
class BasePlugin(ABC):
    ...
    @abstractmethod
    def on_form_submit(self, action_id: str, payload: Any) -> ToastMessage: ...
