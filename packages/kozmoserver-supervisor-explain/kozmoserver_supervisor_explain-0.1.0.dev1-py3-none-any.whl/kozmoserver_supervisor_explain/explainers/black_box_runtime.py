...
from kozmoserver import ModelSettings
from kozmoserver.codecs import NumpyCodec
from kozmoserver.types import MetadataModelResponse
from kozmoserver_supervisor_explain.common import (
...
from kozmoserver_supervisor_explain.runtime import SupervisorExplainRuntimeBase


class SupervisorExplainBlackBoxRuntime(SupervisorExplainRuntimeBase):
    """
    ...
    def _explain_impl(self, input_data: Any, explain_parameters: Dict) -> Explanation:
        if not self.supervisor_explain_settings.explainer_batch:
    ...
    def _infer_impl(self, input_data: Union[np.ndarray, List[str]]) -> np.ndarray:
        # The contract is that supervisor-explain would input/output ndarray
        ...
        v2_request = to_v2_inference_request(
            ...
            output=self.supervisor_explain_settings.infer_output,
        )
        ...
        if decoded.ndim > 1 and decoded.shape[-1] == 1:
            ...
            # explicit dimensionality shape (which Supervisor doesn't seem to like)
