...
from kozmoserver.codecs import StringCodec, NumpyCodec
from kozmoserver.types import (
    ResponseOutput,
    InferenceResponse,
    InferenceRequest,
    Parameters,
...
from kozmoserver.utils import generate_uuid

from kozmoserver_supervisor_explain.errors import RemoteInferenceError, InvalidExplanationShape

_DEFAULT_INPUT_NAME = "predict"

EXPLAINER_TYPE_TAG = "explainer_type"

ENV_PREFIX_SUPERVISOR_EXPLAIN_SETTINGS = "KOZMOSERVER_MODEL_SUPERVISOR_EXPLAIN_"
EXPLAIN_PARAMETERS_TAG = "explain_parameters"
KOZMO_SKIP_LOGGING_HEADER = "Kozmo-Skip-Logging"
...
def remote_predict(
    ...
    response_raw = requests.post(
        ...
        headers={KOZMO_SKIP_LOGGING_HEADER: "true"},
        verify=verify,
    )
...
class SupervisorExplainSettings(BaseSettings):
    """
    Parameters that apply only to supervisor explain models
    """

    class Config:
        env_prefix = ENV_PREFIX_SUPERVISOR_EXPLAIN_SETTINGS
...
def to_v2_inference_request(
    ...
    # KozmoServer does not really care about a correct input name!
    ...
    v2_request = InferenceRequest(
        ...
        # TODO: we probably need to tell supervisor about the expected types to use
