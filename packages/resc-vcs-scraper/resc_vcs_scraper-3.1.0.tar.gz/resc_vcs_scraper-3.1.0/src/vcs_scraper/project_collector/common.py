...
def collect_projects_from_vcs_instance(vcs_instance: VCSInstance) -> None:
    ...
    if vcs_instance.scope:
        logger.info(
            f"Checking if all projects within the scope exist in {vcs_client.url}"
        )
        for project_in_scope in vcs_instance.scope:
            if vcs_client.project_exists(project_in_scope):
                logger.debug(
                    f"Project '{project_in_scope}' is present in {vcs_client.url}"
                )
                projects_to_scrape.append(project_in_scope)
            else:
                logger.warning(
                    f"Project '{project_in_scope}' is not present in {vcs_client.url}"
                )
    ...
    logger.info(
        f"{projects_to_scrape_count} projects fetched successfully from '{vcs_client.url}'."
    )
    ...
    for project_name in projects_to_scrape:
        if vcs_instance.exceptions:
            if project_name in vcs_instance.exceptions:
                logger.info(
                    f"Skipping project {project_name} because it is within the exceptions."
                )
                continue
        try:
            for attempt in Retrying(
                ...
                with attempt:
                    try:
                        ...
                        logger.error(
                            f"{exc} sending '{project_name}' to 'projects'. Retrying ..."
                        )
                        raise
            projects_sent += 1
            ...
            raise SystemExit(
                f"Error while sending project '{project_name}' to the 'projects' : "
                f"retry timed out"
            ) from exc
