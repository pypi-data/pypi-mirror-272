...
def extract_project_information(project_key, vcs_client, vcs_instance_name):
    ...
    """
    logger.info(
        f"Fetching repositories for project: '{project_key}' in vcs instance '{vcs_instance_name}'"
    )
    project_repositories = vcs_client.get_repos(project_key)
    logger.info(
        f"Fetched {len(project_repositories)} repositories for project: '{project_key}'"
    )
    project_tasks = []
    for repository in project_repositories:
        try:
            logger.info(
                f"Fetching latest commit for repository: '{project_key}/{repository['name']}'"
            )
            latest_commit = vcs_client.get_latest_commit(
                project_key=project_key, repository_id=repository["name"]
            )
            task_parameters = vcs_client.export_repository(
                repository, latest_commit, vcs_instance_name
            )
            if latest_commit:
                project_tasks.append(task_parameters)
                logger.info(
                    f"Information for repository: '{project_key}/{repository['name']}' "
                    f"was fetched successfully"
                )
            else:
                # Repository has no commits, will not forward to scanner
                logger.info(
                    f"Repository: '{project_key}/{repository['name']}' has no commits, skipping"
                )
...
def send_tasks_to_celery_queue(
    task_name: str, queue_name: str, project_tasks: List[Repository]
):
    for task in project_tasks:
        celery_client.send_task(
            task_name, kwargs={"repository": task.json()}, queue=queue_name
        )
...
def collect_repositories(project_key, vcs_instance_name):
    ...
    """
    vcs_instances_map = load_vcs_instances_into_map(
        env_variables[VCS_INSTANCES_FILE_PATH]
    )

    logger.info(f"Project processor received an azure project key: '{project_key}'")

    vcs_client = VCSConnectorFactory.create_client_from_vcs_instance(
        vcs_instances_map[vcs_instance_name]
    )
    project_tasks = extract_project_information(
        project_key, vcs_client, vcs_instance_name
    )
    send_tasks_to_celery_queue(
        SECRET_SCANNER_TASK_NAME, REPOSITORY_QUEUE, project_tasks
    )
