...
def create_celery_client(
    queue_name: str, username: str, password: str, service_host: str, virtual_host: str
):
    celery_client = Celery(
        queue_name, broker=f"amqp://{username}:{password}@{service_host}/{virtual_host}"
    )
...
def load_vcs_instances_into_map(file_path: str) -> Dict[str, VCSInstance]:
    ...
    if not vcs_instances:
        logger.info(
            "Exiting due to issues in VCS Instances definition in file " f"{file_path}"
        )
        sys.exit(-1)
