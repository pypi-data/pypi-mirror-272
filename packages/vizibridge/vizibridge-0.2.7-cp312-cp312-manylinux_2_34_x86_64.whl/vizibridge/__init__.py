...
Pykmers = [getattr(rust_types, a) for a in dir(rust_types) if a.startswith("PyKmer") or a.startswith("PyLongKmer")]
...
class DNA:
    ...
    def enum_canonical_kmer(self, k: int) -> Iterator["Kmer"]:
        if k <= 31:
            ...
            return map(
                lambda e: Kmer(e, k), getattr(self.data, f"enumerate_canonical_long_kmer{k-32}")()
            )
    ...
    def enum_kmer(self, k: int) -> Iterator["Kmer"]:
        if k <=31:
            ...
            return map(lambda e: Kmer(e, k), getattr(self.data, f"enumerate_canonical_long_kmer{k-32}")())
        raise NotImplementedError(f"{k}>63 is not supported")
...
class Kmer:
    ...
    def from_dna(cls, seq: DNA) -> Self:
        return cls(list(seq.enum_kmer(len(seq)))[0])
