...
def depend_cache_html_file(reload_freq: ReloadFreqEnum = ReloadFreqEnum.daily) -> Callable:
    """
    依赖（缓存html文件）
    :param reload_freq:
    :return:
    ...
    async def func_wrap(request: Request) -> str:
        request_id = generate_request_id(request)
        cache_html = os.path.join(settings_tools.get_config('server.template_path'), 'cache', f'{request_id}.html')
