...
def download_file(ali_api: CustomAligo, pan_file: BaseFile, local_path: str,
                  ...
                  delete_to_trash: bool = False) -> str:
    """
    下载文件/文件夹
    :param ali_api: Aligo
    :param pan_file: 网盘文件
    ...
    if pan_file.type == 'file':
        ...
        if not download_url:
            ...
            save_result = ali_api.share_file_saveto_drive(file_id=pan_file.file_id, share_token=share_token)
            if hasattr(save_result, 'message'):
                raise Exception(save_result.message)
            pan_file = ali_api.get_file(file_id=save_result.file_id)
        local_file = ali_api.download_file(file=pan_file, local_folder=local_path)
        if os.path.exists(local_file):
            if os.stat(local_file).st_size == 350:
                os.remove(local_file)
                local_file = ali_api.download_file(file=pan_file, file_id=pan_file.file_id, local_folder=local_path)
            elif download_after_delete:
                if delete_to_trash:
                    ali_api.move_file_to_trash(file_id=pan_file.file_id)
                else:
                    ali_api.delete_file(file_id=pan_file.file_id)
            return local_file
    else:
