...
class Output(OutputBase, OutputStub):
    ...
    def index(self, index: int, text: str, max_index: int | None = None) -> None:
        self.write_line(self.index_str(index, text, max_index))
    ...
    def write_result(
        ...
        if e(data):
            ...
            if ne(title):
                self.write_line(StringTool.bold(title))
                    ...
                    if n(label_function):
                        ...
                        for field in result.fields.list:
                            ...
                            default_value_label_function: Callable[
                                ...
                                True,
                                j((StringTool.bold(field.caption), ": ", data_value)),
                            )
...
class Input(InputBase):
        ...
        with self.output.make_indent(1, True):
            while True:
                if (
                    ...
                    # data_item_label_to_item_map: dict[str, Any] = {}
                    ...
                    for index, item in enumerate(data):
                        ...
                        # data_item_label_to_item_map[data_label] = item
                selected_index = nni(
                    PIH.DATA.EXTRACT.decimal(
                        value := self.input(j((index_caption, text_after))),
                        min_value,
                        max_value,
                        simple=True,
    ...
    def item_by_index(
        ...
        index: int = self.index(
            ...
            # simple_integer_parse=simple_integer_parse
        )
    ...
    def datetime(
        ...
        while use_date and True:
            date_value: str | None = self.input(
                js(("Введите дату в формате", StringTool.bold("День.Месяц.Год")))
            ).lower()
        ...
        if ask_time_input and self.yes_no(js("Ввести время")):
            ...
            while use_time and True:
                time_value: str | None = self.input(
                    js(("Введите время в формате", StringTool.bold("Час:Минуты")))
                )
    ...
    def choose_file(
        ...
        def label_function(file: File, _) -> str:
            ...
            if len(title_list) == 3:
                return j(
                    (
                        StringTool.bold(title_list[0].upper()),
                        ": ",
                        title_list[-1],
                        " (",
                        title_list[-2],
            ...
            return StringTool.bold(title_list[1])
    ...
    def choose_note(
        ...
        with self.output.make_loading(1, "Идет загрузка заметок. Ожидайте"):
            ...
            def label_function(note: Note, _) -> str:
                return StringTool.bold(note.title)
...
class PIH:
    ...
    class UPDATER:
        ...
        def package_exists(name: str) -> bool:
            ...
            result: bool = complete_process.stdout.find(name) != -1
            """
        ...
        def package_operation(
            ...
            if n(PIH.CACHE.get(PIH.UPDATER.__name__, cache_name)):
                ...
                PIH.CACHE.set(cache_name, result)
            return result
    ...
    class DATA(
        ...
        class FORMAT:
            ...
            def console_message(text: str) -> str:
                return (
                    text.replace("<b>", Style.BRIGHT)
                    .replace("</b>", Style.RESET_ALL)
                    .replace("<i>", "")
                    .replace("</i>", "")
                    .replace("<li>", "")
                    .replace("</li>", "")
            ...
            def whatsapp_message(value: str | None) -> str | None:
                if n(value):
                    return value
                b: str = CONST.MESSAGE.WHATSAPP.STYLE.BOLD
                i: str = CONST.MESSAGE.WHATSAPP.STYLE.ITALIC
                return (
                    value.replace("<b>", b)
                    .replace("</b>", b)
                    .replace("<i>", i)
            ...
            def wappi_status(
                ...
                title: str = j(
                    (
                        ...
                        ": ",
                        StringTool.bold(value.name or ("" if n(profile) else profile.name)),  # type: ignore
                        nl(),
                    )
                )
                if value.authorized:
        ...
        class WHATSAPP:
            class WAPPI:
                ...
                def get_message_list(
                    ...
                    profile, profile_value = (
                        PIH.MESSAGE.WHATSAPP.WAPPI._get_profile_and_value(profile)
                    )
                    url: str = j(
                        (
                ...
                def send_location(
                    ...
                    payload: strdict = {"recipient": telephone_number}
                    payload["latitude"] = value[0]
                    payload["longitude"] = value[1]
                    payload["address"] = address
                    url: str = j((CONST.MESSAGE.WHATSAPP.WAPPI.URL_SEND_LOCATION, profile_value))
                    try:
                ...
                def send(
                    ...
                    if isinstance(value, str):
                        ...
                        for item_name in value.__dataclass_fields__:
                            item_value: Any = value.__getattribute__(item_name)
                            if ne(item_value):
                        ...
                        if isinstance(value, WhatsAppMessageListPayload):
