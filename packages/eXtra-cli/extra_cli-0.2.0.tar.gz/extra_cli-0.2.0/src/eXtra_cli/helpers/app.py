...
def get_app_paths(base_path: str | PathLike[str] | None = None) -> AppPaths:
    ...
    if not base_path:
        ...
        while True:
            if base_path.joinpath("eXtra.yaml").is_file():
                break

            if base_path == Path("/"):
                console().print_error(f"Could not find {format_path('eXtra.yaml')} in the current directory or "
                                      f"any of its parents. Please run {format_command('eXtra init')} to create one.")
                raise Exit(code=1)

            base_path = base_path.parent

    config_file = Path(base_path) / "eXtra.yaml"
