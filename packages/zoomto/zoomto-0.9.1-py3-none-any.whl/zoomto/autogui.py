...
def shareVideo(address : str, monitor = 0):
    ...
    if hasattr(conf, "custom_auto_sharevideo"):
        getattr(conf, "custom_auto_sharevideo")(selectAWnd)
    else:
        click_advanced = getattr(conf, "sharevideo_click_advanced", (selectAWnd.topleft[0] + selectAWnd.width*3/10, selectAWnd.topleft[1] +50))
        pg.doubleClick(*click_advanced)

        click_video = getattr(conf, "sharevideo_click_video", (selectAWnd.topleft[0]+ selectAWnd.width/8, selectAWnd.centery))
        pg.doubleClick(*click_video)
    sleep(0.1)
    # get open wnd
    try:
