...
class ContextRecall:
    ...
    def score(self, dataset: Dataset, sample_kwargs: Dict[str, Any] = None):
        ...
        reasoning = self.REASONING_MESSAGE.format(ideal=gt, context=ctx, statements="\n".join(classification), final_score=result_score)
    ...
    def classify_context_recall(self, context: str, ideal: str, sample_kwargs: dict = None) -> list[str]:
        ...
        prompt = self.CONTEXT_RECALL_RA.format(context=context, ideal=ideal)
        ...
        attribute_answer = self.parse_response(attribute_response)
    ...
    def calculate_score(self, classification: list[str]) -> float:
        verdict_token = "[归因]"
        numerator = sum(verdict_token in sentence for sentence in classification)
