...
class Faithfulness:
    ...
    def score(self, dataset: Dataset, sample_kwargs: Dict[str, Any] = None):
        ...
        human_prompt = self.LONG_FORM_ANSWER_PROMPT.format(question=question, answer=answer)
        ...
        logging.info("sample_kwargs: %s", sample_kwargs)
        
        start_time = time.time() # 开始计时
        statements_response = ModelCaller.call(self, messages=messages, **sample_kwargs)
        end_time = time.time() # 结束计时
        elapsed_time = end_time - start_time # 计算经过的时间
        
        logging.info("statements_response: %s", statements_response)
        statements = self.parse_response(statements_response)
        ...
        nli_prompt = self.NLI_STATEMENTS_MESSAGE.format(context=context, statements=statements)
        ...
        start_time = time.time() # 开始计时
        nli_response = ModelCaller.call(self, messages=messages, **sample_kwargs)
        end_time = time.time() # 结束计时
        elapsed_time = end_time - start_time # 计算经过的时间
        
        logging.info("nli_response: %s", nli_response)
        final_statement = self.parse_response(nli_response)
        ...
        reasoning = self.REASONING_MESSAGE.format(statements=statements, final_statement=final_statement)
