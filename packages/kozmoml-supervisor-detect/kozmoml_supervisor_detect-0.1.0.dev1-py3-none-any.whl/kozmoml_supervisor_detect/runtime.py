...
import kozmoml
from kozmoml.batching import BatchedRequests
from kozmoml.types import InferenceRequest, InferenceResponse
from kozmoml.settings import ModelSettings
...
from kozmoml.errors import KozmoMLError, InferenceError
from kozmoml.logging import logger

ENV_PREFIX_SUPERVISOR_DETECT_SETTINGS = "KOZMOML_MODEL_SUPERVISOR_DETECT_"
...
class SupervisorDetectSettings(BaseSettings):
    ...
    Note that this is different to KozmoML's adaptive batching, since the rest
...
class SupervisorDetectRuntime(MLModel):
    ...
    async def load(self) -> bool:
        ...
        try:
            ...
            kozmoml.register("kozmo_model_drift", "Drift metrics")
    ...
    def _log_metrics(self, current_pred: dict) -> None:
        ...
        if "is_drift" in data:
            ...
            for is_drift_instance in is_drift:
                kozmoml.log(kozmo_model_drift=is_drift_instance)
