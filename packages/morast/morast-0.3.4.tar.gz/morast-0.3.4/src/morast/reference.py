...
class MorastModule(BasePage):
    ...
    def __init__(
        ...
        *   _superconfig_: a [SuperConfig] instance that is also passed through
        ...
        self.add_subnode(
            core.MORAST_DOCSTRING, nodes.DocString(self.docstring)
        )
...
class LinkList(nodes.MorastBaseNode):
    ...
    def add_module(self, module: MorastModule) -> None:
        ...
        *   _module_: a [MorastModule] instance
        ...
        elements.append(
            mde.Link(
                module.namespaced_module, url=f"{module.namespaced_module}.md"
            ),
        )
        if module.docstring:
            elements.append(
                mde.BlockQuote(mds.declare_as_safe(module.docstring))
            )
        #
