...
import config
from proxy.providers import new_jisu_http_proxy, new_kuai_daili_proxy, new_free_proxy_1
...
class ProxyIpPool:
    ...
    async def _is_valid_proxy(self, proxy: IpInfoModel) -> bool:
        ...
        is_auth_bypassed = not proxy.user and not proxy.password
        try:
            httpx_proxy = {
                f"{proxy.protocol}": f"http://{proxy.ip}:{proxy.port}"
                    if is_auth_bypassed else f"http://{proxy.user}:{proxy.password}@{proxy.ip}:{proxy.port}"
            }
...
IpProxyProvider: Dict[str, ProxyProvider] = {
    ...
    ProviderNameEnum.FREE_PROXY_LIST_PROVIDER_1ST.value: new_free_proxy_1(),
}
