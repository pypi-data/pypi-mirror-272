    ...
    for scan_path in scan_paths:
        ...
        for root, dirs, files in sorted(os.walk(scan_path)):
            ...
            if not valid_yoda_file:
                continue
            cfg.runpoint = '/'.join(yoda_file_path.split('/')[1:-1]) # store current beam/runpoint info in config
...
def main(args):
    ...
    if cfg.grid:
        ...
        # do we want map (pickle) file output?
    ...
    cfg.results_dbfile = os.path.join(cfg.output_dir,args["DBFILE"])

    if args['ADD_DB']:
        cfg.add_to_results_db = True
        
#    elif os.path.exists(cfg.results_dbfile) and cfg.gridmode:
    elif os.path.exists(cfg.results_dbfile):
#        try:
