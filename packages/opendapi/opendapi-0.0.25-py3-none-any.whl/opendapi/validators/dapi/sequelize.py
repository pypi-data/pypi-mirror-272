...
class SequelizeDapiValidator(DapiValidator):
    """Validator class for DAPI files created for Sequelize v6 models"""

    PACKAGE_JSON = "package.json"

    def _sequelize_settings(self):
        """Get the settings frm the config file."""
        return self.config.get_integration_settings("sequelize")
    ...
    def get_all_packages_with_configs(self) -> Dict[str, Dict]:
        ...
        config = self._sequelize_settings()
    ...
    def get_all_js_files(self) -> List[Tuple[str, str, str, Dict]]:
        """Return all JS files in project as a tuple of path and content"""
        packages = self.get_all_packages_with_configs()
        files = []
        for pkg, config in packages.items():
            pkg_files = find_files_with_suffix(pkg, [".js"])
    ...
    def get_sequelize_tables(self) -> List[Table]:
        ...
        in_scope_files = self.get_all_js_files()
        file_configs = {}
        file_pkgs = {}
