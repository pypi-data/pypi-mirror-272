from faasmsctl.util.batch import batch_exec_factory
from faasmsctl.util.config import (
    get_faasm_ini_file,
    get_faasm_planner_host_port,
)
from faasmsctl.util.docker import in_docker
from faasmsctl.util.gen_proto.faabric_pb2 import BatchExecuteRequestStatus
from faasmsctl.util.message import message_factory
from faasmsctl.util.planner import prepare_planner_msg
...
def invoke_and_await(url, json_msg, expected_num_messages):
    ...
    ber_status = Parse(response.text, BatchExecuteRequestStatus(),ignore_unknown_fields=True)
