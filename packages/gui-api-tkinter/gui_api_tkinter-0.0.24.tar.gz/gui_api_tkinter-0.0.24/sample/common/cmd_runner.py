...
class CmdRunner:
    ...
    def _runner(self, tag, cmd, working_dir):
        ...
        if self._logger is not None:
            self._logger.info(f'{tag}: terminating process: "{cmd}"')
        ...
        if self._logger is not None:
            self._logger.info(f'{tag}: process rc: {proc.returncode}')
    ...
    def _start_task(self, tag, cmd, working_dir='.'):
        if self._logger is not None:
            ...
            self._logger.info(f'   dir : {working_dir}')

        proc = self._start_process(cmd, working_dir)
        if self._logger is not None:
            self._logger.info(f'   pid : {proc.pid}')
