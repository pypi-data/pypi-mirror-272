...
class DoCheck:
    ...
    # @return None
    def run(self):
        svc.log.start(f'{svc.gbl.tag}: starting...')
    ...
    def _check_mod_names(self):
        ...
        if svc.cfg.is_module:
            svc.log.start(f'{svc.cfg.mod_name} is a module')
        else:
            svc.log.start(f'{svc.cfg.mod_name} is not a module')
    ...
    def _check_gitignore(self):
        ...
        if svc.cfg.mod_tech == 'cpp':
            if svc.cfg.is_module:
                fname = os.path.join(svc.utils_fs.root_dir, 'lib', '.gitignore')
            else:
                fname = os.path.join(svc.utils_fs.root_dir, 'src', '.gitignore')
            ...
            if svc.cfg.is_module:
                ...
                fname = os.path.join(str(mod_dir), 'lib', '.gitignore')
            else:
                fname = os.path.join(svc.utils_fs.root_dir, 'lib', '.gitignore')
    ...
    def _check_home(self):
        ...
        if OsSpecific.os_name == 'win':
            ...
            rc, bash_home = svc.utils_ps.run_cmd('echo ~', msys2=True)
        else:
            py_home2 = py_home
            rc, bash_home = svc.utils_ps.run_cmd('echo ~')
