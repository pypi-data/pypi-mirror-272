...
class DvsConfig():
    ...
    def get_folder_file_list(self, path: Path = None):
        if path is None:
            path = self.rootPath / "datasets"
    ...
    def load_initial_dataset_definition(self,dataset_path : Path = None):
        try :
    ...
    def save_raw_data_to_json(self,raw_data,filename = None ,path : Path = None):
        if filename is None:
            filename = "raw_data.json"
        if path is None:
            path = self.rootPath / "datasets" / filename
