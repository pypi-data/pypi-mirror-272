...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=3,
        ...
        self.averagepooling = nn.AdaptiveAvgPool2d(16)
...
class TransformerExtraDecoder(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=3,
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec)
        self.reconsturct_decoder = TransformerDecoder(decoder_layer, 1, decoder_norm,
                                          return_intermediate=return_intermediate_dec)
    ...
    def forward(self, style, mask , content, pos_embed_c, pos_embed_s):
        ...
        style_encoded = self.encoder_s(style, src_key_padding_mask=mask, pos=pos_embed_s)
        content_encoded = self.encoder_c(content, src_key_padding_mask=mask, pos=pos_embed_c)

        hs = self.decoder(content_encoded, style_encoded, memory_key_padding_mask=mask,
                          pos=pos_embed_s, query_pos=pos_embed_c)[0]
        hs = self.reconsturct_decoder(hs, content, memory_key_padding_mask=mask,
                          pos=pos_embed_s, query_pos=pos_embed_c)[0]
