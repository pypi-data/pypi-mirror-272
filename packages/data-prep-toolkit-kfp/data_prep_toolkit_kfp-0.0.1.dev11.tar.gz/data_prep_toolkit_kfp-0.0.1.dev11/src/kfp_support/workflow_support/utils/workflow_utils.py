...
class PipelinesUtils:
    ...
    def upload_pipeline(
        ...
        delete_existing_pipeline:  bool = False,
        ...
        :param delete_existing_pipeline: Optional. Delete a pipeline if it exists.
        ...
        """
        if delete_existing_pipeline:
            pipeline = self.kfp_client.get_pipeline_by_name(name=pipeline_name)
            if pipeline is not None:
                try:
                    ...
                    return None
        try:
            pipeline = self.kfp_client.upload_pipeline(pipeline_package_path=pipeline_package_path, pipeline_name=pipeline_name,
