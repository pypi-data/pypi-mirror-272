...
class RegistryConnectionInfo(BaseModel):
    """
    ConnectionInfo specifies how to connect to a data source.
    """ # noqa: E501
    data_file: Optional[RegistryDataFileInfo] = Field(default=None, alias="dataFile")
    data_loading: Optional[RegistryDataLoadingInfo] = Field(default=None, alias="dataLoading")
    data_collector: Optional[RegistryDataCollectorInfo] = Field(default=None, alias="dataCollector")
    databricks: Optional[RegistryDatabricksInfo] = None
    hugging_face: Optional[RegistryHuggingFaceDataInfo] = Field(default=None, alias="huggingFace")
    __properties: ClassVar[List[str]] = ["dataFile", "dataLoading", "dataCollector", "databricks", "huggingFace"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "dataFile": RegistryDataFileInfo.from_dict(obj["dataFile"]) if obj.get("dataFile") is not None else None,
            "dataLoading": RegistryDataLoadingInfo.from_dict(obj["dataLoading"]) if obj.get("dataLoading") is not None else None,
            "dataCollector": RegistryDataCollectorInfo.from_dict(obj["dataCollector"]) if obj.get("dataCollector") is not None else None,
            "databricks": RegistryDatabricksInfo.from_dict(obj["databricks"]) if obj.get("databricks") is not None else None,
            "huggingFace": RegistryHuggingFaceDataInfo.from_dict(obj["huggingFace"]) if obj.get("huggingFace") is not None else None
        })
        return _obj
