...
from flutter_gen.template.component_cmd import ComponentCommand
...
def cmd_template(parser, context, args):
    ...
    options = {"navigator": args.navigator}
    TemplateCommand(template_name, scene_name, options).create_files()


@subcmd("component", help="create components files for a scene")
def cmd_template(parser, context, args):
    parser.description = "Create components files for a scene"
    parser.add_argument("parent_name", nargs=1, help="parent name")
    parser.add_argument("name", nargs=1, help="component name")
    args = parser.parse_args(args)
    parent_name = args.parent_name[0]
    name = args.name[0]
    ComponentCommand(parent_name, name).create_files()
...
def cmd_api(parser, context, args):
    ...
    parser.add_argument("name", nargs=1, help="api name")
    args = parser.parse_args(args)
...
def cmd_template(parser, context, args):
    ...
    parser.add_argument("key", nargs=1, help="document path")
    parser.add_argument("text", nargs=1, help="document path")
    args = parser.parse_args(args)
    CreateI18NCommand(args.key[0], args.text[0]).create()


@subcmd(
    "watch", help="watching the file system for updates and rebuilding as appropriate"
)
def cmd_template(parser, context, args):
    parser.description = (
