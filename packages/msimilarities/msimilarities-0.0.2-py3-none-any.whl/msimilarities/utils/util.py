...
def cos_sim(a: Union[mindspore.Tensor, np.ndarray], b: Union[mindspore.Tensor, np.ndarray]):
    """
    ...
    a_norm = a / ops.norm(a, ord=2, dim=1, keepdim=True)
    b_norm = b / ops.norm(b, ord=2, dim=1, keepdim=True)
    return ops.mm(a_norm, b_norm.swapaxes(0, 1))


def dot_score(a: Union[mindspore.Tensor, np.ndarray], b: Union[mindspore.Tensor, np.ndarray]):
    """
    ...
    return ops.mm(a, b.swapaxes(0, 1))


def pairwise_dot_score(a: Union[mindspore.Tensor, np.ndarray], b: Union[mindspore.Tensor, np.ndarray]):
    """
...
def pairwise_cos_sim(a: Union[mindspore.Tensor, np.ndarray], b: Union[mindspore.Tensor, np.ndarray]):
    """
...
def normalize_embeddings(embeddings: mindspore.Tensor):
    """
    Normalizes the embeddings matrix, so that each sentence embedding has unit length
    """
    return embeddings / ops.norm(embeddings, ord=2, dim=1, keepdim=True)


def semantic_search(
        query_embeddings: Union[mindspore.Tensor, np.ndarray],
        corpus_embeddings: Union[mindspore.Tensor, np.ndarray],
    ...
    if isinstance(query_embeddings, (np.ndarray, np.generic)):
        query_embeddings = mindspore.Tensor.from_numpy(query_embeddings)
    ...
    if isinstance(corpus_embeddings, (np.ndarray, np.generic)):
        corpus_embeddings = mindspore.Tensor.from_numpy(corpus_embeddings)
...
def community_detection(embeddings, threshold=0.75, min_community_size=10, batch_size=10000):
    ...
    threshold = mindspore.tensor(threshold, device=embeddings.device)
