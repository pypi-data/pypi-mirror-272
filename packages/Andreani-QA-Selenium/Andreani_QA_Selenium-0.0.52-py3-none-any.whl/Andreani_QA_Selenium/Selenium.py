...
class Selenium(Functions, Parameters):
    ...
    def open_browser(self, url=None, browser=Parameters.browser, options_headless=Parameters.headless,
                     download_path=None, record=None, privated=True):
        ...
        Functions.LoggingFeature(f"{self.color_message('YELLOW', 'AGUARDANDO:')} Inicio del navegador "
              f"'{self.color_message('BLUE', browser)}'.").send_log()
        ...
        if Parameters.environment == "Windows":
            if Parameters.environment_configuration == "standalone" or Parameters.environment_configuration is None:
                Selenium.set_mode_debug(False)
        ...
        try:
            if browser == "CHROME":
                if Parameters.environment_configuration == "server":
                    options.binary_location =  r'C:\\Chromium\\chrome.exe'
                else:
                    if os.getenv('PYBOT_CHROMIUM') is not None:
                        options.binary_location = os.getenv('PYBOT_CHROMIUM')
                    else:
    ...
    def tear_down(self):
        ...
        try:
            if Selenium.data_cache not in ([], {}):
                ...
                print("=====================Fin Cache=====================")
            Functions.LoggingFeature(f"{Selenium.color_message('YELLOW', 'AGUARDANDO:')} Se cerrará el web driver.").send_log()
            ...
            Functions.LoggingFeature(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Finaliza la ejecución.").send_log()
            if Selenium.record:
    ...
    def locator_element(self, type_locator, indentify, entity=None):
        ...
        try:
            ...
            Functions.LoggingFeature(f"Se interactuo con el elemento {indentify}").send_log()
            Functions.LoggingFeature(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Se detecto el elemento web "
                  f"'{Selenium.color_message('BLUE', entity)}' utilizando el "
                  f"identificador {type_locator} apuntando a '{indentify}'").send_log()
            ...
            Functions.LoggingFeature(f"No se pudo encontrar el elemento web '{Selenium.color_message('BLUE', entity)}'"
            ...
            Functions.LoggingFeature(f"Se agoto el tiempo de busqueda intentando encontrar el elemento web "
            ...
            Functions.exception_logger(e)
            Functions.LoggingFeature(f"Ha ocurrido un error inesperado en tiempo de ejecución.").send_log()
    ...
    def capture_element(self, entity, variable_x=None, variable_y=None):
        ...
        if not find_element:
            if os.getenv('PYBOT_SYSTEM') is not None:
    ...
    def get_json_file(self, file):
        ...
        try:
            with open(json_path, "r", encoding='utf8') as read_file:
                ...
                Functions.LoggingFeature(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Se a cargado el respositorio de objetos "
                      f"'{Selenium.color_message('BLUE', f'{file}.json')}' encontrado en el directorio "
                      f"'{json_path}'.").send_log()
                #print(f"{Selenium.color_message('GREEN', 'REALIZADO:')} Se a cargado el respositorio de objetos "
                      #f"'{Selenium.color_message('BLUE', f'{file}.json')}' encontrado en el directorio "
                      #f"'{json_path}'.")
        except FileNotFoundError:
            Selenium.json_strings = False
            Functions.LoggingFeature(
                f"{Selenium.color_message('RED', 'ERROR:')} No se encontro "
                  f"'{Selenium.color_message('BLUE', f'{file}.json')}' en el directorio '{json_path}'.").send_log()
...
class ElementUI(Selenium):
    ...
    def execute_action(self, action, value=None):
        ...
        Functions.LoggingFeature(f"{Functions.color_message('GREEN', 'REALIZADO:')} Se pudo {self.message} sobre "
              f"'{Functions.color_message('BLUE', self.entity)}'.").send_log()
