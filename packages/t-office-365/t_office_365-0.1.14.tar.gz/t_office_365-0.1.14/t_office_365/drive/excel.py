...
class Excel(Core):
    ...
    @retry_if_exception
    def get_row_cells(
        self, file_id: str, sheet_name: str, row: int, headers_row: int = 1, end_column: int = None
    ) -> List[Cell]:
        """Get rows values from Excel.
        ...
        result = self.get_rows_range_data(file_id, sheet_name, start_row=row, end_row=row, end_column=end_column)
        try:
            return json_to_row_cells(result, headers, row)
        except IndexError:
            raise ValueError(f"Unable to read values in {row} row.")
    ...
    def get_rows_range_data(
        ...
        - UnexpectedError: If there is an unexpected error during the request.
        """
        last_column, last_row = self.get_last_row_column(file_id, sheet_name)
        if not end_row:
            end_row = last_row
        if not end_column:
    ...
    def get_last_row_column(self, file_id: str, sheet_name: str) -> Tuple[int, int]:
        ...
        return int(range_info["columnCount"]), int(range_info["rowCount"])
