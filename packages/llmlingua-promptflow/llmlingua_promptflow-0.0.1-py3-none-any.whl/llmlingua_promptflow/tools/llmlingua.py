...
class PromptCompressor:
    ...
    def __init__(
        ...
        infer_api_config: dict = None,
    ):
        self.infer_api_config = infer_api_config
    ...
    def load_model(
        ...
        if self.infer_api_config is None:
            raise ValueError("Base model API config is required.")
        def model(input_ids):
            token_len = input_ids.shape[1]
            prompt = self.tokenizer.batch_decode(input_ids)[0]
            data =  {
            "prompt": prompt,
            "temperature": 0,
            "max_tokens": 0,
            ...
            req = urllib.request.Request(self.infer_api_config['url'], body, {'Content-Type':'application/json', 'Authorization':('Bearer '+ self.infer_api_config['api_key'])})
            res = urllib.request.urlopen(req).read()
            res = json.loads(res)

            logits = - np.array(res['choices'][0]['logprobs']['token_logprobs'][2:token_len + 1])
            return logits

        self.model = model
        self.context_idxs = []
        self.max_position_embeddings = config.max_position_embeddings - 100 if self.infer_api_config is not None else config.max_position_embeddings
    ...
    def get_ppl(
        ...
        if past_key_values is not None:
            ...
            past_length = 0 if past_length is None else past_length
        if end is None:
            end = input_ids.shape[1]
        end = min(end, past_length + self.max_position_embeddings)
        if past_length:
            past_length -=1
        loss = self.model(input_ids[:, past_length:end])
    ...
    def compress_prompt(
        ...
        iterative_size = self.max_position_embeddings
    ...
    def get_compressed_input(
        ...
        if self_loss is not None:
            ...
            need_idx = np.concatenate([loss > threshold, loss[:1] > 0])
        ...
        if need_idx.shape[0] < input_ids.shape[1]:
            need_idx = np.concatenate([need_idx, np.ones(input_ids.shape[1] - need_idx.shape[0], dtype=bool)])
    ...
    def get_estimate_threshold_base_distribution(
        ...
        if condition_flag:
            sorted_ppl = np.sort(ppl)
        else:
            sorted_ppl = np.sort(ppl)[::-1]
        return sorted_ppl[target_token]

    def trunk_token_compress(
        self,
        context: List[str],
        target_token: float,
        iterative_size: int = 200,
        ...
        iterative_size = min(iterative_size, compressed_input_ids.shape[1] - start)
        keep_flag = None, None
        if keep_split:
            ...
            keep_flag = np.array(keep_flag)
        
        import threading
        self.api_results = {}
        lock = threading.Lock()
        def parallel_trunk_compress(input_id_start, input_id_end, trunk_out_id=0):
            loss = self.get_ppl(
                "",
                "token",
                compressed_input_ids,
                ...
                past_length=input_id_start,
            )
            trunk_compressed_input_ids = compressed_input_ids[:, input_id_start:input_id_end].clone()
            trunk_compressed_attention_mask = compressed_attention_mask[:, input_id_start:input_id_end].clone()
            iterative_size = input_id_end - input_id_start
            trunk_end = iterative_size
            
            for delta_end, ratio in iterative_ratios[trunk_out_id]:
                threshold = self.get_estimate_threshold_base_distribution(
                    loss, ratio, False
                )
                (
                    trunk_compressed_input_ids,
                    trunk_compressed_attention_mask,
                    keep_flag,
                    trunk_end,
                    past_loss,
                    self_past_loss,
                    ...
                    trunk_compressed_input_ids,
                    trunk_compressed_attention_mask,
                    end=trunk_end - iterative_size + delta_end,
                ...
                trunk_end += iterative_size
            with lock:
                self.api_results[trunk_out_id] = (trunk_compressed_input_ids[:, start:], trunk_compressed_attention_mask[:, start:])
        
        threads = []
        trunk_sizes = [sum([token_len for token_len, token_ratio in trunk_ratios]) for trunk_ratios in iterative_ratios]
        trunk_prefix = [0]
        trunk_num = len(iterative_ratios)
        for i in range(len(trunk_sizes)):
            trunk_prefix.append(trunk_prefix[-1] + trunk_sizes[i])
        for i in range(len(iterative_ratios)):
            thread = threading.Thread(target=parallel_trunk_compress, args=(trunk_prefix[i], trunk_prefix[i+1], i))
            thread.start()
        ...
        compressed_attention_mask = np.concatenate([self.api_results[id][1] for id in range(trunk_num)], axis=1)
...
@tool
def prompt_compress(prompt: str,  myconn: CustomConnection, rate: float=0.5,) -> str:
    # Call LLMLingua API
    ...
    print(f'Compression rate: {res["rate"]} | original tokens: {res["origin_tokens"]} | compressed tokens: {res["compressed_tokens"]}')
