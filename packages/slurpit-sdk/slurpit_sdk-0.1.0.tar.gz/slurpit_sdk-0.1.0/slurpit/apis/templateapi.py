...
class TemplateAPI(BaseAPI):
    ...
    def get_templates(self, export_csv: bool = False):
        """
        Fetches all templates from the API and returns them as instances of the Template model. Optionally exports the data to a CSV format if specified.

        Args:
            export_csv (bool): If True, returns the template data in CSV format as bytes. If False, returns a list of Template instances.

        Returns:
            list[Template] | bytes | None: A list of Template instances if successful, bytes if exporting to CSV, None otherwise.
        """
    ...
    def get_template(self, template_id: int):
        ...
        Returns:
            Template: A Template instance if successful, None otherwise.
        """
        url = f"{self.base_url}/templates/{template_id}"
        try:
            response = self.get(url)
            return Template(**response.json()) if response else None
        except Exception as e:
            print(f"Response error: {e}")
            return None
    ...
    def update_template(self, template_id: int, update_data: dict):
        ...
        Returns:
            Template: An updated Template instance if successful, None otherwise.
        """
        url = f"{self.base_url}/templates/{template_id}"
        try:
    ...
    def delete_template(self, template_id: int):
        ...
        Returns:
            Template: A Template instance representing the deleted template if successful, None otherwise.
        """
        url = f"{self.base_url}/templates/{template_id}"
        try:
    ...
    def create_template(self, new_template: dict):
        ...
        Returns:
            Template: A new Template instance if successful, None otherwise.
        """
        url = f"{self.base_url}/templates"
        try:
    ...
    def search_template(self, search_data: dict, export_csv: bool = False):
        """
        Searches for templates based on the provided search criteria and optionally exports the data to a CSV format.
        
        Args:
            search_data (dict): A dictionary containing search parameters.
            export_csv (bool): If True, returns the search results in CSV format as bytes. If False, returns a list of Template instances.

        Returns:
            list[Template] | bytes | None: A list of Template instances matching the search criteria if successful, bytes if exporting to CSV, None otherwise.
        """
    ...
    def run_template(self, run_info: dict):
        ...
        Returns:
            dict: The result of running the template if successful, None otherwise.
        """
    ...
    def validate_textfsm(self, textfsm: str):
        ...
        Returns:
            dict: The validation result if successful, None otherwise.
        """
