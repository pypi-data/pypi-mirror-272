...
class DiffItem:
    ...
    def compute_diff(self):
        ...
        self.diff = list(
            difflib.unified_diff(tmp_content, src_content, "before", "after", n=9999999)
        )
...
class TrackerApp(App):
    ...
    BINDINGS = [("f", "copy_file_path", "Copy file name"), ("c", "copy_selection", "Copy selected text"), ("f10", "exit", "Exit")]
    ...
    def action_copy_selection(self):
        if len(self.text_area.selected_text) > 0:
            subprocess.run("pbcopy", text=True, input=self.text_area.selected_text)
