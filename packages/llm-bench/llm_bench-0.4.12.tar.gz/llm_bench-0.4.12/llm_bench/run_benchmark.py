...
def clean_output(text):
    ...
    # Normalize whitespace around newlines and ensure metric lines are formatted correctly
    text = re.sub(r'\s*\n+\s*', ' ', text)  # Reduce multiple newlines to single spaces

    # Ensure that certain phrases start on a new line by checking if the phrase is preceded by a letter or digit and not already preceded by a newline
    metrics = [
    ...
    # Fix any potential issues where 'prompt' may appear incorrectly on a new line
    text = re.sub(r"(prompt)\n(eval)", r"\1 \2", text)

    # Ensure no leading or trailing whitespace and trim excessive newlines
    text = text.strip()
