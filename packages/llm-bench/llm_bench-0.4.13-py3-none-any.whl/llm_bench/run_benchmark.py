...
def clean_output(text):
    ...
    # Normalize space around text
    text = re.sub(r'\s*\n+\s*', ' ', text)  # Collapse newlines and trim space

    # Fix misplaced newlines for prompt-related lines
    prompt_metrics = [
        "prompt eval count:", "prompt eval duration:", "prompt eval rate:"
    ]
    for metric in prompt_metrics:
        text = re.sub(f"prompt\s*\n+\s*({metric})", r"prompt \1", text)
