...
def clean_output(text):
    ...
    # Normalize whitespace around newlines and remove extra newlines
    text = re.sub(r'\s*\n+\s*', ' ', text)

    # Correct the splitting issue with "prompt" and its metrics
    prompt_metrics = ["prompt eval count:", "prompt eval duration:", "prompt eval rate:"]
    for prompt_metric in prompt_metrics:
        text = re.sub(f"prompt\s+({prompt_metric})", r"prompt \1", text)
    ...
    for metric in metrics:
        text = re.sub(f"\\b{metric}", r"\n\1", text)

    # Trim and ensure there is no leading or trailing whitespace
    text = text.strip()
