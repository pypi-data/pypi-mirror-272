...
if "win" in sys.platform:
  ...
  def write_lnk(path:str,target:str,args:str=None,cwd:str=None,desc:str=None,hotkey:str=None,icon:str=None,):
    ...
    if args!=None:lnk.Arguments=args
    if cwd!=None:lnk.WorkingDirectory=cwd.replace("/","\\")
    if desc!=None:lnk.Description=desc
    if hotkey!=None:lnk.Hotkey=hotkey
    if icon!=None:lnk.IconLocation=icon.replace("/","\\")
    lnk.Save()
