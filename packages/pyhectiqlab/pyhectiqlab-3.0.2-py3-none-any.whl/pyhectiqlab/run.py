...
class Run:
    ...
    @clisync.include()
    def track_version(
        repos: Union[str, List[str], None] = None, run: Optional[str] = None, project: Optional[str] = None
    ):
        ...
        Args:
            ...
            run (str): The run to update. If None, the current run is used. Default: None.
        """
        if not isinstance(repos, list) and (repos is not None):
            repos = [repos]
        Run._repos = repos
        if info := PackageVersion.all(repos=repos):
            Run._update(run_id=run, package_versions=info)
    ...
    def set_status(status: str, run_id: Optional[int] = None):
        """Set the run to the given status."""
        Run._update(run_id=run_id, status=status)

    @staticmethod
    @online_method
    @functional_alias("run_has_failed")
    def failed(run_id: Optional[int] = None):
        """Set the run to the failed status."""
        Run.set_status(run_id=run_id, status="failed")

    @staticmethod
    @online_method
    @functional_alias("run_is_stopped")
    def stopped(run_id: Optional[int] = None):
        """Set the run to the stopped status."""
        Run.set_status(run_id=run_id, status="stopped")

    @staticmethod
    @online_method
    @functional_alias("run_is_completed")
    def completed(run: Optional[int] = None):
        """Set the run to the completed with success status."""
        Run.set_status(run_id=run, status="completed")

    @staticmethod
    @online_method
    @functional_alias("run_is_pending")
    def pending(run_id: Optional[int] = None):
        """Set the run to the pending status."""
        Run.set_status(run_id=run_id, status="pending")

    @staticmethod
    @online_method
    @functional_alias("run_is_running")
    def running(run_id: Optional[int] = None):
