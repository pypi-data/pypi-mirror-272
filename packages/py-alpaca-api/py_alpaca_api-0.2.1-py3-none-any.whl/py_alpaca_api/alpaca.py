...
################################################################################################
    ...
    ############################
    def get_stock_historical_data(self, symbol, start, end, timeframe='1d', feed='iex', currency='USD', limit=1000, sort='asc', adjustment='raw'):
        '''
        ...
        bar_data_df = bar_data_df.astype({'open': 'float', 'high': 'float', 'low': 'float', 'close': 'float', 'symbol': 'str', 'date': 'datetime64[ns]', 'vwap': 'float', 'trade_count': 'int', 'volume': 'int'})
        # Return historical stock data as a DataFrame
        return bar_data_df
...
################################################################################################
    ...
    #\\\\\\\\\\\\\\\\  Submit Limit Order /////////////////#
    ########################################################
    def limit_order(self, symbol: str, limit_price: float, qty: float=None, notional: float=None, side: str='buy', time_in_force: str = 'day', extended_hours: bool = False):
        '''
    ...
    #\\\\\\\\\\\\\\\\  Submit Stop Order /////////////////#
    ########################################################
    def stop_order(self, symbol: str, stop_price: float, qty: float, side: str='buy', time_in_force: str = 'day', extended_hours: bool = False):
        '''
        Submit a Stop Order
        symbol: Asset symbol to buy/sell
