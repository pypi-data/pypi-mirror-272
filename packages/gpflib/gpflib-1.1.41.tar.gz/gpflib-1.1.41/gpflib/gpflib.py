...
class GPF:
                    ...
                    for Rel,Tail in V.items():
                        ...
                        if isinstance(Tail ,dict):
                            for k,v in Tail.items():
                                ...
                                DoctInfo+=self.DrawGraph(v,Tag[k])
        return DoctInfo
        ...
        if isinstance(Json,list):
            for i in range(len(Json)):
                if isinstance(Json[i],str) or isinstance(Json[i],int):
                    ...
                    DoctInfo+=self.DrawTree(Json[i],Root)
    ...
    def ShowTree(self,Json):
        ...
        Script=""
        Script+=Head
        Script+=self.DrawTree(json.loads(Json))
        Script+=Tail
        return Script
    ...
    def ShowGraph(self,Json):
        ...
        Script=""
        Script+=Head
        Script+=self.DrawGraph(json.loads(Json))
        Script+=Tail
        return Script
