...
def forall_exists_encodings(universal_encoding, existential_encoding):
    ...
    for clause in universal_encoding.get_clauses(no_dups=True):
    ...
    for clause in existential_encoding.get_clauses(no_dups=True):
        converted_clause = [existential_encoding.lit_to_str(lit) for lit in clause]
        fclause = converted_clause + [-Z.v(f"__f_{c}") for c in universal_encoding.get_clauses(no_dups=True)]
