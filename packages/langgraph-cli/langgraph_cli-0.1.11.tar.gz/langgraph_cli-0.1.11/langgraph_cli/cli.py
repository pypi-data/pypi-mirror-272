...
@OPT_PORT
@cli.command()
def watch(config: pathlib.Path, docker_compose: Optional[pathlib.Path], port: int):
    with open(config) as f:
        config_json = json.load(f)
    compose = langgraph_cli.docker.compose(
        port=port, python_version=config_json["python_version"]
    )
    args = ["--project-directory", config.parent, "-f", "-"]
    if docker_compose:
        args.extend(["-f", str(docker_compose)])
    with asyncio.Runner() as runner:
        try:
            runner.run(
                gather(
                    exec("docker", "compose", *args, "watch", input=compose),
                    exec_loop(
                        "docker",
                        "compose",
                        *args,
            ...
            runner.run(exec("docker", "compose", *args, "kill", input=compose))
