    ...
    def run(self, args: argparse.Namespace):
        for index, target in enumerate(self.url_list):
            ...
            if self.is_url(target):
                try:
                    scrape_result = websites.WebsiteScraper(target, args, self.extract_types, self.ptjsonlib).process_website( target, args, self.extract_types, self.ptjsonlib )
                    if scrape_result:
            ...
            # Process Files
                ...
                scrape_result = files.FileScraper(path_to_local_file=os.path.abspath(target), args=args, things_to_extract=self.extract_types, ptjsonlib=self.ptjsonlib).process_file(path_to_local_file=os.path.abspath(target), args=args, things_to_extract=self.extract_types, ptjsonlib=self.ptjsonlib)
                if scrape_result:
                    self.result_list.append(scrape_result)
                else:
                    self.handle_whitespace(index)
