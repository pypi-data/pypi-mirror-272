...
class SQSStreamSupervisor(MiddlewareBase):
    ...
    def load_worker(self):
        ...
        while True:
            ...
            if not self.ctx:
                ...
                self.total_parts = 0
                self.filepath = f"s3://{S3_BUCKET}/storage/{self.ctx.application}/{today}/{self.ctx.id}.mp3"
    ...
    def _process_audio(self, stream_ctx: StreamContext):
        for packet in self.out_stream.encode(stream_ctx.payload):
            self.out_container.mux(packet)
