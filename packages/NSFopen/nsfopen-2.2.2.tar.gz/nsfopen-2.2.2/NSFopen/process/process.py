...
class process():
    ...
    def image(self,
        ...
        im = plt.imshow(d, extent=(0, x, 0, y), cmap=cmap, origin='lower', **kwargs)
        ...
        if scalebar:
            ...
            txt = plt.text(x - pad - dx / 2, pad + x * 0.03, f'%i $\mu$m' % dx,
    ...
    def hist(self,
        ...
        data = np.array(self.data['Image'][direction][channel]*scale,dtype=float).reshape(-1)
        ...
        n, edge, patches = plt.hist(data, bins=bins, density=density, color='green', **kwargs)
...
if __name__ == "__main__":
    ...
    fig = afm.image(cbar='Z [nm]', title='', channel=chan, scalebar={'color':'w','length':0.25}, scale=1e9, theme='dark')
    plt.show()
    
    afm.hist(scale=1e12,xlabel='Height [pm]',theme='dark')
