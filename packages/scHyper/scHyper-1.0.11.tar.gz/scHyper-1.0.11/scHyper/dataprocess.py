...
def generate_data_signaling(adata_h, file_path):
    ...
    data_signaling.to_csv("file_path/data_signaling.csv", index=True, header=True)
...
def generate_results(adata, df_nn, tensor_pval, validlrs, train_nums_type, file_path):
    ...
    interaction_df.to_csv('file_path/interaction_tensor.csv', index=False, header=False)
    tensor_pval.to_csv("file_path/tensor_pval.csv", index=False, header=False)
    adata.obs.to_csv("file_path/meta.csv", index=True, header=True)
    data_signaling = generate_data_signaling(adata)
    validlrs.to_csv("file_path/validlrs.csv", index=False, header=True)