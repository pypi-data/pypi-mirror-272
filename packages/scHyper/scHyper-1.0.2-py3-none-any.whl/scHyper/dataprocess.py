...
def process_ligands_receptors(adata, species_option, highly_variable=True):
    ...
    #file_path = f"database/{species_option}.csv"
...
def genenrate_df_nn_candidates(validlrs, validsenders, validreceivers, triplets, use_to_predict):
    ...
    import pickle
    file_path = 'model/predictions.pkl'
...
def generate_data_signaling(adata_h):
    ...
    data_signaling.to_csv("data_signaling.csv", index=True, header=True)

    return data_signaling

def generate_results(adata, df_nn, tensor_pval, validlrs, x1, x2, file_path, ):

    interaction_df = pd.DataFrame(np.array(df_nn).reshape(-1, x1 * x2))
    celltype = pd.DataFrame(adata.uns["gene_call"]).columns
    tensor_pval = pd.DataFrame(tensor_pval.reshape(-1, x1 * x2))
    cell_type_counts = adata.obs['labels'].value_counts()

    interaction_df.to_csv('interaction_tensor.csv', index=False, header=False)
    tensor_pval.to_csv("tensor_pval.csv", index=False, header=False)
    adata.obs.to_csv("meta.csv", index=True, header=True)
    data_signaling = generate_data_signaling(adata)
    validlrs.to_csv("validlrs.csv", index=False, header=True)