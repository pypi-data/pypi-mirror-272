...
def synthese_adherent(year,ctg_path):
    ...
    file_out = Path(GLOBAL['ROOT']) / Path(str(year)) / Path('STATISTIQUES') / Path('synthese_adherent.xlsx')
...
def nbr_sejours_adherent(year):
    ...
    def addlabels(x,y):
        for i in range(len(x)):
            plt.text(x[i]-0.2,y[i]+1,y[i],size=20)
            
    file_in = Path(GLOBAL['ROOT']) / Path(str(year)) / Path('STATISTIQUES') / Path('synthese_adherent.xlsx')
    df_total = pd.read_excel(file_in)
    ...
    plt.title(str(year),pad=20, fontsize=30)
    
    ax.set_xlabel('N séjours', fontsize = 20)
    ax.set_ylabel('Nombre de CTG ayant \n participé à N séjours', fontsize = 20)
...
def evolution_sorties(type,ctg_path):
    ...
    def add_memory(stat_dic,years):
        ...
        for year,v in memory['memory'].items():
            ...
            stat_dic[str(year)] = statyear(v['SEJOUR'],                # nbr_sejours
                                           0,                          # jours_sejour
    ...
    def plot_stat(years,nb_participants,title):
        plt.subplot(1,1,1)
        colors = ['#fdaa48'] 
        size_label = 20
        plt.bar(years,nb_participants,color=colors)
        plt.ylabel("# participants",size=size_label)
    ...
    if type == 'nbr_jours_participation_sejours':
        plot_stat(years,[stat_dic[year].nbr_jours_participation_sejours for year in years],type)
    elif  type == 'sortie_dimanche_club':
        plot_stat(years,[stat_dic[year].sortie_dimanche_club for year in years],type)
    elif  type == 'sortie_samedi_club':
        plot_stat(years,[stat_dic[year].sortie_samedi_club for year in years],type)
    elif  type == 'sortie_jeudi_club':
        plot_stat(years,[stat_dic[year].sortie_jeudi_club for year in years],type)  
    elif  type == 'randonnee':
        plot_stat(years,[stat_dic[year].randonnee for year in years],type)  
    elif  type == 'nbr_participations_sejours':
        plot_stat(years,[stat_dic[year].nbr_participations_sejours for year in years],'Nombre de participations aux séjours') 
    elif  type == 'nbr_sejours':
        plot_stat(years,[stat_dic[year].nbr_sejours for year in years],'Nombre de séjours')
    elif  type == 'nbr_jours_sejours':
        plot_stat(years,[stat_dic[year].nbr_jours_sejours for year in years],'Nombre de séjours')        