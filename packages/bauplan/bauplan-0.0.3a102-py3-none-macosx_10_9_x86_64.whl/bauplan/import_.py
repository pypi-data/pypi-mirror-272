...
class ApplyPlanState:
    ...
    task_lifecycle_events: Optional[List[TaskLifecycleEvent]]
...
def _handle_apply_import_log(log: RunnerInfo, state: ApplyPlanState) -> None:
    ...
    if log.task_lifecycle_event.task_id:
        if (
            os.getenv('BPLN_DEBUG')
            and 'pip install' not in log.task_lifecycle_event.task_description
            and 'docker pull' not in log.task_lifecycle_event.task_description
        ):
            print(
                'task_id',
                log.task_lifecycle_event.task_id,
                'task_description',
                log.task_lifecycle_event.task_description,
            )
...
def _handle_plan_import_log(log: RunnerInfo, state: PlanImportState) -> None:
    state.logs.append(log)
    # log.runner_event.task_completion.
    if log.task_lifecycle_event.task_id:
        state.task_lifecycle_events.append(log.task_lifecycle_event)
        if (
            os.getenv('BPLN_DEBUG')
            and 'pip install' not in log.task_lifecycle_event.task_description
            and 'docker pull' not in log.task_lifecycle_event.task_description
