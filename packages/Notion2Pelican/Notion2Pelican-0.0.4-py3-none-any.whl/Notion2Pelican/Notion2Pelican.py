import json
from os.path import abspath, exists, join, pardir, splitext
...
def get_img(url, dpo):
    ...
    # logic so far works for Notion files stored on AWS only
    if url.find("https://prod-files-secure.s3") < 0:
        print("url not on AWS", url)
        return None
    ...
    if exists(fp):
        ...
        fn_pattern = f"*{fn_base}_*"
        try:
            matches = Path(dpo).glob(fn_pattern)
            idx_new = len(list(matches))
        except FileNotFoundError:
            idx_new = 0
        fn = f"{fn_base}_{idx_new}{fn_ext}"
    ...
    for block in res["results"]:
        ...
        if btype == "paragraph":  # in block:
            ...
            if img_local:
                ...
                if fn is None:
                    img = f"![{caption}]({rsc_folder}missing.png)\n\n"
                else:
