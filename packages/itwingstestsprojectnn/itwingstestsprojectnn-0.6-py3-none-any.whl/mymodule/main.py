...
def get_pagespeed_report(url):
    ...
    def setup_no_data():
        data.update(
            {f"{strategy}_evaluation_result": "-" for strategy in ps_config["strategies"]})
        data['phase_4'] = 'no data'

    try:
        for strategy in ps_config["strategies"]:
            # API request
            response = requests.get(
                f'https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={url}&strategy={strategy}')
            ...
            if ("LARGEST_CONTENTFUL_PAINT_MS" in report.get("loadingExperience", {}).get("metrics", {}) and
                ...
                if (data.get('desktop_evaluation_result', 'no data') == 'passed' or 
                    data.get('mobile_evaluation_result', 'no data') == 'passed'):
                    data['phase_4'] = 'passed'
                else:
                    data['phase_4'] = 'failed'
            else:
