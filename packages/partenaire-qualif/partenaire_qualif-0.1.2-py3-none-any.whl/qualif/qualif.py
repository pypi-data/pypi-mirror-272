...
class CompanyQualificationTool:
    ...
    def __complete(self, messages) -> str | None:
        while True:
            try:
    ...
    def __find_website(self, partner_name: str) -> str | None:
        if self.browsing:
            bing_search_results = self.__bing_search(query=f"{partner_name} website address")
    ...
    def qualify(self, partner_name: str) -> json:
        ...
        if self.browsing:
            website = self.__find_website_loop(partner_name)
