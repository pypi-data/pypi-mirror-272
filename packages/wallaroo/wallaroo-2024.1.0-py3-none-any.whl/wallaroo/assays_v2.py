...
class AssayV2(Object):
    ...
    def results(self, start=None, end=None):
        ret = sync_detailed_results(
            client=self._client.mlops(),
            body=GetResultsBody(id=self.id, start=start, end=end),
        )
        if ret.parsed is None:
            raise Exception("Failed to get results", ret)
        return AssayResultsList([AssayResultV2(self, x) for x in ret.parsed], self)
    ...
    def set_active(self, active: bool):
        ...
        ret = sync_detailed_set_active(
            client=self._client.mlops(),
            body=SetActiveBody(
                active, self.id, self.targeting.data_origin.workspace_id
            ),
        )
    ...
    def _repr_html_(self):
        ...
        return f"""<table>
          ...
          <tr><td>Last Run</td><td>{next_run_data.last_run.strftime(fmt)}</td></tr>
          <tr><td>Next Run</td><td>{next_run_data.next_run.strftime(fmt)}</td></tr>
    ...
    def _repr_html_(self):
        def row(assay: AssayV2):
            ...
            <tr>
              ...
              <td>{next_run_data.last_run.strftime(fmt)}</td>
              <td>{next_run_data.next_run.strftime(fmt)}</td>
    ...
    def to_dataframe(self):
        ...
        x = (
            ...
            .assign(
                ...
                alert_threshold=self.parent_assay.targeting.iopath[0].thresholds.alert,
            )
        )

        # Repeated values as convenience for V1 compatibility/helper
        x.insert(0, "iopath", iopath)
        x.insert(
            0, "workspace_id", self.parent_assay.targeting.data_origin.workspace_id
        )
        x.insert(
...
class AssayV2Builder:
    ...
    def preview(self, start: datetime, end: datetime):
        self._validate()
        ...
        if ret.parsed is None:
            raise Exception("An exception occured while previewing assay", ret.content)

        arr = [PreviewResult(x, self) for x in ret.parsed]
...
class PreviewResultList(List[PreviewResult]):
    ...
    def to_dataframe(self):
        ...
        x = pd.DataFrame([x.to_df_row() for x in self]).assign(
            warning_threshold=self.parent_assay.targeting.iopath[0].thresholds.warning,
            alert_threshold=self.parent_assay.targeting.iopath[0].thresholds.alert,
        )
