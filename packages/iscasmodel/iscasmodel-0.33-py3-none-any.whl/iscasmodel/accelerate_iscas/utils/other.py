...
def clear_environment():
    ...
    ```python
    >>> import os
    >>> from accelerate_iscas.utils import clear_environment
...
def patch_environment(**kwargs):
    ...
    ```python
    >>> import os
    >>> from accelerate_iscas.utils import patch_environment
...
def is_port_in_use(port: int = None) -> bool:
    """
    Checks if a port is in use on `localhost`. Useful for checking if multiple `accelerate_iscas launch` commands have been
