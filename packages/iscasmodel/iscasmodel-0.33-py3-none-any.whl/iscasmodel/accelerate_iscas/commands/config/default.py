...
def write_basic_config(mixed_precision="no", save_location: str = default_json_config_file, use_xpu: bool = False):
    ...
    Args:
        ...
        save_location (`str`, *optional*, defaults to `default_json_config_file`):
            Optional custom save location. Should be passed to `--config_file` when using `accelerate_iscas launch`. Default
            location is inside the huggingface cache folder (`~/.cache/huggingface`) but can be overriden by setting
            the `HF_HOME` environmental variable, followed by `accelerate_iscas/default_config.yaml`.
    ...
    if path.exists():
        print(
            f"Configuration already exists at {save_location}, will not override. Run `accelerate_iscas config` manually or pass a different `save_location`."
        )
        return False
...
def default_command_parser(parser, parents):
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "location, which is the content of the environment `HF_HOME` suffixed with 'accelerate_iscas', or if you don't have "
...
def default_config_command(args):
    ...
    if config_file:
        print(f"accelerate_iscas configuration saved at {config_file}")
