...
class _CustomHelpAction(argparse._HelpAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        if "accelerate_iscas" in sys.argv[0] and "launch" in sys.argv[1:]:
            args = sys.argv[2:]
        else:
            args = sys.argv[1:]
...
def sagemaker_launcher(sagemaker_config: SageMakerConfig, args):
    if not is_sagemaker_available():
        raise ImportError(
            "Please install sagemaker to be able to launch training on Amazon SageMaker with `pip install accelerate_iscas[sagemaker]`"
        )
...
def _validate_launch_command(args):
    ...
    if any(warned):
        message = "The following values were not passed to `accelerate_iscas launch` and had defaults used instead:\n"
        message += "\n".join(warned)
        message += (
            "\nTo avoid this warning pass in values for each of the problematic parameters or run `accelerate_iscas config`."
        )
        logger.warning(message)
