...
class Accelerator:
    ...
    Args:
        ...
        mixed_precision (`str`, *optional*):
            ...
            accelerate_iscas config of the current system or the flag passed with the `accelerate_iscas.launch` command. 'fp8'
    ...
    **Available attributes:**
    ...
    def __init__(
        ...
        if deepspeed_plugin is None:  # init from env variables
            ...
            ), "`deepspeed_plugin` must be an `accelerate_iscas.utils.DeepSpeedPlugin` object."
        ...
        if kwargs_handlers is not None:
            for handler in kwargs_handlers:
                ...
                ), f"Unsupported kwargs handler passed: {handler}, must be one that inherits `accelerate_iscas.utils.KwargsHandler`."
    ...
    def split_between_processes(self, inputs: list | tuple | dict | torch.Tensor, apply_padding: bool = False):
        ...
        from accelerate_iscas import Accelerator
        ...
        if (getattr(model, "is_loaded_in_8bit", False) or getattr(model, "is_loaded_in_4bit", False)) and getattr(
            ...
            if len(model_devices) > 1 and self.distributed_type != DistributedType.NO:
                raise ValueError(
                    ...
                    " Therefore you should not specify that you are under any distributed regime in your accelerate_iscas config."
                )
        ...
        if deepspeed_plugin.deepspeed_config["train_micro_batch_size_per_gpu"] == "auto":
            if is_dataloader_present:
                ...
                if any(bs is None for bs in batch_sizes):
                    raise ValueError(
                        "At least one of the dataloaders passed to `accelerate_iscas.prepare()` has `None` as batch size. "
                ...
                raise ValueError(
                    "When using DeepSpeed, `accelerate_iscas.prepare()` requires you to pass at least one of training or evaluation dataloaders "
        ...
        if optimizer is not None:
            if "optimizer" in deepspeed_plugin.deepspeed_config and not isinstance(optimizer, (DummyOptim)):
                raise ValueError(
                    ...
                    "create `accelerate_iscas.utils.DummyOptim` in the code."
                )
        ...
        if scheduler is not None:
            if "scheduler" in deepspeed_plugin.deepspeed_config and not isinstance(scheduler, (DummyScheduler)):
                raise ValueError(
                    ...
                    "create `accelerate_iscas.utils.DummyScheduler` in the code."
                )
            elif (
                ...
                raise ValueError(
                    ...
                    "pass in the `lr_scheduler_callable` parameter when using `accelerate_iscas.utils.DummyScheduler`."
                )

        if optimizer is not None and scheduler is not None:
            if isinstance(optimizer, (DummyOptim)) and not isinstance(scheduler, (DummyScheduler)):
                raise ValueError(
                    "You can only specify `accelerate_iscas.utils.DummyScheduler` in the code when using "
                    "`accelerate_iscas.utils.DummyOptim`."
                )
    ...
    def _prepare_megatron_lm(self, *args):
        ...
        if not megatron_lm_plugin.megatron_dataset_flag:
            ...
            if len(batch_sizes) == 0:
                raise ValueError(
                    "You must specify a training or evaluation dataloader in `accelerate_iscas.prepare()` when using Megatron-LM."
                )
        ...
        if scheduler is not None:
            ...
            if not is_dummy_scheduler:
                raise ValueError(
                    "You can't use a custom scheduler with Megatron-LM. Please use the `accelerate_iscas.utils.MegatronLMDummyScheduler` instead."
                )
    ...
    def _prepare_msamp(self, *args):
        ...
        if optimizer is None or model is None:
            raise ValueError(
                "You must pass a model and an optimizer together to `accelerate_iscas.prepare()` when using MS-AMP."
            )
            ...
            if self.deepspeed_config["zero_optimization"]["stage"] == 3:
                if model.zero_gather_16bit_weights_on_model_save():
                    ...
                    raise ValueError(
                        ...
                        "set `zero3_save_16bit_model` to True when using `accelerate_iscas config`. "
