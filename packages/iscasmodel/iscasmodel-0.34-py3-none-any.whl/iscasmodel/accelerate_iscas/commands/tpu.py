...
_description = "Run commands across TPU VMs for initial setup before running `accelerate_iscas launch`."
...
def tpu_command_parser(subparsers=None):
    ...
    config_args = parser.add_argument_group(
        "Config Arguments", "Arguments that can be configured through `accelerate_iscas config`."
    )
    config_args.add_argument(
        "--config_file",
        type=str,
        default=None,
        help="Path to the config file to use for accelerate_iscas.",
    )
    config_args.add_argument(
        "--tpu_name",
        default=None,
    ...
    pod_args.add_argument(
        "--install_accelerate",
        action="store_true",
        help="Whether to install accelerate_iscas on the pod. Defaults to False.",
    )
    pod_args.add_argument(
        "--accelerate_version",
        default="latest",
        help="The version of accelerate_iscas to install on the pod. If not specified, will use the latest pypi version. Specify 'dev' to install from GitHub.",
    )
    pod_args.add_argument(
...
def tpu_command_launcher(args):
    ...
    if args.accelerate_version == "dev":
        ...
        args.accelerate_version = "accelerate_iscas -U"
    elif isinstance(parse(args.accelerate_version), Version):
        args.accelerate_version = f"accelerate_iscas=={args.accelerate_version}"
    ...
    # Default to the shared folder and install accelerate_iscas
