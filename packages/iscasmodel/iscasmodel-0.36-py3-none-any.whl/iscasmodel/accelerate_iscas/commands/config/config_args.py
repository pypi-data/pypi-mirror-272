...
cache_dir = os.path.join(hf_cache_home, "accelerate_iscas")
    ...
    if config_file is not None:
        if not os.path.isfile(config_file):
            raise FileNotFoundError(
                ...
                "Please pass an existing file to `accelerate_iscas launch`, or use the the default one "
                "created through `accelerate_iscas config` and run `accelerate_iscas launch` "
...
class BaseConfig:
    ...
    def from_json_file(cls, json_file=None):
        ...
        if len(extra_keys) > 0:
            raise ValueError(
                f"The config file at {json_file} had unknown keys ({extra_keys}), please try upgrading your `accelerate_iscas`"
    ...
    def from_yaml_file(cls, yaml_file=None):
        ...
        if len(extra_keys) > 0:
            raise ValueError(
                f"The config file at {yaml_file} had unknown keys ({extra_keys}), please try upgrading your `accelerate_iscas`"
                " version or fix (and potentially remove) these keys from your config file."
            )
        return cls(**config_dict)
...
class SageMakerConfig(BaseConfig):
    ...
    num_machines: int = 1
    gpu_ids: str = "all"
    base_job_name: str = f"accelerate_iscas-sagemaker-{num_machines}"
