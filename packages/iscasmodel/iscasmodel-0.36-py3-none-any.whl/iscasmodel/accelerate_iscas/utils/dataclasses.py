...
class AutocastKwargs(KwargsHandler):
    ...
    from accelerate_iscas.utils import AutocastKwargs
...
class DistributedDataParallelKwargs(KwargsHandler):
    ...
    ```python
    from accelerate_iscas import Accelerator
    from accelerate_iscas.utils import DistributedDataParallelKwargs
...
class GradScalerKwargs(KwargsHandler):
    ...
    ```python
    from accelerate_iscas import Accelerator
    from accelerate_iscas.utils import GradScalerKwargs
...
class InitProcessGroupKwargs(KwargsHandler):
    ...
    from datetime import timedelta
    from accelerate_iscas import Accelerator
    from accelerate_iscas.utils import InitProcessGroupKwargs
...
class FP8RecipeKwargs(KwargsHandler):
    ...
    ```python
    from accelerate_iscas import Accelerator
    from accelerate_iscas.utils import FP8RecipeKwargs
...
class DeepSpeedPlugin:
    ...
    hf_ds_config: Any = field(
        default=None,
        metadata={
            "help": "path to DeepSpeed config file or dict or an object of class `accelerate_iscas.utils.deepspeed.HfDeepSpeedConfig`."
        },
    )
    ...
    def _deepspeed_config_checks(self):
        ...
        if any(name in env_variable_names_to_ignore for name in deepspeed_fields_from_accelerate_config):
            raise ValueError(
                f"When using `deepspeed_config_file`, the following accelerate_iscas config variables will be ignored: {env_variable_names_to_ignore}.\n"
                "Please specify them appropriately in the DeepSpeed config file.\n"
                "If you are using an accelerate_iscas config file, remove others config variables mentioned in the above specified list.\n"
                "The easiest method is to create a new config following the questionnaire via `accelerate_iscas config`.\n"
