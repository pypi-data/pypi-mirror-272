...
class MultiProcessAdapter(logging.LoggerAdapter):
    ...
    def log(self, level, msg, *args, **kwargs):
        ...
        if PartialState._shared_state == {}:
            raise RuntimeError(
                "You must initialize the accelerate_iscas state by calling either `PartialState()` or `Accelerator()` before using the logging utility."
            )
...
def get_logger(name: str, log_level: str = None):
    ...
    ```python
    >>> from accelerate_iscas.logging import get_logger
