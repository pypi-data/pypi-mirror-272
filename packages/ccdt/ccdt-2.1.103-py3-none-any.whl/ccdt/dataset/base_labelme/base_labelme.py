...
class BaseLabelme(metaclass=SingletonMeta):
        ...
        for dataset in tqdm(self.datasets):
            ...
            if dataset.get('background') is True:  # 如果不是背景就进行自动打组
                ...
                for shape_key, shapes_value in structure_shapes.items():
                    ...
                    if self.judge_polygon_whether_box(shapes_value, None) is False:
                        self.error_dataset_handle(dataset)
                    # 分组数量等一的，且是多边形标注的，必然标注存在错误。分组数量大于五的，必然打组存在错误。
                    elif len(shapes_value) > 5 or len(shapes_value) == 1:  # 如果分组数量大于3，不等于2，则自动打组出错。这里有个Bug当shapes_value为两个车牌多边形标注的时候，没有车，抠出来的图像为黑框。（需要增加逻辑判断）
                        self.error_dataset_handle(dataset)
                    # 分组数量等于五的，且group_id值都相同的，必然打组存在错误。or 分组数量等于四的，且group_id值都相同的，必然打组存在错误。或者等于3等于2
                    elif len(shapes_value) == 5 or len(shapes_value) == 4:
                        if self.judge_polygon_whether_box(shapes_value, "group_id") is True:
                            self.error_dataset_handle(dataset)
                        else:  # group_id值存在不同的情况，需要单独保存人工查看，矫正标注问题
                            self.cut_out_labelme_image(shape_key, shapes_value, dataset, "please_manually_check_data")  # 截取矩形框标注图像，并保存多边形属性为labelme数据集格式
                    # 国内车牌一辆车一块车牌，香港车一辆车两块车牌
                    elif shapes_value[0].get("shape_type") == shapes_value[1].get("shape_type") == "polygon":  # 根据排序规则，矩形框第一，多边形第二，如果第一第二都是多边形则标注有问题
                        self.error_dataset_handle(dataset)
                    else:  # 如下抠图执行的逻辑，必须是正确分组的标注才能够执行，否则抠图会出错。如果所有条件都为 False，执行这个代码块
    ...
    def judge_polygon_whether_box(shapes_value, group_id):
        ...
        if len(list_of_rectangle) >= 2 or len(list_of_rectangle) == 0:  # 如果矩形框存在两个,或者一个都不存在，返回false，必然打组出错
            return False
