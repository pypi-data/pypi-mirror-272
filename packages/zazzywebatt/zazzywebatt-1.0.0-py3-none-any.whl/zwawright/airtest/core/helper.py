...
from package.airtest.utils.logwraper import Logwrap, AirtestLogger
...
class G(object):
    ...
    LOGGING = get_logger("package.airtest.core.api")
    SCREEN = None
    DEVICE = None
    DEVICE_LIST = []
    RECENT_CAPTURE = None
...
def log(arg, timestamp=None, desc="", snapshot=False, image_path=None):
    """
    ...
    Examples:
        >>> log("hello world", snapshot=True, image_path="path/to/image.png")
        >>> log({"key": "value"}, timestamp=time.time(), desc="log dict", image_path="path/to/image.png")
        >>> try:
                1/0
            except Exception as e:
                log(e, image_path="path/to/image.png")

    """
    from package.airtest.core.cv import try_log_screen
    if G.LOGGER:
        depth = 0
        if snapshot:
        ...
        if isinstance(arg, Exception):
            ...
            G.LOGGER.log("info", log_data, depth=depth, timestamp=timestamp)
...
def import_device_cls(platform):
    ...
    if platform in G.CUSTOM_DEVICES:
        ...
        from package.airtest.core.android.android import Android as cls
    elif platform == "windows":
        from package.airtest.core.win.win import Windows as cls
    elif platform == "ios":
        from package.airtest.core.ios import IOS as cls
    elif platform == "linux":
        from package.airtest.core.linux.linux import Linux as cls
    else:
