from ..one_asset_option_base import interpolate, np, spilu, sp, OneAssetFDSnowball


class SnowballPde(OneAssetFDSnowball):
    ...
    def _solve_intraday_(self, idx, KIgrid, NKIgrid):
        if self._Tau > 0 and self._Nt > 0:
            ...
            U = KIgrid[1: -1, idx + 1].copy()
            U[0] += self._l[0] * dt / self.param.year_base * KIgrid[0, idx]
            U[-1] += self._u[-1] * dt / self.param.year_base * KIgrid[-1, idx]
            KIgrid[1: -1, idx] = iluM.solve(U)
    ...
    def _solve_(self):
        ...
        for j in list(reversed(np.arange(self._Nt))):
            ...
            U = self.KIgrid[1: -1, j + 1].copy()
            U[0] += self._l[0] * self.dt / self.param.year_base * self.KIgrid[0, j]
            U[-1] += self._u[-1] * self.dt / self.param.year_base * self.KIgrid[-1, j]
            self.KIgrid[1: -1, j] = iluM.solve(U)
