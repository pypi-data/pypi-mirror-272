...
class EnhancedAsianSpread(OneAssetOptionBase):

    def __calculate_present_value__(self) -> float:
        assert {OptionType.CALL: self.param.enhanced_strike_price2 >= self.param.enhanced_strike_price1 >= self.param.strike_price,
                OptionType.PUT: self.param.enhanced_strike_price2 <= self.param.enhanced_strike_price1 <= self.param.strike_price}[self.param.option_type]
        ...
        for obs in self.param.obs_date:
            ...
            param['strike_price'] = self.param.enhanced_strike_price1
            ...
            param['exercise_type'] = self.param.exercise_type
            param['spot_price'] = obs.obs_price if self.param.current_date > obs.obs_index else self.param.spot_price
            param['strike_price'] = self.param.enhanced_strike_price2
            param['expiry_date'] = obs.obs_index
            param['current_date'] = self.param.current_date
            param['volatility'] = self.param.volatility
            param['riskfree_rate'] = self.param.riskfree_rate
            ...
            rst = rst + (vanilla1.present_value() - vanilla2.present_value()) / len(self.param.obs_date)
        return rst + phi * (self.param.enhanced_strike_price1 - self.param.strike_price) * df
