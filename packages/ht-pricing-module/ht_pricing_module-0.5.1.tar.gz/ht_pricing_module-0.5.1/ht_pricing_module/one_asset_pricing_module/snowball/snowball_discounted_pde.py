...
class SnowballDiscountedPde(OneAssetFDSnowball):
    ...
    def _set_terminal_condition_(self):
        ...
        self.NKIgrid[self._S_KO, -1] = (self.param.bonus_rate - self._IntVec[-1]) * self.param.notional
    ...
    def _set_boundary_condition_(self):
        ...
        self.KIgrid[0, :] = (self._omega * ((self.Svec[0] - self.param.knock_in_strike_price) / self.param.entrance_price) - self._IntVec) * self.param.notional
        self.KIgrid[-1, :] = (self._omega * ((self.Svec[-1] - self.param.knock_in_strike_price) / self.param.entrance_price) - self._IntVec) * self.param.notional
        if self._is_call:
            ...
            if not np.isscalar(self._Rbt_KO) and len(self._Rbt_KO) > 1:
                ...
                self.NKIgrid[-1, :] = (f1(self.Tvec) - self._IntVec) * self.param.notional * np.exp(-self.param.riskfree_rate * (f2(self.Tvec) - self.Tvec) / self.param.year_base)
            else:
                self.NKIgrid[-1, :] = (self._Rbt_KO - self._IntVec) * self.param.notional * DFr
        else:
