...
from ..sharkfin.basket_sharkfin_as import BasketSharkfin
from ..basket.basket_vanilla_as import BasketVanilla


class BasketLinearAcc(OneAssetOptionBase):
    ...
    def __calculate_present_value__(self) -> float:
        ...
        for obs in self.param.obs_date:
            ...
            param['riskfree_rate'] = self.param.riskfree_rate
            param['year_base'] = self.param.year_base
            basket_sharkfin2 = BasketSharkfin(param)
            rst = rst + (-self.param.leverage * basket_vanilla1.present_value() + basket_sharkfin2.present_value())
        return rst
