...
/*
TODOs for better observation:

ADD OBJECT SYMBOLS TO THE OBSERVATION
*/
...
namespace nle_language_obsv
{
  ...
  private:
    ...
    std::unordered_map<int64_t, std::string> encumbrance_map{
        {UNENCUMBERED, "Unencumbered"}, {SLT_ENCUMBER, "Burdened"}, {MOD_ENCUMBER, "Stressed"}, {HVY_ENCUMBER, "Strained"}, {EXT_ENCUMBER, "Overtaxed"}, {OVERLOADED, "Overloaded"}};

    std::unordered_map<int64_t, std::string> hunger_map{
        {SATIATED, "Satiated"}, {NOT_HUNGRY, "Not Hungry"}, {HUNGRY, "Hungry"}, {WEAK, "Weak"}, {FAINTING, "Fainting"}, {FAINTED, "Fainted"}, {STARVED, "Starved"}};
                                         ...
                                         py::array_t<int64_t> blstats)
  {
         ...
         it != glyph_distance_direction.end(); it++)
    {
      ...
      if (idx + 1 < (glyph_distance_direction.size()))
        output += "\n";
      idx++;
    }
    return py::bytes(output);
  ...
  py::bytes NLELanguageObsv::text_message(py::array_t<uint8_t> tty_chars)
  {
    ...
    for (uint64_t row_idx = 0; row_idx < rows; row_idx++)
    {
      ...
      if (death_screen && blank_row_count > 1)
        return py::bytes(output);
    }
    return py::bytes(first_row_str);
  }
...
namespace nle_hybrid_obsv
{
  ...
  private:
    ...
    const std::set<std::string> cmap_screen_include{
        ...
        "vertical raised drawbridge",
        "horizontal raised drawbridge",
        /*Added the below to have more observations regarding traps*/
        "arrow trap",
        "dart trap",
        "falling rock trap",
        "squeaky board",
  ...
  void NLEHybridObsv::build_screen_pos_to_distance_direction()
  {
    for (uint64_t x = 0; x < DUNGEON_WIDTH * 2; x++)
    {
      for (uint64_t y = 0; y < DUNGEON_HEIGHT * 2; y++)
  ...
  void NLEHybridObsv::build_noun_plural_lookup()
  {
    for (auto screen_it = fullscreen_view_glyph_map.begin();
         screen_it != fullscreen_view_glyph_map.end(); ++screen_it)
    {
  ...
  std::string NLEHybridObsv::pluralize(std::string noun)
  {
    std::string plural;
    // Mass nouns
    if (noun.size() >= 5 && noun.substr(noun.size() - 5) == "boots")
  ...
  std::list<std::tuple<std::string, std::string, std::string>>
  NLEHybridObsv::compress_by_glyph(
      std::list<std::tuple<std::string, std::string, std::string>>
          glyph_distance_direction)
  {
    std::list<std::tuple<std::string, std::string, std::string>>
  ...
  std::pair<int, int> NLEHybridObsv::local_glyph_to_global(int glyph_idx,
                                                           int player_x,
                                                           int player_y)
  {
    int glyph_x = glyph_idx % DUNGEON_WIDTH;
  ...
  std::list<std::tuple<std::string, std::string, std::string>>
  NLEHybridObsv::sort_by_distance_direction(
      std::list<std::tuple<std::string, std::string, std::string>>
          glyph_distance_direction)
  {
    std::list<std::tuple<std::string, std::string, std::string>>
  ...
  std::list<std::tuple<std::string, std::string, std::string>>
  NLEHybridObsv::ray_march(DIRECTION direction, int64_t player_x,
                           int64_t player_y, int16_t *glyphs_data)
  {
    std::list<std::tuple<std::string, std::string, std::string>>
        glyph_distance_direction;
  ...
  std::list<std::tuple<std::string, std::string, std::string>>
  NLEHybridObsv::fullscreen_view(int16_t *glyphs_data, int64_t *blstats_data)
  {
    int64_t player_x = blstats_data[0];
    int64_t player_y = blstats_data[1];
  ...
  std::list<std::tuple<std::string, std::string, std::string>>
  NLEHybridObsv::visual_view(int16_t *glyphs_data, int64_t *blstats_data)
  {
    int64_t player_x = blstats_data[0];
    int64_t player_y = blstats_data[1];
  ...
  void NLEHybridObsv::build_visual_view_glyph_map()
  {
    visual_view_glyph_map.fill("");
    for (int16_t glyph = 0; glyph < MAX_GLYPH; glyph++)
    {
      if (glyph >= GLYPH_CMAP_OFF && glyph < GLYPH_EXPLODE_OFF)
      { /* cmap */
  ...
  void NLEHybridObsv::build_fullscreen_view_glyph_map()
  {
    fullscreen_view_glyph_map.fill("");
    for (int16_t glyph = 0; glyph < MAX_GLYPH; glyph++)
    {
      if (glyph >= GLYPH_STATUE_OFF)
      {
  ...
  py::bytes NLEHybridObsv::text_cursor(py::array_t<int16_t> glyphs,
                                       py::array_t<int64_t> blstats,
                                       py::array_t<int64_t> tty_cursor)
  {
    py::buffer_info glyphs_buffer = glyphs.request();
  ...
  py::bytes NLEHybridObsv::text_glyphs(py::array_t<int16_t> glyphs,
                                       py::array_t<int64_t> blstats)
  {
    py::buffer_info glyphs_buffer = glyphs.request();
    py::buffer_info blstats_buffer = blstats.request();
  ...
  py::bytes NLEHybridObsv::text_inventory(py::array_t<uint8_t> inv_strs,
                                          py::array_t<uint8_t> inv_letters)
  {
    py::buffer_info inv_strs_buffer = inv_strs.request();
    py::buffer_info inv_letters_buffer = inv_letters.request();
  ...
  std::pair<std::string, std::string> NLEHybridObsv::pos_to_str(int x, int y)
  {
    int diag_dist = diagonal_distance(x, y);
    std::string diagonal = "";
    if (diag_dist > 0)
  ...
  std::string NLEHybridObsv::offset_to_str(int offset)
  {
    if (offset == 1)
      return "adjacent";
    else if (offset == 2)
  ...
  int NLEHybridObsv::diagonal_distance(int dx, int dy)
  {
    dx = abs(dx);
    dy = abs(dy);
    int diagonal_steps = min(dx, dy);
  ...
  py::bytes NLEHybridObsv::text_blstats(py::array_t<int64_t> blstats)
  {
    py::buffer_info blstats_buffer = blstats.request();
    int64_t *blstats_data = reinterpret_cast<int64_t *>(blstats_buffer.ptr);
  ...
  std::string NLEHybridObsv::trim(std::string input)
  {
    const std::string WHITESPACE = " \32\0\n\r\t\f\v";
    size_t start = input.find_first_not_of(WHITESPACE);
    if (start == (input.size() - 1))
  ...
  py::bytes NLEHybridObsv::text_message(py::array_t<uint8_t> tty_chars)
  {
    py::buffer_info tty_chars_buffer = tty_chars.request();
    uint8_t *tty_chars_data = reinterpret_cast<uint8_t *>(tty_chars_buffer.ptr);
...
PYBIND11_MODULE(nle_language_obsv, m)
{
  ...
  py::class_<nle_hybrid_obsv::NLEHybridObsv>(m, "NLEHybridObsv")
      .def(py::init<>())
      .def("text_glyphs", &nle_hybrid_obsv::NLEHybridObsv::text_glyphs,
           "Convert glyphs to text description")
      .def("text_blstats", &nle_hybrid_obsv::NLEHybridObsv::text_blstats,
           "Convert blstats to text description")
      .def("text_inventory",
           &nle_hybrid_obsv::NLEHybridObsv::text_inventory,
           "Convert inventory to text description")
      .def("text_cursor", &nle_hybrid_obsv::NLEHybridObsv::text_cursor,
           "Convert tty_cursor to text description")
      .def("text_message", &nle_hybrid_obsv::NLEHybridObsv::text_message,
           "Convert tty_chars to text message including menus");
}
