...
from nle_language_wrapper.nle_language_obsv import NLELanguageObsv, NLEHybridObsv
...
class NLELanguageWrapper(Wrapper):
    ...
    def clean_message(self, nle_obsv):
        message = self.nle_language.text_message(nle_obsv["tty_chars"]).decode(
            "latin-1"
        )
        done = False
        while "--More--" in message and not done:
            ...
            add = self.nle_language.text_message(nle_obsv["tty_chars"]).decode(
                "latin-1"
            )
            message += add
        return message, nle_obsv

    def nle_hybrid_obsv(self, nle_obsv):
        """Translate NLE Observation into a hybrid vision-language observation.
        Args:
            nle_obsv (dict): NLE observation from the base environment
        Returns:
            (dict): vision-language observation
        """
        ...
        return f"""\nInventory:\n{self.nle_hybrid.text_inventory(
                inv_strs, inv_letters
            ).decode("latin-1")}\n\nMap observation:\n{self.nle_hybrid.text_glyphs(glyphs, blstats).decode(
                "latin-1"
            )}\n\n{ascii_map}\n
        """
    ...
    def post_reset(self, obsv):
        ...
        Returns:
            (dict): language observation
        """
        return self.nle_process_obsv(obsv)
