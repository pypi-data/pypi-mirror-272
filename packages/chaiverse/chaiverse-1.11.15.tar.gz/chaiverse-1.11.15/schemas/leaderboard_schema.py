...
class Leaderboard(BaseModel, extra=Extra.allow):
    ...
    def to_display_df(self, includes=None, excludes=None, sort_params=None, header_mapping=None) -> pd.DataFrame:
        ...
        df = self.df_aggregated
        if len(df) > 0:
            df = _filter_out_rejected_models(df)
...
def _add_repo_aggregate_columns(df):
    df['model_repo'] = df.get('model_repo', '')
    ...
    for column in ["celo_rating", "double_thumbs_up_ratio", "thumbs_down_ratio", "win_ratio", "safety_score", "model_score"]:
        groupby_result = df.where(df["best_of"] == 1).groupby("model_repo")[column]
        df[f"{column}_by_repo_open"] = groupby_result.transform("max") if len(groupby_result) else float('nan')
    for column in ["feedback_count", "num_battles"]:
        groupby_result = df.where(df["best_of"] == 1).groupby("model_repo")[column]
        df[f"{column}_by_repo_open"] = groupby_result.transform("sum") if len(groupby_result) else float('nan')
    return df


def _filter_out_rejected_models(df):
