...
class LabelPowersetTransformation:
    ...
    def transform_dataset(self, dataset: MIMLDataset) -> MIMLDataset:
        ...
        for label in labels:
            print("label: ", label)
            print("operation: ", 2**np.arange(label.shape[0]))
            labels_transformed.append(np.dot(np.flip(label), 2 ** np.arange(len(label))))
        for i in range(number_labels, 1, -1):
            dataset.delete_attribute(self.dataset.get_number_features()+i)

        dataset.show_dataset()
        #dataset.add_attribute(self.dataset.get_number_attributes(),labels_transformed)
