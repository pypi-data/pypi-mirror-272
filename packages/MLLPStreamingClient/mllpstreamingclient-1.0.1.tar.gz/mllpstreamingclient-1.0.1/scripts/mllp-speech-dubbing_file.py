...
for resp in cli.Text2Speech(args.t2s_system_id, cli.Text2Text(args.t2t_system_id, cli.Speech2Text(args.s2t_system_id, myAudioStreamIterator)), args.t2s_language_code):
    if "audio_data" in resp:
        adata = np.concatenate((adata, np.frombuffer(resp["audio_data"], dtype=np.int16)))
