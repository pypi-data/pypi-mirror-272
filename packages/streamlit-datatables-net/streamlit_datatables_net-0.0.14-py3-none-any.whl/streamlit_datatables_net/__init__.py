...
def st_datatable(data: pd.DataFrame | list[dict], column_order: List[str] = None, props: Dict = None, dt_config: str = "default", table_id: str = None, class_name: str = None, js_scripts: List[str] = None, stylesheets: List[str] = None, key=None):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if column_order:
            props["columns"] = [{"data": column, "title": column}
        ...
        if column_order:
            props["columns"] = [{"data": column, "title": column}
                                for column in column_order]
        elif "columns" not in props and data:
            df = pd.json_normalize(data)
            df_dict = df.to_dict(orient="records")
            columns = list(df_dict[0].keys())
            props["columns"] = props["columns"] = [{"data": column, "title": column}
                ...
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, column_order=column_order, props=props, table_id="example_1", class_name=class_name, js_scripts=js_scripts, stylesheets=stylesheets, key="example_1")
                        st.json(dt_response)
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            data, props=props, table_id="example_1", class_name=class_name, js_scripts=js_scripts, stylesheets=stylesheets, key="example_1b")
                        st.json(dt_response)
        ...
        with st.container(border=True):
            ...
            with st.expander("Code"):
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(data, key="example_2")
                        st.json(dt_response)
