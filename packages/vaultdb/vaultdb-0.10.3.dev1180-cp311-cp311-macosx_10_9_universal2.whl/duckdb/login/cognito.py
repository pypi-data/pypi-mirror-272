...
def cognito(username: str, password: str, path: str=":memory:", aws_region: str='us-east-2', **userpoolargs)->DuckDBPyConnection:
    ...
    connection.execute(
        f"CREATE OR REPLACE SECRET {username} (TYPE S3, PROVIDER config, KEY_ID '{aws_cred['AccessKeyId']}', SECRET '{aws_cred['SecretKey']}', URL_COMPATIBILITY_MODE true, USE_SSL true, REGION '{aws_region}', SESSION_TOKEN '{aws_cred['SessionToken']}')"
    )
