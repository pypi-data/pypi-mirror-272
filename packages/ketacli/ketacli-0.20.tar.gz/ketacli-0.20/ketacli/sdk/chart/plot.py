        ...
        if self.field_group and self.field_group in self.data.header:
            ...
            for group_name in group_names:
                if self.data.header[self.field_x_index] == "_time":
                    ...
                    x = [datetime.utcfromtimestamp(x[self.field_x_index] / 1000).strftime('%Y-%m-%d %H:%M:%S') for x in
                ...
                for y in self.field_y_index:
                    ydata.append([x[y] for x in self.data.rows if x[field_group_index] == group_name])
                for y in ydata:
            ...
            ydata = []
            for y in self.field_y_index:
                ydata.append([x[y] for x in self.data.rows])
