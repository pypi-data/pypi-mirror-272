...
from ketacli.sdk.chart.layout import create_layout
...
def list(asset_type, groupId=-1, order="desc", pageNo=1, pageSize=10, prefix="", sort="updateTime", fields="",
         format=None, raw=False, lang=None, extra=None, watch=False, interval=3.0):
...
@command
def admin(asset_type, format=None, extra=None, watch=False, interval=3.0):
    """List asset (such as repo,sourcetype,metric...) from ketadb
...
def plot(spl, start=None, end=None, limit=100, interval=3.0,
         ...
         y_field="value", group_field="", extra=None):
    ...
    def generate_table():
        resp = search_spl(spl=spl, start=start, end=end, limit=limit)
        return search_result_output(resp)

    layout = create_layout(rows=1, cols=1, layout_configs=None)

    phase = 0
    with Live(layout, refresh_per_second=0.01) as live:
        while True:
            try:
                data = generate_table()
                plot = Plot(field_x=x_field, field_y=[y_field], data=data, title=title, x_label=x_label,
                            y_label=y_label,
                            field_group=group_field, plot_type=type, **extra_args_map)
                phase += 1
                panel = Panel(PlotextMixin(make_plot=plot.build))
                layout['row0-col0'].update(panel)
                time.sleep(interval)
                live.refresh()
            except KeyboardInterrupt:
                live.stop()
...
@command
def dashboard(chart=None, file_path=None, interval=30, theme=""):
    ...
    layout = create_layout(rows=layout_config.get("rows", 1), cols=layout_config.get("columns", 1),
                           layout_configs=layout_config.get('configs', {}))

    def get_chart(theme=None):
        for chart_config in chart_configs:
            if not theme:
                theme = chart_config.get("theme", layout_config.get("theme", "dark"))
            ...
            if chart_type in ["line", "bar", "scatter"]:
                ...
                plot = Plot(field_x=x_field, field_y=y_field, data=search_data, title=title,
                            ...
                            field_group=group_field, plot_type=chart_type, theme=theme, **extra_data)
                panel = Panel(PlotextMixin(make_plot=plot.build))
                layout[position].update(panel)
            elif chart_type == "single":
                title = chart_config.get("title", "")
                single_field = chart_config.get("single_field", "")
                extra_fields = chart_config.get("extra_fields", [])
                suffix = chart_config.get("suffix", "")
                prefix = chart_config.get("prefix", "")
                single_chart = SingleValueChart(search_data, single_field=single_field, title=title,
                                                extra_fields=extra_fields, suffix=suffix, prefix=prefix)
                layout[position].update(single_chart)
            elif chart_type == "table":
                title = chart_config.get("title", "")
                table = KTable(search_data, title=title)
                layout[position].update(table)
        return layout

    with Live(get_chart(theme), refresh_per_second=0.01) as live:
        while True:
            try:
                get_chart(theme)
                time.sleep(interval)
...
def config(operation, name=None):
    ...
    :param -n, --name: Setting default cluster
    """
    resp = []
...
def mock_data(repo="default", data=None, file=None, number=1):
    ...
    datas = []
    resps = []
    for i in track(range(number), description="Mocking..."):
