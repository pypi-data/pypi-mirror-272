...
def search_result_output(result=None):
    if result is None:
        result = {}
    header = []
    ...
    if "methods" in conf:
        ...
        for m in methods_obj:
            if isinstance(methods_obj[m], dict):
                cmd = help_methods(rs, m, json.dumps(methods_obj[m].get("data", {}), ensure_ascii=False))
            else:
...
def help_methods(rs: str, m: str = "list", example=None):
    if m == "list":
    ...
    if m == "close":
        return f"关闭资源：ketacli update {rs} --name \"resource_id_or_name\" -o {m}"
    if m == "open":
        return f"打开资源：ketacli update {rs} --name \"resource_id_or_name\" -o {m}"
    if m == "delete":
        return f"删除资源：ketacli delete {rs} [--extra key=value] [--data '{example}']"
