...
from .compat import load_local_result, transform_device_data
        ...
        if data.get("qbraidDeviceId") == "qbraid_qir_simulator":
            ...
            if status != "ONLINE":
                raise QuantumServiceRequestError(
                    "The qir-runner simulator is not currently available"
                )

            bitcode = data.pop("bitcode")
            entrypoint = data.pop("entrypoint")
            shots = data.get("shots")
            vendor_job_id = simulator.run(bitcode, entrypoint, shots=shots)
            data["vendorJobId"] = vendor_job_id
            data["status"] = "COMPLETED"
            data["qbraidStatus"] = "COMPLETED"
