...
def cut_roi(r, int_tol=1000, sigma=1.5, prominence_ratio=0.1):
    """
    ...
    if len(r.int_seq) < 10 or r.peak_height < 3*int_tol or r.noise_level > 0.6:
        return [r]

    ss = gaussian_filter1d(r.int_seq, sigma=sigma)
    peaks, _ = find_peaks(ss, prominence=np.max(ss)*prominence_ratio, distance=5)
