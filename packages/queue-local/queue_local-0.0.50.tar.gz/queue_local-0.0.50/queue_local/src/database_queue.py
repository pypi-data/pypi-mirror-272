...
class DatabaseQueue(OurQueue, GenericCRUD, metaclass=MetaLogger, object=queue_logger_object):
    ...
    def __init__(self, *, schema_name: str = "queue", table_name: str = "queue_item_table",
                 view_name: str = "queue_item_view", column_name: str = "queue_item_id",
        ...
        GenericCRUD.__init__(self, default_schema_name=schema_name, default_table_name=table_name,
                             default_view_table_name=view_name, default_column_name=column_name,
                             is_test_data=is_test_data)
        self.user_context = UserContext()

    # TODO queue_data_dict
    def push(self, data_dict: dict) -> int:
        """Pushes a new entry to the queue and returns the new queue id.
        data_dict consists of columns and values to insert into the queue table."""
        created_user_id = UserContext().get_effective_user_id()

        self._fix_parameters_json(data_dict)

        data_dict["queue_status_id"] = QueueStatusEnum.NEW.value
        data_dict["created_user_id"] = created_user_id

        if self.column_name in data_dict:
            column_value = data_dict[self.column_name]
            del data_dict[self.column_name]
            self.update_by_column_and_value(column_value=column_value, data_dict=data_dict)
            push_result = column_value
        else:
            push_result = self.insert(data_dict=data_dict)
        # TODO What we return? queue_id, message_id ...? Can we give it more meaningful name?
        return push_result
        # TODO Can we do this everywhere?  [I will make it auto in the meta logger]
        # TODO logger.end("...", {'push_result': push_result} )

    def push_back(self, queue_item: dict) -> None:
        """Push a taken queue item back to the queue."""
        data_dict = {"queue_status_id": QueueStatusEnum.NEW.value, "process_id": None}
        self.update_by_column_and_value(column_value=queue_item[self.column_name], data_dict=data_dict)

    def get(self, *, action_ids: tuple = (), custom_condition: str = "") -> dict:
        """
        ...
        data_dict = {"process_id": os.getpid()}
        ...
        self.update_by_where(where=update_where, order_by=self.column_name, limit=1, data_dict=data_dict)

        # TODO In which cases queue_status_id IS NULL? - Does it indicate a problem/bug?
        select_where = f"process_id = {os.getpid()} AND (queue_status_id IS NULL OR queue_status_id = {QueueStatusEnum.NEW.value})"
        queue_item = self.select_one_dict_by_where(where=select_where)
        if queue_item:
            update_data_dict = {"queue_status_id": QueueStatusEnum.RETRIEVED.value,
                                "user_jwt": self.user_context.get_user_jwt()}
            self.update_by_column_and_value(column_value=queue_item[self.column_name], data_dict=update_data_dict)
    ...
    def peek(self, action_ids: tuple = ()) -> dict:
        ...
        queue_item = self.select_one_dict_by_where(where=where, order_by=self.column_name)
        return queue_item
    ...
    @staticmethod
    def _fix_parameters_json(data_dict: dict):
