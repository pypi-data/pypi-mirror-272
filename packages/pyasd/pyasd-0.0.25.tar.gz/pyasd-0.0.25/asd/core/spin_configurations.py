...
def gen_regular_order_on_triangular_lattice(conf_name='Tetrahedra', latt_choice=2):
    ...
    if conf_name=='FM':
        latt_muc, sites_muc = build_latt('triangular',1,1,1,latt_choice=2,return_neigh=False)
        sp_lat = np.zeros((1,1,1,3),float)
        sp_lat[...,2] = 1   

    elif conf_name=='Tetrahedra':
        latt_muc, sites_muc = build_latt('triangular',2,2,1,latt_choice=2,return_neigh=False)
        ...
        latt_muc, sites_muc = build_latt('triangular',1,1,1,latt_choice=1,return_neigh=False)
        r3h = np.zeros(3)/2
...
def gen_regular_order_on_square_lattice(conf_name, Neel_vec=np.array([0,0,1])):
    ...
    if conf_name=='FM':
        latt_muc, sites_muc = build_latt('square',1,1,1,latt_choice=2, return_neigh=False)
        conf = np.zeros((1,1,1,3))
        conf[0,0,0] = Neel_vec
    elif conf_name=='AFM':
        latt_muc, sites_muc = build_latt('square',1,1,1,latt_choice=1, return_neigh=False)
...
def gen_regular_order_on_kagome_lattice(conf_name, Neel_vec=np.array([0,0,1])):
    ...
    latt_muc, sites_muc = build_latt('kagome',1,1,1,return_neigh=False)
    ...
    if conf_name=='FM':
        ...
        latt, sites_muc = build_latt('kagome',2,2,1,return_neigh=False)
        sp_lat = np.zeros((2,2,1,3),float)
        sp_lat[0,0,0] = np.array([1,1,1])
        sp_lat[1,0,0] = np.array([-1,-1,1])
        sp_lat[0,1,0] = np.array([1,-1,-1])
        ...
        latt_muc, sites_muc = build_latt('kagome',2,2,1,return_neigh=False)
...
def regular_order(lat_type='triangular',conf_name='Tetrahedra',Neel_vec=np.array([0,0,1]),nnx=1,nny=1,latt_choice=2):
    ...
    if lat_type=='Lieb':       result = gen_regular_order_on_Lieb_lattice(conf_name,Neel_vec=Neel_vec)
