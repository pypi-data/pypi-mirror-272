...
def data_comparison_check(data, country):
    ...
    if user_data:
        ...
        # print(f"Extracted last name: {last_name.replace('-', '').replace('_', '')}")
        # print(f"User data l_name: {user_data.get('last_name')}")
...
def data_consistency_check(data, front_id_text, back_id_text, country):
    ...
    if country == 'QAT':
        ...
        if id_number and serial_number:
            ...
            print(f"SERIAL NUMBER 2: {id_number_from_sr_no}")
...
def image_integrity_check(data, front_id_text, back_id_text, coloured, blurred, glare, missing_fields, country):
    ...
    if country == 'UAE':
        print(f"logo: {data['front_logo_result']}")
