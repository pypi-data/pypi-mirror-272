...
from flask.globals import current_app, request_ctx
...
from .helpers import root_transaction, url_for
...
class Nereid(Flask):
    ...
    #: Remove unused attribute and property database
    #:
    #: .. versionremoved:: 7.0.7
    ...
    _url_route_rule_cache = MemoryCache('nereid.route.rule')
    ...
    def get_urls(self):
        ...
        """
        rules = self._url_route_rule_cache.get(self.database_name)
        if rules is not None:
            return rules
        rules = []
        models = self.pool._pool[self.database_name]['model']
                ...
                for rule in f._url_rules:
                    ...
                    if rule_obj.is_csrf_exempt:
                        self.csrf_protection._exempt_views.add(
                            rule_obj.endpoint)
        self._url_route_rule_cache.set(self.database_name, rules)
        return rules
        ...
        for model_name, model in models.items():
            for f_name, f in inspect.getmembers(
                ...
                if hasattr(f, '_context_processor'):
                    ctx_proc_as_func = getattr(self.pool.get(
                            model_name), f_name)
        ...
        for model_name, model in models.items():
            for f_name, f in inspect.getmembers(
                ...
                if hasattr(f, '_template_filter'):
                    filter = getattr(self.pool.get(model_name), f_name)
    ...
    def load_backend(self):
        ...
        also initialises the pool on the go
        """
    ...
    def _dispatch_request(self, req, language, active_id):
        ...
        with Transaction().set_context(language=language):
            ...
            if req.url_rule.endpoint in self.view_functions:
                ...
                meth = getattr(self.pool.get(model), method)
            ...
            if not inspect.isfunction(meth):
                ...
                model = self.pool.get(req.url_rule.endpoint.rsplit('.', 1)[0])
                i = model(active_id)
                try:
                    i.rec_name
                except UserError:
