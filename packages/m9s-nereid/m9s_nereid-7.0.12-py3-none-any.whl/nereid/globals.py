# The COPYRIGHT file at the top level of this repository contains the full
# copyright notices and license terms.
from flask.globals import (  # noqa
    LocalProxy, current_app, g, request, request_ctx, session)
from flask_login import current_user  # noqa


def _find_cache():
    """
    The application context will be automatically handled by
    _find_app method in flask
    """
    app = current_app
    return app.cache


def _get_locale():
    Locale = current_app.pool.get('nereid.website.locale')
    locale_id = getattr(request_ctx, 'locale', None)
    if locale_id is None:
        locale_id = _set_locale()
    return Locale(locale_id)


def _set_locale():
    website = _get_website()
    locale = website.get_current_locale(request_ctx.request)
    request_ctx.locale = locale.id
    return locale.id


def _get_website():
    Website = current_app.pool.get('nereid.website')
    website_id = getattr(request_ctx, 'website', None)
    if website_id is None:
        website_id = _set_website()
    return Website(website_id)


def _set_website():
    Website = current_app.pool.get('nereid.website')
    website = Website.get_from_host(request_ctx.request.host)
    request_ctx.website = website.id
    return website.id


cache = LocalProxy(_find_cache)
current_locale = LocalProxy(lambda: _get_locale())
current_website = LocalProxy(lambda: _get_website())
