...
def run_convert(name="ag_news", diroot: str = "./ztmp/hf_datasets", 
    ...
    if name == "cnn_dailymail": dataset = datasets.load_dataset(name, "3.0.0")    
    ...
    nrows=0    
    for key in splits:
        for k, data_k in  enumerate( dataset[key].batch(kbatch)):
           if k > kmax: break  ## Break

           dfk = pd.DataFrame(data_k)
           dfk = convert_fun(dfk, meta_dict=cc)
    ...
    json_save(cc.to_dict(), f"{diroot}/meta/{name2}.json")
