...
class CIR(Model1D):
    ...
    def __init__(self, expo_scale_bessel: bool = True):
        """
        :param expo_scale_bessel: bool, True by default. Set to false to experiment with the numerical
            instability of the exact transition density when the exponential scaling technique is not applied
            to control underflow and overflow.
            In general, expo_scale_bessel should always be set to True in practical applications
        """
        super().__init__(has_exact_density=True)
        self._expo_scale_bessel = expo_scale_bessel
    ...
    def exact_density(self, x0: float, xt: float, t0: float, dt: float) -> float:
        ...
        if self._expo_scale_bessel:
            p = c * np.exp(-(u + v) + np.abs(z)) * (v / u) ** (q / 2)
            p *= ive(q, z)
        else:
            # For testing the (numerically instable) version that does not apply exponential scaling
            p = c * np.exp(-(u + v)) * (v / u) ** (q / 2)
            p *= iv(q, z)
