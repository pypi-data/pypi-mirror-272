...
from .camera_motion import CoordinatesTransformation
...
class Tracker:
    ...
    def select_object(self, global_id, selected=True):
        for obj in self.tracked_objects:
            if obj.global_id == global_id:
                obj.selected = selected


    def get_tracked_object(self, global_id):
        for obj in self.tracked_objects:
            if obj.global_id == global_id:
                return obj
        return None
    ...
    def re_id(self, detection: "Detection") -> None:
        ...
        for obj in self.tracked_objects:
            if obj.global_id == detection.global_id:
                reid_obj = obj
            if obj.object_id == detection.object_id or obj.id == detection.id:
                other_obj = obj

        if reid_obj is not None:
            if (reid_obj.object_score is None or
                    detection.object_score is not None and reid_obj.object_score - 0.02 > detection.object_score):
                reid_obj.object_name = detection.object_name
                reid_obj.object_id = detection.object_id
                reid_obj.object_score = detection.object_score
                reid_obj.id = detection.id
...
class TrackedObject:
    ...
    def __init__(
        ...
        self.selected = False     # object selection state
